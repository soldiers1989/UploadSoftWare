...
{
    public partial class ucDY64000 : BasicContent
    {
        private clsDY6400 dy6400 = new clsDY6400();
        ...
        /// 检测单位、被检单位信息
        /// </summary>
        private string[] BaseUnitInfo = new string[4];
        public ucDY64000()
        {
            InitializeComponent();
        }

        private void ucDY64000_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cdt != null && cdt.Rows.Count > 0)
                {
                    for (int n = 0; n < cdt.Rows.Count; n++)
                    {
                        ...
                        {
                            BaseUnitInfo[0] = cdt.Rows[n]["TestUnitName"].ToString(); //检测单位
                            BaseUnitInfo[1] = cdt.Rows[n]["DetectUnitName"].ToString();//被检单位
                            BaseUnitInfo[2] = cdt.Rows[n]["SampleAddress"].ToString();//采样地址
                            BaseUnitInfo[3] = cdt.Rows[n]["Tester"].ToString();//检测人
                        }
        ...
        private void cmbChkItem_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmbChkItem.Text == "以下相同")
            {
                ...
                {
                    CheckDatas.Rows[k].Cells["检测项目"].Value = CheckDatas.CurrentCell.Value.ToString();
                }
            }
