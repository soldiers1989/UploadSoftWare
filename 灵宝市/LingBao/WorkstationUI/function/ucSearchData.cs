    ...
    public partial class ucSearchData : UserControl
    {
        ...
        private void DataGridViewModefile()
        {
            if (CheckDatasdd.Rows.Count > 0)
            {
                ...
                CheckDatasdd.Columns["SampleCode"].Visible = false;
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (D == DialogResult.No)//导出全部记录
                {
                    ...
                    P_SaveFileDialog.FileName = "检测数据 "+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                ...
                else if (D == DialogResult.Yes) //导出选中的记录
                {
                    ...
                    SaveFileDialog P_SaveFileDialog = new SaveFileDialog();//创建保存文件对话框对象
                    P_SaveFileDialog.FileName = "检测数据 " + DateTime.Now.ToString("yyyy-MM-dd");
                    P_SaveFileDialog.Filter = "*.xls|*.xls";
    ...
    /// <param name="e"></param>
      private void btnUpload_Click(object sender, EventArgs e)
      {
          try
          {
              ...
              else if (Global.Platform == "LB")//灵宝平台
              {
                  for (int i = 0; i < CheckDatasdd.SelectedRows.Count; i++)
                  {
                      sb.Length = 0;
                      ...
                      if (dt != null && dt.Rows.Count > 0)
                      {
                          ...
                          sb.AppendFormat("&jcdw={0}", dt.Rows[0]["DetectUnit"].ToString().Trim());
                          sb.AppendFormat("&rwbh={0}", "");
                          sb.AppendFormat("&bjdw={0}", dt.Rows[0]["CheckUnit"].ToString().Trim());
                          sb.AppendFormat("&jcxm={0}", dt.Rows[0]["Checkitem"].ToString().Trim());
                          sb.AppendFormat("&jcdt={0}", dt.Rows[0]["CheckTime"].ToString().Trim());
                          sb.AppendFormat("&jcz={0}", dt.Rows[0]["CheckData"].ToString().Trim());
                          sb.AppendFormat("&jczdw={0}", dt.Rows[0]["Unit"].ToString().Trim());
                          sb.AppendFormat("&jgpd={0}", dt.Rows[0]["Result"].ToString().Trim());
                          sb.AppendFormat("&ybbh={0}", "");
                          sb.AppendFormat("&ybmc={0}", dt.Rows[0]["SampleName"].ToString().Trim());
                          sb.AppendFormat("&ybcd={0}", "");
                          sb.AppendFormat("&xlbz={0}", dt.Rows[0]["TestBase"].ToString().Trim());
                          sb.AppendFormat("&sbbh={0}", dt.Rows[0]["MachineNum"].ToString().Trim());
                          sb.AppendFormat("&xlz={0}", dt.Rows[0]["LimitData"].ToString().Trim());
                          sb.AppendFormat("&jcy={0}", dt.Rows[0]["Tester"].ToString().Trim());
                          ...
                          if (rtn.Contains("status") || rtn.Contains("info"))
                          {
                              ...
                              {
                                  IsSuccess = IsSuccess + 1;
                                  sql.SetUploadResult(dt.Rows[0]["ID"].ToString().Trim(), out err);
                                  CheckDatasdd.SelectedRows[i].DefaultCellStyle.ForeColor = Color.Green;
                              }
                              else if(rd != null)
                              {
                  ...
                  {
                      MessageBox.Show("共成功上传" + IsSuccess.ToString() + "条数据！", "数据上传");
                  }
                  else if (errstr == "" && isupdata != 0)
                  {
                      MessageBox.Show("共成功能够上传" + IsSuccess.ToString() + "条数据； 共" + isupdata + "条数据已传！", "数据上传");
                  }
                  else
                  {
          ...
          {
              dy.savediary(DateTime.Now.ToString(), "数据上传失败：" + ex.Message, "数据上传");
