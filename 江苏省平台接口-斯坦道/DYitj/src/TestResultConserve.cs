            ...
            try
            {
                ...
                for (int i = 0; i < result.GetLength(0); i++)
                {
                    if (!result[i, 4].ToString().Contains("NA") && !result[i, 4].ToString().Contains("*") && !string.IsNullOrEmpty(result[i, 4]))
                    {
                        ...
                        if (Cpoint != null)
                        {
                            _model.CheckPlace = Cpoint.Organization;
                            _model.CheckPlaceCode = Cpoint.CheckPlaceCode;
                            _model.CheckUnitName = Cpoint.CheckPointName;
                            _model.APRACategory = Cpoint.CheckPointType;
                        }
                        else
                        {
