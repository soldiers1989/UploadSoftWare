...
namespace DYSeriesDataSet
{
    ...
    public class clsCompanyOpr
    {
        ...
        public int DeleteByID(int ID, out string sErrMsg)
        {
            ...
            try
            {
                string deleteSql = string.Format("DELETE FROM company WHERE ID={0}", ID);
        ...
        public DataTable GetAsDataTable(string whereSql, string orderBySql, int type)
        {
            ...
            try
            {
                ...
                else if (type == 2)
                {
                    ...
                    sb.Append("SELECT * FROM company");//tCompany
                ...
                else if(type ==13)//查询摊位编号
                    sb.Append("SELECT * FROM JSstall");
