...
namespace DY.FoodClientLib
{
    ...
    public class clsStandardOpr
    {
        ...
        /// <param name="code"></param>
        /// <returns></returns>
        public static string GetStandValueFromCode(string code)
        {
            string errMsg = string.Empty;
            if (string.IsNullOrEmpty(code))
            {
            ...
            try
            {
                string sql = string.Format("SELECT StandardValue FROM tCheckItem WHERE StandardCode='{0}'", code);
                object obj = DataBase.GetOneValue(sql, out errMsg);
                if (obj == null || obj == DBNull.Value)
                {
                    return string.Empty;
        ...
        /// <param name="code"></param>
        /// <returns></returns>
        public static string GetSignValueFromCode(string code)
        {
            string errMsg = string.Empty;
            if (string.IsNullOrEmpty(code))
            {
            ...
            try
            {
                string sql = string.Format("SELECT DemarcateInfo FROM tCheckItem WHERE StandardCode='{0}'", code);
                object obj = DataBase.GetOneValue(sql, out errMsg);
                if (obj == null || obj == DBNull.Value)
                {
                    return string.Empty;
