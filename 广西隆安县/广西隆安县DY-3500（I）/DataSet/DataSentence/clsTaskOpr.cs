    ...
    public class clsTaskOpr
    {
            ...
            try
            {
                ...
                if (type == 1)
                {
                    ...
                    if (!SampleName.Equals(string.Empty) && !Name.Equals(string.Empty))
                    {
                        ...
                        sb.AppendFormat(IsPreciseQuery ? " AND Name = '{0}'" : " AND Name = '{0}'", Name);
                    }
                    else if (SampleName.Equals(string.Empty) && !Name.Equals(string.Empty))
                    {
                        sb.AppendFormat(IsPreciseQuery ? " WHERE Name = '{0}'" : " WHERE Name = '{0}'", Name);
                    }
                    else if (!SampleName.Equals(string.Empty) && Name.Equals(string.Empty))
                    {
                        sb.AppendFormat(" WHERE FtypeNmae Like '%{0}%'", SampleName);
        ...
        public DataTable GetAsDataTable(string whereSql, string orderBySql, int type)
        {
            ...
            try
            {
                ...
                else if(type ==4)//下载抽样任务
                {
                    sb.Append("SELECT * FROM taskSample");
                }
        ...
        public int InsertTask(downsample model,out string errMsg )
        {
            int rtn = 0;
            errMsg = "";
            try
            {
                sb.Clear();
                sb.Append("Insert Into taskSample(cdName,cdIdNum,regBusLicence,regName,regContactPerson,sampleNO,foodType,foodName,checkItem,");
                sb.Append("checkAccord,productionPlace,nominalSourcePlace,takeInNumber,takeInDate,supplier,supplierContact,supplierPhone,isTest)values(");
                sb.AppendFormat("'{0}',",model.cdName );
                sb.AppendFormat("'{0}',", model.cdIdNum);
                sb.AppendFormat("'{0}',", model.regBusLicence);
                sb.AppendFormat("'{0}',", model.regName);
                sb.AppendFormat("'{0}',", model.regContactPerson);
                sb.AppendFormat("'{0}',", model.sampleNO);
                sb.AppendFormat("'{0}',", model.foodType);
                sb.AppendFormat("'{0}',", model.foodName );
                sb.AppendFormat("'{0}',", model.checkItem);
                sb.AppendFormat("'{0}',", model.checkAccord);
                sb.AppendFormat("'{0}',", model.productionPlace);
                sb.AppendFormat("'{0}',", model.nominalSourcePlace);
                sb.AppendFormat("'{0}',", model.takeInNumber);
                sb.AppendFormat("'{0}',", model.takeInDate );
                sb.AppendFormat("'{0}',", model.supplier );
                sb.AppendFormat("'{0}',", model.supplierContact );
                sb.AppendFormat("'{0}',", model.supplierPhone );
                sb.AppendFormat("'{0}')", "否");
                DataBase.ExecuteCommand(sb.ToString(), out errMsg);
                rtn = 1;
            }
        ...
        public int DeleteTask(string where, string order, out string errMsg)
        {
            int rtn = 0;
            errMsg = "";
            try 
            {
                sb.Clear();
                sb.Append("delete from taskSample " );
                if(where !="")
                {
