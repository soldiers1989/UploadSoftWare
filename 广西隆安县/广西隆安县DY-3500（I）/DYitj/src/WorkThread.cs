...
namespace AIO
{
    public class WorkThread : ChildThread, IDisposable
    {
            ...
            switch (msg.what)
            {
                ...
                case MsgCode.MSG_CHECK_CONNECTION:
                    ...
                    try
                    {
                        ...
                        else if (Global.InterfaceType.Equals("LA"))//广西隆安县
                        {
                            msg.errMsg = "";
                            string url = InterfaceHelper.GetGguangxiAddr(msg.str1 ,1);
                            string postData = string.Format("{0}?userName={1}&passWord={2}&serialNumber={3}&deviceType={4}", url, msg.str2,Global.MD5(msg.str3), msg.str4, msg.str5);
                            msg.responseInfo = InterfaceHelper.registerUser(postData, "");
                            if (msg.responseInfo != "" && msg.responseInfo.Contains("resultCode"))
                            {
                                LAResult rtn= JsonHelper.JsonToEntity<LAResult>(msg.responseInfo);
                                if(rtn.resultCode =="success1")
                                {
                                    msg.result =true ;
                                }
                                else
                                {
                                    msg.errMsg  = rtn.resultDescripe;
                                }
                            }
                ...
                case MsgCode.MSG_UPLOAD:
                    ...
                    else if (Global.InterfaceType.Equals("LA"))//广西隆安
                    {
                        msg.errMsg = LAUploadData((CheckPointInfo)msg.obj1, msg.table);
                    }
                ...
                case MsgCode.MSG_DownTask:
                    ...
                    {
                        string url = InterfaceHelper.GetGguangxiAddr(msg.str1, 2);
                        string postData = string.Format("{0}?userName={1}&passWord={2}&serialNumber={3}&deviceType={4}&startDate={5}&endDate={6}&sampleNO={7}", url, msg.str2, Global.MD5(msg.str3), msg.str4, msg.str5,msg.str6 ,msg.str7,msg.str8);
                        string respon = InterfaceHelper.DownTestSample(postData, "");
                        if (respon != "" && respon.Contains("resultCode"))
                        {
                            clsLASample Sresult = JsonHelper.JsonToEntity<clsLASample>(respon);
                            if (Sresult.resultCode == "success1")
                            {
                                msg.result = true;
                                if(Sresult.result!=null && Sresult.result.ToString().Contains("cdName"))
                                {
                                    msg.responseInfo = Sresult.result.ToString();
                                   
                                }
                                else
                                {
                                    msg.errMsg = Sresult.resultDescripe;
                                }
                            }
                            else
                            {
        ...
        /// </summary>
        /// <returns></returns>
        private string LAUploadData(CheckPointInfo server, DataTable dt)
        {
            ...
            try
            {
                ...
                if(dtList !=null && dtList.Count >0)
                {
                    ...
                    {
                        clsLAUpData upd = new clsLAUpData();
                        List<resultdetail> Dlist = new List<resultdetail>();
                        resultdetail detail = new resultdetail();
                        List<ChkRList> rlist = new List<ChkRList>();
                        ChkRList clist = new ChkRList();
                        clist.checkUser = dtList[i].CheckUnitInfo;
                        clist.checkConclusion = dtList[i].Result;
                        clist.checkDate = dtList[i].CheckStartDate;
                        clist.checkItem = dtList[i].CheckTotalItem;
                        clist.checkMethod = dtList[i].CheckMethod;
                        clist.checkResult = dtList[i].CheckValueInfo;
                        clist.limitValue = dtList[i].StandValue;
                        rlist.Add(clist);
                        clist.sysCode = dtList[i].SysCode.Replace('-',' ').Trim();
                        detail.sampleNO = dtList[i].CheckPlanCode;
                        detail.checkResultList = rlist;
                        Dlist.Add(detail);
                        upd.result = Dlist;

                        string json = JsonHelper.EntityToJson(upd);
                        string up= InterfaceHelper.LAUpload(server.url ,server.user ,server.pwd ,server.serial ,server.mechineType ,json);
                        if (up != "" && up.Contains("resultCode"))
                        {
                            LAResult rr = JsonHelper.JsonToEntity<LAResult>(up);
                            if (rr.resultCode == "success1")
                            {
                            ...
                            {
                                rtn += string.Format("样品名称：[{0}] 异常信息：\r\n{1}\r\n", dtList[i].FoodName , rr.resultDescripe);
                                Global.UploadFCount++;
                            }
                        }
                        else
                        {
                            rtn += string.Format("样品名称：[{0}] 异常信息：\r\n{1}\r\n", dtList[i].FoodName, up);
                            Global.UploadFCount++;
                        }
