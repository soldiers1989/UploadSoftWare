...
namespace AIO.src
{
    ...
    public static class InterfaceHelper
    {
        ...
        public static string registerUser(string url, string paramsData)
        {
             responseInfo = string.Empty;
             responseInfo= HttpPost("POST", url, paramsData);
             return responseInfo;
        }
        /// <summary>
        /// HTTP接口测试
        ...
        /// <returns></returns>
        public static string HttpPost(string type, string url, string paramsValue)
        {
            string result = string.Empty;
            try
            {
                //HttpUtility.UrlEncode(paramsContent, Encoding.UTF8);
        ...
        public static string LAUpload(string url, string user, string pwd,string serialNum, string deviceType, string json)
        {
            responseInfo = string.Empty;
            string urlPost = string.Format("{0}?userName={1}&passWord={2}&serialNumber={3}&deviceType={4}", GetGguangxiAddr(url, 3), user, Global.MD5(pwd), serialNum, deviceType); ;
          
            responseInfo = HttpPost("POST", urlPost, json);
            return responseInfo;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static string DownTestSample(string url,string paramarValue)
        {
            responseInfo = string.Empty;
            try
            {
                responseInfo=HttpPost("POST", url, paramarValue);
            }
            catch (Exception ex)
            {
                 responseInfo = ex.Message;
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetGguangxiAddr(string url, int type)
        {
            int index = url.LastIndexOf('/');
            //服务器通讯验证
            if (type == 1)
            {
                url += (index == url.Length - 1) ? "checkUser.do" : "/checkUser.do";
            }
            //样品信息下载
            else if (type == 2)
            {
                url += (index == url.Length - 1) ? "downloadSample.do" : "/downloadSample.do";
            }
            //数据上传
            else if (type == 3)
            {
                url += (index == url.Length - 1) ? "upLoadCheckData.do" : "/upLoadCheckData.do";
            }
            return url;
        }
