...
namespace AIO
{
    ...
    public partial class JtjEditItemWindow : Window
    {
            ...
            try
            {
                ...
                if (null != _item)
                {
                    ...
                    for (int method = 0; method < 5; ++method)
                    {
                        if (0 == method)
                        {
                            TextBoxDXXXPlusT.Text = string.Empty + _item.dxxx.Abs_Max;
                            TextBoxDXXXMinusT.Text = string.Empty + _item.dxxx.Abs_Min;
                        }
                        else if (1 == method)
                        {
                            TextBoxDXBSPlusT.Text = _item.dxbs.Abs_Max.ToString();
                            TextBoxDXBSMinusT.Text = _item.dxbs.Abs_Min.ToString();
                        }
                        else if (2 == method)
                        {
            ...
            try
            {
                if (Check())
                {
                    ...
                    if (sender == null && e == null)
                    {
                        ...
                        //int.TryParse(JTJItemNameType[9].ToString(), out item.Wave);//波长
                        //检测方法选择
                    ...
                    if (0 == _item.Method)
                    {
                        double.TryParse(TextBoxDXXXPlusT.Text, out _item.dxxx.Abs_Max);
                        double.TryParse(TextBoxDXXXMinusT.Text, out _item.dxxx.Abs_Min);
                    }
                    else if (1 == _item.Method)
                    {
                        double.TryParse(TextBoxDXBSPlusT.Text, out _item.dxbs.Abs_Max);
                        double.TryParse(TextBoxDXBSMinusT.Text, out _item.dxbs.Abs_Min);
                    }
        ...
        private void ComboBoxMethod_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            Btn_CalculatedCurve.Visibility = TabControlJTJMethod.SelectedIndex == 0 ? Visibility.Collapsed  : Visibility.Collapsed;
        }
        ...
        private void BtnSettingAll_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (Global.jtjItems[i].Method != _item.Method) continue;
                    Global.jtjItems[i].InvalidC = int.Parse(TextBoxInvalidC.Text.Trim());
                    Global.jtjItems[i].SampleNum = int.Parse(TextBoxSampleNum.Text.Trim());
                    Global.jtjItems[i].Unit = TextBoxUnit.Text.Trim();
                    Global.jtjItems[i].Password = TextBoxPassword.Text.Trim();
                    Global.jtjItems[i].HintStr = TextBoxHintStr.Text.Trim();
                    if (Global.jtjItems[i].Method == 0)//定性消线
                    {
                        double.TryParse(TextBoxDXXXPlusT.Text, out Global.jtjItems[i].dxxx.Abs_Max);
                        double.TryParse(TextBoxDXXXMinusT.Text, out Global.jtjItems[i].dxxx.Abs_Min);
                    }
                    else if (Global.jtjItems[i].Method == 1)//定性比色
                    {
                        double.TryParse(TextBoxDXBSPlusT.Text, out Global.jtjItems[i].dxbs.Abs_Max);
                        double.TryParse(TextBoxDXBSMinusT.Text, out Global.jtjItems[i].dxbs.Abs_Min);
                    }
                    else if (Global.jtjItems[i].Method == 2)//定量T
                    {
                        double.TryParse(TextBoxDLTA0.Text.Trim(), out Global.jtjItems[i].dlt.A0);
                        double.TryParse(TextBoxDLTA1.Text.Trim(), out Global.jtjItems[i].dlt.A1);
                        double.TryParse(TextBoxDLTA2.Text.Trim(), out Global.jtjItems[i].dlt.A2);
                        double.TryParse(TextBoxDLTA3.Text.Trim(), out  Global.jtjItems[i].dlt.A3);
                        double.TryParse(TextBoxDLTB0.Text.Trim(), out Global.jtjItems[i].dlt.B0);
                        double.TryParse(TextBoxDLTB1.Text.Trim(), out Global.jtjItems[i].dlt.B1);
                        double.TryParse(TextBoxDLTPPB.Text.Trim(), out Global.jtjItems[i].dlt.Limit);
                    }
                    else if (Global.jtjItems[i].Method == 3)//定量T/C
                    {
                        double.TryParse(TextBoxDLTCA0.Text.Trim(), out Global.jtjItems[i].dltc.A0);
                        double.TryParse(TextBoxDLTCA1.Text.Trim(), out Global.jtjItems[i].dltc.A1);
                        double.TryParse(TextBoxDLTCA2.Text.Trim(), out  Global.jtjItems[i].dltc.A2);
                        double.TryParse(TextBoxDLTCA3.Text.Trim(), out Global.jtjItems[i].dltc.A3);
                        double.TryParse(TextBoxDLTCB0.Text.Trim(), out Global.jtjItems[i].dltc.B0);
                        double.TryParse(TextBoxDLTCB1.Text.Trim(), out Global.jtjItems[i].dltc.B1);
                        double.TryParse(TextBoxDLTCPPB.Text.Trim(), out Global.jtjItems[i].dltc.Limit);
                    }
                    count++;
                }
                _isUpdate = false;
        ...
        private void Btn_CalculatedCurve_Click(object sender, RoutedEventArgs e)
        {
            if (MessageBox.Show("确定要重新配置当前曲线内容吗?\r\n\r\nTips:曲线配置一般为兼容第三方检测卡，非专业人士请勿修改！", "操作提示", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
            {
                ...
                if (Global.CurveValue != null && Global.CurveValue.Length > 0 && Global.CurveValue.IndexOf(',') > 0)
                {
                    ...
                    {
                        TextBoxDXXXMinusT.Text = vals[0];
                        TextBoxDXXXPlusT.Text = vals[1];
                    }
                }
            }
        }
