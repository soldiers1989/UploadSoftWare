...
namespace AIO.xaml.Dialog
{
    ...
    public partial class StandardRepositoryWindow : Window
    {
        ...
        public string _standard = string.Empty;
        /// <summary>
        /// 当前页数
        /// </summary>
        ...
        private void LoadStandard()
        {
            ...
            whereSql += whereSql.Length == 0 ? "IsDeleted = 'N' " : " And IsDeleted = 'N' ";
        ...
        private string pdfUrl = "ftp://xiaoyl:123456@120.24.239.96:54321/pdf/";
        ...
        private void Btn_Del_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (_SelectStandards != null && _SelectStandards.Count > 0)
            {
                if (MessageBox.Show("确定要删除当前所选数据吗?", "操作提示", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.No)
                {
                    return;
                }
                ...
                for (int i = 0; i < _SelectStandards.Count; i++)
                {
                    sql = string.Format("Update FoodStandard Set IsDeleted='Y' where Name = '{0}' And ReleaseUnit = '{1}' And Path = '{2}' And ImplementationTime = '{3}'",
                        _SelectStandards[i].Name, _SelectStandards[i].ReleaseUnit, _SelectStandards[i].Path, _SelectStandards[i].ImplementationTime);
                    SqlHelper.Update(sql, out sql);
                    if (sql.Length == 0)
                    {
                        delNum++;
                    }
                ...
                if (delNum > 0)
                {
                    MessageBox.Show(string.Format("成功删除 {0} 条标准信息！", delNum));
                    LoadStandard();
                }
                else
                {
        ...
        private void Label_PreviewMouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            string sql = "alter table FoodStandard add IsDeleted varchar(10)";
            bll.DataBaseRepair(sql, out sql);

            sql = "Update FoodStandard Set IsDeleted='N'";
            SqlHelper.Update(sql, out sql);
            LoadStandard();
        }
