            ...
            try
            {
                if (DataGridRecord.SelectedItems.Count > 0)
                {
                    if (!DataGridRecord.SelectedItems[0].ToString().Equals("{NewItemPlaceholder}"))
                    {
                        ...
                        string[] vals = _decide.ItemDes.Split(' ');
                        if (vals.Length > 1)
                        {
                            window._standard = vals[1];
                        }
                        else
                        {
                            window._standard = _decide.ItemDes;
                        }
                        //window._standard = _decide.ItemDes;//.Substring(5, _decide.ItemDes.Length - 5);
