            ...
            switch (msg.what)
            {
               ...
               //通信测试
                case MsgCode.MSG_CHECK_CONNECTION:   
                    ...
                    try
                    {
                        postreturn= InterfaceHelper.QDCommunicate(msg.str1, msg.str2, msg.str3);
                        if (postreturn == "上传成功0条，失败0条，重复0条！")
                        {
                            msg.result = true;
                        }
                ...
                case MsgCode.MSG_UPLOAD:
                    ...
                    CheckPointInfo webinfo = (CheckPointInfo)msg.obj1;
                    ...
                    for(int i=0;i<msg.selectedRecords.Count ;i++)
                    {
                        ...
                        sb.Append(3);
                        sb.Append(",username:'");
                        sb.Append(webinfo.user);
                        sb.Append("',password:'");
                        sb.Append(webinfo.pwd);
                        sb.Append("',items:[{");
                        sb.AppendFormat("spdm:'{0}',", msg.selectedRecords[i].SampleCode);
                        sb.AppendFormat("spmc:'{0}',", msg.selectedRecords[i].FoodName);
                        sb.AppendFormat("jcxm:'{0}',", msg.selectedRecords[i].CheckTotalItem);
                        sb.AppendFormat("jcz:'{0}',", msg.selectedRecords[i].CheckValueInfo);
                        sb.AppendFormat("jcjg:'{0}',", msg.selectedRecords[i].Result);
                        sb.AppendFormat("jcrq:'{0}',", msg.selectedRecords[i].CheckStartDate);
                        sb.AppendFormat("twh:'{0}',", msg.selectedRecords[i].stallnum == "" ? "11" : msg.selectedRecords[i].stallnum);//摊位号
                        sb.AppendFormat("weight:'{0}',", msg.selectedRecords[i].weightg == "" ? "2.1" : msg.selectedRecords[i].weightg);//重量
                        sb.AppendFormat("cljg:'{0}'", msg.selectedRecords[i].handstyle == "" ? "已处理" : msg.selectedRecords[i].handstyle);//处理方式
                        sb.Append("}");
                        sb.Append("]}");

                        Uri uri = new Uri(webinfo.url);
                        HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create(uri);
                        webRequest.Method = "POST";
                        webRequest.ContentType = "application/x-www-form-urlencoded";
                        webRequest.Timeout = 5000;
                        byte[] paramBytes = Encoding.Default.GetBytes(sb.ToString());
