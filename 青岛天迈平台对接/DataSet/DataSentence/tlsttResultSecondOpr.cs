    ...
    public class tlsttResultSecondOpr
    {
        ...
        public int Insert(tlsTtResultSecond model, out string errMsg)
        {
            ...
            try
            {
                ...
                sql.Append(" CheckPlanCode,Hole,CheckMethod,APRACategory,CheckType,DateManufacture,ContrastValue,DeviceId,SampleId,ProduceCompany,SysCode,stallnum,weightg,handstyle) ");
                sql.Append(" VALUES(");
                ...
                sql.AppendFormat("'{0}',", model.stallnum);
                sql.AppendFormat("'{0}',", model.CheckedCompany);
                sql.AppendFormat("'{0}')", model.ProduceCompany);
                DataBase.ExecuteCommand(sql.ToString(), out errMsg);
                sql.Length = 0;
