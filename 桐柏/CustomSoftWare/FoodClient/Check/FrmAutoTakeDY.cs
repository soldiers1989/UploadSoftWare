...
namespace FoodClient
{
    public partial class FrmAutoTakeDY : TitleBarBase
    {
        ...
        private   clsFoodClassOpr _foodBll = new clsFoodClassOpr();
        //时间控件
        ...
        private void c1FlexGrid1_Click(object sender, System.EventArgs e)
        {
            ...
            if (c1FlexGrid1.RowSel >= 1)
            {
                ...
                if (_tag != "DYRSY2" && _tag != "DY6400")//两款肉类水分仪除外
                {
                    ...
                    cmbFood.Text = c1FlexGrid1.Rows[_intRowSel]["样品名称"].ToString();//LZ4000自动赋值
                    dtpTakeDate.Value = dtpCheckStartDate.Value;

                    DataTable dt=  _foodBll.GetAsDataTable("IsLock=false And IsReadOnly=true and Name='" + cmbFood.Text.Trim()+"'", "SysCode", 0);
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        string stdcode = dt.Rows[0]["StdCode"].ToString();
                        _foodSelectedValue = stdcode;
                        string sysCode = stdcode.Substring(0, 10);
                        dt = _foodBll.GetAsDataTable("IsLock=false And IsReadOnly=true and SysCode= '" + sysCode + "'", "SysCode", 0);
                        if (dt.Rows.Count > 0)
                        {
                    ...
                    {
                        cmbFood.Text = "";
                        MessageBox.Show("检测到系统不存在该样品！\r\n请到数据中心录入或选择其他已存在的样品!","数据录入",MessageBoxButtons.OK ,MessageBoxIcon.Information);
                    }
