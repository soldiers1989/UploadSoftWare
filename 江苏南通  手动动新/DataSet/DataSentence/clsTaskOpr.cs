...
namespace DYSeriesDataSet
{
    ...
    public class clsTaskOpr
    {
        ...
        public int UpDateNTtask(clsNTtaskresult sel, out string errMsg)
        {
             errMsg = string.Empty;
             int rtn = 0;
             try
             {
                 sb.Length = 0;
                 sb.AppendFormat("UPDATE NTtask SET istest='{0}'", "是");
                 sb.AppendFormat(" WHERE SampleID='{0}' and SampleName='{1}' and ItemName='{2}' and tasktime='{3}'", sel.SampleID, sel.SampleName, sel.ItemName ,sel.tasktime);
        ...
        public DataTable SearchSampleStd(string Tsample,string Titem, int type)
        {
            ...
            try
            {
                ...
                if (type == 1)
                {
                    ...
                    if (!Tsample.Equals(string.Empty))
                    {
                        sb.AppendFormat(" WHERE FtypeNmae = '{0}' and Name='{1}'", Tsample, Titem);                   
                    }                  
                }
                else if (type == 2)
                {
                    sb.AppendFormat("Select Top 1 ID,FtypeNmae,SampleNum,Name,ItemDes,StandardValue,Demarcate,Unit,UDate From ttStandDecide Where SampleNum Like'{0}'", Tsample);
                }
        ...
        public DataTable GetNTtask(string whereSql, string orderBySql, int type)
        {
            string errMsg = string.Empty;
            DataTable dtbl = null;
            ...
            try
            {
                ...
                {
                    sb.Append("SELECT * FROM NTtask");
                }
                else if (type == 2)
                {
                    sb.Append("SELECT SampleID,SampleName,ItemID,ItemName,tasktime  FROM NTtask");
                }
                if (type == 3)
                {
                    // SELECT CPCODE,CPTITLE,CPSDATE,CPEDATE,CPTPROPERTY,CPFROM,CPEDITOR,CPPORGID,CPPORG,CPEDDATE,PLANDETAIL,
        ...
        /// <param name="errMsg"></param>
        /// <returns></returns>
        public int InsertNTtask(clsTask model, out string errMsg)
        {
            errMsg = string.Empty;
            int rtn = 0;
            try
            {
                ...
                sb.Append("Insert Into NTtask");
                sb.Append("(SampleID,SampleName,ItemID,ItemName,istest,tasktime)");
                sb.Append("VALUES(");
                sb.AppendFormat("'{0}',", model.sampleid);
                sb.AppendFormat("'{0}',", model.samplename);
                sb.AppendFormat("'{0}',", model.itemid);
                sb.AppendFormat("'{0}',", model.itemname);
                sb.AppendFormat("'{0}',", model.istest);
                sb.AppendFormat("'{0}'", model.tasktime);
                sb.Append(")");
