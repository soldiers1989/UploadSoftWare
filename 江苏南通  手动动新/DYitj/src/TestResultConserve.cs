            ...
            try
            {
                ...
                for (int i = 0; i < result.GetLength(0); i++)
                {
                    if (!result[i, 4].ToString().Contains("NA") && !result[i, 4].ToString().Contains("*") && !string.IsNullOrEmpty(result[i, 4]))
                    {
                        ...
                        _model.MachineCompany = Global.MachineMaker;  //"广东达元绿洲食品安全科技股份有限公司"; //检测设备生产厂家
                        _model.ContrastValue = string.Empty;
                        _model.SampleId = result[i, 15];
                        _model.DeviceId = Global.MachineSerial; //Wisdom.DeviceID;
                        //_model.ProduceCompany = Global._selSampleID;
                        _model.SAMPLEUUID = result[i, 16];

                        _bll.Insert(_model, out ourEgg);
                        if (ourEgg.Equals(string.Empty))
                            CountLength++;

                        clsNTtaskresult testtask = new clsNTtaskresult();
                        testtask.SampleID = result[i, 16];
                        testtask.SampleName = result[i, 8];
                        testtask.ItemName = result[i, 2];
                        testtask.tasktime = result[i, 15];

                        _Tskbll.UpDateNTtask(testtask, out ourEgg);
                    }
                }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                for (int i = 0; i < forLenght; i++)
                {
                    if (!result[i, 4].ToString().Contains("NA") && !result[i, 4].ToString().Contains("*") && !string.IsNullOrEmpty(result[i, 4]))
                    {
                        ...
                        _model.SAMPLEUUID = result[i, 16];//南通样品编号
                        ...
                        {
                            testtask.SampleID = Global.NTCheckMassage[i, 2]; 
                            testtask.SampleName = Global.NTCheckMassage[i, 1];
                            testtask.ItemName = Global.NTCheckMassage[i, 0]; 
                            testtask.tasktime = Global.NTCheckMassage[i, 3]; 
                        }
                        else 
                        {
                            testtask.SampleID = result[i, 16];
