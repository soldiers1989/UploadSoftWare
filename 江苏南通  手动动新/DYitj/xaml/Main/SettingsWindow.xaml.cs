    ...
    public partial class SettingsWindow : Window
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                textBoxRegisterID.Text = Global.LoggonNmae;
                ...
                textBoxCheckNumber.Text = Global.MachineMaker;
                textBoxCheckName.Text = Global.MachineSerial;
        ...
        private void ButtonServerTest_Click(object sender, RoutedEventArgs e)
        {
            ...
            string[] a = addr.Split('?');
            string inter = a[0].Substring(a[0].Length - 12, 12);
            string address = "";
            if (inter == "Testplaninfo")
            {
                address = a[0] ;
            }
            else
            ...
            string configString = @"configuration/applicationSettings/AIO.Properties.Settings/setting[@name='DY_Detector_WebReference_Testplaninfo']/value";
        ...
        private void btnUsersave_Click(object sender, RoutedEventArgs e)
        {
            ...
            CFGUtils.SaveConfig("LogonUser", textBoxRegisterID.Text.Trim());
            Global.LoggonNmae = textBoxRegisterID.Text.Trim();
            MessageBox.Show("数据保存成功", "操作提示");
            btnUsersave.IsEnabled = true;
        }
        /// <summary>
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnMachineProdu_Click(object sender, RoutedEventArgs e)
        {
            btnMachineProdu.IsEnabled = false;
            if (textBoxCheckNumber.Text.Trim() == "")
            {
                MessageBox.Show("设备厂家不能为空", "提示");
                btnMachineProdu.IsEnabled = true;
                return;
            }
            CFGUtils.SaveConfig("MachineMaker", textBoxCheckNumber.Text.Trim());
            Global.MachineMaker = textBoxCheckNumber.Text.Trim();
            MessageBox.Show("数据保存成功", "操作提示");
            btnMachineProdu.IsEnabled = true;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnSearial_Click(object sender, RoutedEventArgs e)
        {
            btnSearial.IsEnabled = false;
            if (textBoxCheckName.Text.Trim() == "")
            {
                MessageBox.Show("设备唯一码不能为空", "提示");
                btnSearial.IsEnabled = true;
                return;
            }
            CFGUtils.SaveConfig("MachineSerial", textBoxCheckName.Text.Trim());
            Global.MachineSerial = textBoxCheckName.Text.Trim();
            MessageBox.Show("数据保存成功", "操作提示");
            btnSearial.IsEnabled = true;
        }
