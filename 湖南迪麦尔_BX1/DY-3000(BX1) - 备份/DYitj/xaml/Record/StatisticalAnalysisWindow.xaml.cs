    ...
    public partial class StatisticalAnalysisWindow : Window
    {
        ...
        private void Search()
        {
            ...
            try
            {
                ...
                //用户名
                val = LoginWindow._userAccount.UserName;
                if (val.Length > 0 )
                {
                    where.AppendFormat(" {0} CheckUnitInfo ='{1}'", (where.Length == 0 ? "" : "And"), val);
                }
        ...
        private void ShowColumnChart(List<tlsTtResultSecond> models)
        {
            ...
            for (int i = 0; i < models.Count; i++)
            {
                ...
                else
                {
                    ...
                    dicNoQualifiedDateTrendList[key] += ishg ? 0 : 1;
                }
            }
            ...
            //折线图
            lineChart1.ItemsSource = dicDateTrendList;//总数
            lineChart2.ItemsSource = dicQualifiedDateTrendList;//合格
            lineChart3.ItemsSource = dicNoQualifiedDateTrendList;//不合格

            int Qualified = 0;
            int buhege = 0;
            try 
            {
                foreach (string skey in dicPercentOfPassList.Keys)
                {

                    if(skey=="合格")
                    {
                        Qualified = dicPercentOfPassList[skey];
                    }
                    else if (skey == "不合格")
                    {
                        buhege =  dicPercentOfPassList[skey];
                    }
                   

                }
                //Qualified = dicPercentOfPassList["合格"];
                //buhege = dicPercentOfPassList["不合格"];
            }
            ...
            LbUnqualified.Content = string.Format("不合格数：{0}   不合格率占比：{1}", (buhege), ((double)(buhege) / count).ToString("P"));//(count - Qualified), ((double)(count - Qualified) / count).ToString("P")
            LbOtheer.Content = string.Format("其他：{0}   其他占比：{1}", (count - Qualified - buhege), ((double)(count - Qualified - buhege) / count).ToString("P"));
        }
