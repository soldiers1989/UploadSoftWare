            ...
            try
            {
                ...
                for (int i = 0; i < Global.deviceHole.HoleCount; ++i)
                {
                    ...
                    if (_item.Method == 0 || _item.Method == 3)
                    {
                        ...
                        {
                            _use[i] = true;
                            _CheckValue = new string[_HoleNumber, 25];
                            _HoleNumber++;
                        }
                        else
                            _use[i] = false;
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        if (dtList.Count > 0)
                        {
                           ...
                           for(int i=0;i<dtList.Count;i++)
                           {
                                ...
                                {
                                    sb.AppendFormat("名称：{0}\r\n", dtList[i].FoodName, dtList[i].CheckStartDate);

                                    string[] timestr = dtList[i].CheckStartDate.Split(' ');
                                    ...
                                    //二维码
                                    string QrCode = "检测结果：" + dtList[i].Result + "\r\n检测人：" + dtList[i].CheckUnitInfo + "\r\n检测时间：" + dtList[i].CheckStartDate;
                    ...
                    if (_contrast)
                    {
                        ...
                        for (int i = 0; i < Global.deviceHole.HoleCount; ++i)
                        {
                            if (_use[i] && i != contrastIdx)
                            {
                                ...
                                //经营户信息
                                if (_item.Hole[i].OperatorInfo != "" && _item.Hole[i].OperatorInfo.Length > 0)
                                {
                                    string[] strInfo = _item.Hole[i].OperatorInfo.Split(',');
                                    if(strInfo !=null && strInfo.Length ==4)
                                    {
                                        _CheckValue[(num > 0 ? (i - num) : i), 21] = strInfo[0];
                                        _CheckValue[(num > 0 ? (i - num) : i), 22] = strInfo[1];
                                        _CheckValue[(num > 0 ? (i - num) : i), 23] = strInfo[2];
                                        _CheckValue[(num > 0 ? (i - num) : i), 24] = strInfo[3];
                                    }
                                }
                    ...
                    if (_sample)
                    {
                        ...
                        for (int i = 0; i < Global.deviceHole.HoleCount; ++i)
                        {
                            ...
                            if (_use[i])
                            {
                                ...
                                //经营户信息
                                if (_item.Hole[i].OperatorInfo != null && _item.Hole[i].OperatorInfo.Length >0)
                                {
                                    string[] strInfo = _item.Hole[i].OperatorInfo.Split(',');
                                    if (strInfo != null && strInfo.Length == 4)
                                    {
