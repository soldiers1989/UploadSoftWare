...
namespace AIO.src
{
    ...
    public static class InterfaceHelper
    {
        ...
        public static string HttpDME(string type, string url, string paramsValue,int ctype)
        {
            string result = string.Empty;
        ...
        public static string DMEGetUrl(string url,int type)
        {
            int index = url.LastIndexOf('/');
            //服务器通讯验证
            if (type == 1)
            {
                url += (index == url.Length - 1) ? "p/v1/login" : "/p/v1/login";
            }
            else if (type == 2)
            {
                url += (index == url.Length - 1) ? "p/v1/produce" : "/p/v1/produce";
            }
            return url;
        }
