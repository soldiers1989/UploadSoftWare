...
namespace AIO
{
    ...
    public class FgdCaculate
    {
        ...
        // 原始数据转为AD值
        public static HolesAD NewRawDataToAD(byte[] data)
        {
            int index = 0;
            int LED_ROW = 2;
            int LED_COL = 8;
            ...
            for (int i = 0; i < LED_ROW; ++i)
            {
                for (int j = 0; j < LED_COL; ++j)
                {
                    ...
                    {
                        ad.adValues[i][j][k] = ((UInt32)data[index]) + ((UInt32)(data[index + 1] << 8));
                        index += 2;
                    }
                }
            }
        ...
        /// <returns></returns>
        public static double WeiTiaoNew(double x, double a, double b, double c, double d, double dsOrbs,double JA,double JB)
        {
            double xFix = 0;
            if (IsValid(x))
                xFix = (a + b * x + c * x * x + d * x * x * x) * dsOrbs*JA+JB;
            else
        ...
        /// </summary>
        /// <returns></returns>
        public static double RepairStandLineData(double x,double a,double b)
        {
            double std = 0;
