...
namespace AIO
{
    public class WorkThread : ChildThread, IDisposable
    {
            ...
            switch (msg.what)
            {
                ...
                case MsgCode.MSG_CHECK_CONNECTION:
                    try
                    {
                        ...
                        {
                            string url = InterfaceHelper.DMEGetUrl(msg.str1, 1);
                            string rtn= InterfaceHelper.HttpDME("POST", url, string.Format("username={0}&password={1}",msg.str2 ,msg.str3 ),1);
                            if (rtn.Contains("code") || rtn.Contains("ok") || rtn.Contains("msg"))
                            {
                                returnList rtndata = Json.JsonToEntity<returnList>(rtn);
                                if (rtndata != null)
                                {
                                    if (rtndata.ok == "True")
                                    {
                                        ...
                                        if (bdy != null)
                                        {
                                            Global.DToken = bdy.token ;
                                            msg.result = true;
                                            users us = Json.JsonToEntity<users>(bdy.user.ToString());
                                            if(us!=null )
                                            {
                                                msg.responseInfo = us.organName;
                                            }
                ...
                case MsgCode.MSG_UPLOAD:
                    ...
                    {
                        msg.outError = UploadDMEResult((CheckPointInfo)msg.obj1,msg.selectedRecords);//UploadResult((CheckPointInfo)msg.obj1, (msg.table));
                    }
        ...
        /// <param name="selectedRecords"></param>
        /// <returns></returns>
        private String UploadDMEResult(CheckPointInfo users ,List<tlsTtResultSecond> selectedRecords)
        {
            string url = InterfaceHelper.DMEGetUrl(users.ServerAddr, 2);
            string err = "";
            try
            {
                ...
                {
                    rtl.userToken = Global.DToken;
                    //时间要转换
                    DateTime dt = DateTime.Parse(selectedRecords[i].CheckStartDate);
                  
                    string dd = GetTimeStamp(dt);

                    string stand = selectedRecords[i].Standard == "" ? "参考国标" : selectedRecords[i].Standard;
                    string resultValue = "";
                    if(selectedRecords[i].ResultType =="分光光度")
                    {
                        resultValue = selectedRecords[i].CheckValueInfo + selectedRecords[i].ResultInfo;
                    }
                    else
                    {
                        resultValue = selectedRecords[i].CheckValueInfo;
                    }

                    string data = "{\"samplename\":\"" + selectedRecords[i].FoodName  + "\","
                     + "\"controlvalue\":\"" + selectedRecords[i].StandValue  + "\","
                     + "\"testproject\":\"" + selectedRecords[i].CheckTotalItem+ "\","
                     + "\"testingtime\":\"" + dd + "\","
                     + "\"testresult\":\"" + resultValue + "\","
                     + "\"inspector\":\"" + selectedRecords[i].CheckUnitInfo + "\","
                     + "\"samplenum\":\"" + selectedRecords[i].SampleCode + "\","
                     + "\"stand_num\":\"" + stand + "\","
                     + "\"deviceNumber\":\"" + selectedRecords[i].CheckMachineModel + "\","
                     + "\"decisionoutcome\":\"" + selectedRecords[i].Result + "\","
                     + "\"cov_unit\":\"" + selectedRecords[i].ResultInfo  + "\","
                     + "\"testsite\":\"" + users.CheckPointName  + "\"}";
                    
                    rtl.body = data;

                    string json = Json.EntityToJson(rtl);

                    string rtn = InterfaceHelper.HttpDME("POST", url, json, 2);
                    if (rtn.Contains("code") || rtn.Contains("ok") || rtn.Contains("msg"))
                    {
                        returnList rtndata = Json.JsonToEntity<returnList>(rtn);
                        if (rtndata != null)
                        {
                            if (rtndata.ok == "True")
                            {
                                _resultTable.UpdateUploads(string.Format("('{0}')", selectedRecords[i].SysCode), out err);
                                Global.UploadSCount++;
                            }
                            else
                            {
