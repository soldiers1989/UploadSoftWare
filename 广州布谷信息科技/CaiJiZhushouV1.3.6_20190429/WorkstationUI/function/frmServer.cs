...
namespace WorkstationUI.function
{
    public partial class frmServer : Form
    {
        ...
        private void frmServer_Load(object sender, EventArgs e)
        {
            ...
            txtDetectUnitNo.Text = ConfigurationManager.AppSettings["photos"];//电话号码
        ...
        private void BtnCommunicate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("设备编号不能为空！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                if (txtOrganize.Text.Trim().Length == 0)
                {
                    MessageBox.Show("所属区域不能为空！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                //设备编号
                Global.MachineSerialCode = txtMachineSerial.Text.Trim();
                //所属区域
                Global.regions = txtOrganize.Text.Trim();
                //实验室编号
                Global.LaboratoryNum = txtOrganizeNo.Text.Trim();
                //检测点名称
                Global.TestUnitName = txtDetectUnit.Text.Trim();
                //负责人
                Global.NickName = txtUserName.Text.Trim();
                //手机号
                Global.PhotoNum = txtDetectUnitNo.Text.Trim();
                ...
                else
                {
                    ...
                    //设备编号
                    string machNum = ConfigurationManager.AppSettings["IntrumentSeriersNum"];
                    if (machNum != null)
                    {
                    ...
                    if (org != null)
                    {
                        cf.AppSettings.Settings["Organize"].Value = Global.regions;
                    }
                    else
                    {
                        cf.AppSettings.Settings.Add("Organize", Global.regions);
                    }

                    //实验室编号
                    string lab = ConfigurationManager.AppSettings["OrganizeNo"];
                    if (lab != null)
                    {
                        cf.AppSettings.Settings["OrganizeNo"].Value = Global.LaboratoryNum;
                    }
                    else
                    {
                        cf.AppSettings.Settings.Add("OrganizeNo", Global.LaboratoryNum);
                    }

                    //检测点名称
                    string Tunit = ConfigurationManager.AppSettings["UpDetectUnit"];
                    if (Tunit != null)
                    {
                        cf.AppSettings.Settings["UpDetectUnit"].Value = Global.TestUnitName;
                    }
                    else
                    {
                        cf.AppSettings.Settings.Add("UpDetectUnit", Global.TestUnitName);
                    }

                    //负责人
                    string name = ConfigurationManager.AppSettings["NickName"];
                    if (name != null)
                    {
                    ...
                    //手机号
                    string photo = ConfigurationManager.AppSettings["photos"];
                    if (photo != null)
                    {
                        cf.AppSettings.Settings["photos"].Value = Global.PhotoNum;
                    }
                    else
                    {
                        cf.AppSettings.Settings.Add("photos", Global.PhotoNum);
                    }
                    cf.Save();
