    ...
    public partial class ucTL310 : Basic.BasicContent
    {
            ...
            try
            {
                ...
                if (CheckDatas.Rows.Count > 0)
                {
                    for (int i = 0; i < CheckDatas.Rows.Count; i++)
                    {
                        ...
                        {
                            if (CheckDatas.Rows[i].Cells["样品名称"].Value.ToString().Trim()=="")
                            {
                                continue;
                            }
                            //查询样品编号
                            ...
                            cdt = sqlSet.GetSamplItemStandard("FtypeNmae='" + CheckDatas.Rows[i].Cells["样品名称"].Value.ToString().Trim() + "' and Name='" +
                                CheckDatas.Rows[i].Cells["检测项目"].Value.ToString().Trim() + "'", "", out err);
                            string samplecode = "";
                            if (cdt != null && cdt.Rows.Count > 0)
                            {
                                samplecode = cdt.Rows[0]["SampleNum"].ToString();
                                //samplecode = cdt.Rows[0]["foodCode"].ToString();
                            }
                            ...
                            //被检单位编号
                            string where = string.Format("DetectUnitName='{0}'", resultdata.CheckUnit);
                            DataTable comdt = sqlSet.GetInformation(where, "", out err);
                            if (comdt != null && comdt.Rows.Count >0)
                            {
                                resultdata.CheckUnitNature = comdt.Rows[0]["DetectUnitNature"].ToString();
                            }
                            else
                            {
                                resultdata.CheckUnitNature = "10011001";
                            }
                            ...
                            resultdata.StallNum = CheckDatas.Rows[i].Cells["摊位号"].Value.ToString().Trim();
        ...
        protected override void btnadd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DialogResult tishi = MessageBox.Show("共有" + IsSave + "条数据是否上传", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                ...
                DataTable dtb = sqlSet.GetResultTable(strWhere.ToString(), "", 1, IsSave, out err);
                if (dtb != null && dtb.Rows.Count > 0)
                {
                    for (int i = 0; i < dtb.Rows.Count; i++)
                    {
                        ...
                        fras.UserName = Global.ServerName;
                        fras.UserPassWord = Global.ServerPassword;
                        ...
                        dd.BoothName = "达元";
                        dd.BoothNum = "B10";
                        dd.SampleCode = dtb.Rows[i]["SampleCode"].ToString();
                        dd.SampleName = dtb.Rows[i]["SampleName"].ToString();
                        dd.DetectionName = dtb.Rows[i]["Checkitem"].ToString();
                        dd.Grade = "";
                        dd.Status = "";
                        dd.DetectionValue = dtb.Rows[i]["CheckData"].ToString();
                        dd.Range = dtb.Rows[i]["LimitData"].ToString();
                        dd.Result = dtb.Rows[i]["Result"].ToString();
                        dd.Unit = dtb.Rows[i]["Unit"].ToString();
                        dd.DetectionDate = dtb.Rows[i]["CheckTime"].ToString();
                        dd.holeNumber = dtb.Rows[i]["HoleNum"].ToString(); 
                        ...
                        string rtn = ZYUpData.HttpPOST(Global.ServerAdd, json);
                        FilesRW.KLog(rtn, "接收", 14);
                ...
                if (errstr == "")
                {
                    MessageBox.Show("共成功能够上传" + upok.ToString() + "条数据；\r\n 共" + isupdata + "条数据已传！", "数据上传");
                }
                else
                {
        ...
        protected override void CheckDatas_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (CheckDatas.CurrentCell.ColumnIndex > -1 && CheckDatas.CurrentCell.RowIndex > -1)
                {
                    ...
                    //else if (CheckDatas.CurrentCell.ColumnIndex == 11)//检测单位
                    //{
                    //    cmbSample.Visible = false;
                    //    cmbChkItem.Visible = false;
                    ...
                    //else if (CheckDatas.CurrentCell.ColumnIndex == 12)//被检单位性质
                    //{
                    //    cmbSample.Visible = false;
                    //    cmbChkItem.Visible = false;
                    //    cmbChkUnit.Visible = false;
                    ...
                    //}
                    else if (CheckDatas.CurrentCell.ColumnIndex == 13)//摊位号
                    {
                        cmbChkItem.Visible = false;
                        cmbSample.Visible = false;
                        cmbChkUnit.Visible = false;
