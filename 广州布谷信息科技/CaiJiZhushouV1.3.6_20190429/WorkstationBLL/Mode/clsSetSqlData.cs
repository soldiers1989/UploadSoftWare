    ...
    {   
        ...
        public int ResuInsert(clsSaveResult result, out string errMsg)
        {
            ...
            try
            {
                ...
                _strBd.AppendFormat("'{0}',", result.Gettime);
        ...
        public int LoadInData(clsSaveResult result, out string errMsg)
        {
            ...
            try
            {
                _strBd.Append("INSERT INTO CheckResult (ChkNum,Checkitem,SampleName,CheckData,Unit,CheckTime,CheckUnit,Result,Save,Machine,SampleTime,SampleAddress,TestBase,LimitData,Tester,");
                ...
                _strBd.Append("Xiguangdu,HoleNum,StallNum)VALUES(");
                _strBd.AppendFormat("'{0}',", result.CheckNumber);
                _strBd.AppendFormat("'{0}',", result.Checkitem);
                _strBd.AppendFormat("'{0}',", result.SampleName);
                _strBd.AppendFormat("'{0}',", result.CheckData);
                _strBd.AppendFormat("'{0}',", result.Unit);
                ...
                _strBd.AppendFormat("'{0}',", result.Result);
                _strBd.AppendFormat("'{0}',", "是");
                _strBd.AppendFormat("'{0}',", result.Instrument);
                _strBd.AppendFormat("'{0}',", result.Gettime == "" ? result.Gettime : result.Gettime);
                _strBd.AppendFormat("'{0}',", result.Getplace);
                _strBd.AppendFormat("'{0}',", result.Testbase);
                _strBd.AppendFormat("'{0}',", result.LimitData);
                _strBd.AppendFormat("'{0}',", result.Tester);
        ...
        public DataTable GetSamplItemStandard(string sql, string orderby, out string errMsg)
        {
            errMsg = string.Empty;
            DataTable dtb1 = null;
            _strBd.Length = 0;
            try
            {
                _strBd.Append("SELECT * from tStandSample");
        ...
        public DataTable ExportExcel(string whereSql, string orderBySql, out string errMsg)
        {
            ...
            try
            {
                ...
                sb.Append("SendTestDate as 送检日期,NumberUnit as 数量单位,CompanyNeture as 被检单位性质,DoResult as 处理结果,HoleNum as 通道号,StockIn as 进货数量,Xiguangdu as 吸光度,StallNum as 摊位号");
