    ...
    public partial class FgdReportWindow : Window
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < HoleCount; ++i)
                {
                    ...
                    {
                        // 对照，只显示第一个。
                        string TempRevice = _item.Hole[i].SampleName;
                        if (_item.SampleName.Count > 0)
                        {
                        ...
                        {
                            UIElement element = GenerateReportLine(String.Format("{0:D2}", (i + 1)), String.Format("{0:D5}", _contrast ? 0 : sampleNum++), string.Empty, string.Empty + _realHeatms / 1000, _item.Unit, string.Empty, string.Empty, string.Empty);
                            StackPanelReport.Children.Add(element);
                            _listDetectResult.Add(UIUtils.GetChildObject<Label>(element, _strdetectresult));
                            _listJudmentValue.Add(UIUtils.GetChildObject<Label>(element, _strJudgmentValueTemp));
                            _listStandardName.Add(UIUtils.GetChildObject<Label>(element, _strStandardNameTemp));
        ...
        private PrintHelper.Report GenerateReport()
        {
            ...
            try
            {
                ...
                report.User = LoginWindow._curAccount.UserName;
        ...
        private void Upload()
        {
            ...
            try
            {
                if (Global.samplenameadapter != null && Global.samplenameadapter.Count > 0)
                {
                    ...
                    //将table的值转换成泛型赋给msg.selectedRecords 上传到系统
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        List<tlsTtResultSecond> dtList = Global.TableToEntity<tlsTtResultSecond>(dt);
                        msg.selectedRecords = dtList;
                    ...
                    if (_contrast)
                    {
                        ...
                        for (int i = 0; i < HoleCount; ++i)
                        {
                            if (_use[i] && i != 0)  //&& i != 0
                            {
                                ...
                                UnqualifiedValue = TestResultConserve.UnqualifiedOrQualified(str, _item.Hole[i].SampleName, _item.Name);//_item.Name
                                ...
                                _CheckValue[(num > 0 ? (i - num) : i), 7] = LoginWindow._curAccount.UserName;
                ...
                switch (msg.what)
                {
                    ...
                    case MsgCode.MSG_UPLOAD:
                        if (msg.result)
                        {
                            ...
                            wnd.IsUpLoad = true;
                            MessageBox.Show("共成功上传" + Global.UploadSCount + "条数据，失败" + Global.UploadFCount + "条数据", "系统提示", MessageBoxButton.OK, MessageBoxImage.Warning); 
                        }
                        else
                        {
                            ...
                            else
                            {
                                ...
                                MessageBox.Show( "数据上传失败！", "系统提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                            }
                        }
                        break;
