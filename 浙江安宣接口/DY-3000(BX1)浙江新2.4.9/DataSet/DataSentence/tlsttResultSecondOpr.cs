...
namespace DYSeriesDataSet
{
    public class tlsttResultSecondOpr
    {
        ...
        public int DelDownItem(out string errMsg)
        {
            ...
            {
                sql.Length = 0;
                sql.AppendFormat("Delete From ZJCheckItem");
                DataBase.ExecuteCommand(sql.ToString(), out errMsg);
                rtn = 1;
            }
            catch (Exception ex)
        ...
        public int InDownItem(int id, string name, string code, out string errMsg)
        {
            ...
            try
            {
                sql.Clear();
                sql.Length = 0;
                sql.Append("Insert Into ZJCheckItem (ID,itemName,itemCode) values(");
                sql.Append(id);
                sql.Append(",'");
                sql.Append(name);
                sql.Append("','");
        ...
        public DataTable GetDownItem(string where, string orderby, out string errMsg)
        {
            ...
            {
                sql.Length = 0;
                sql.Append("select ID,itemName,itemCode from ZJCheckItem");
                if (where != "")
                {
                    sql.Append(" where ");
                    sql.Append(where);
