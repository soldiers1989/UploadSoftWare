                    ...
                    try
                    {
                        if (Global.InterfaceType.Equals("DY") || Global.InterfaceType.Equals(""))
                        {
                            try
                            {
                                ...
                                {
                                    url = Global.geturl(msg.str1, 1);
                                }
                                else
                                {
                                    url = Global.geturl(msg.str1, 4);
                                }

                                string json = "username=" + msg.str2 + "&password=" + msg.str3;
                                ...
                                else
                                {
                                    if (resultCode == "10002")
                                    {
                                        msg.errMsg = "用户名和密码不匹配";
                                    }
                                    else if (resultCode == "10003")
                                    {
                                        msg.errMsg = "未配置市场,请先完善基础信息";
                                    }
                ...
                case MsgCode.MSG_UPLOAD:
                    try
                    {
                        ...
                        if (Global.InterfaceType.Equals("DY") || Global.InterfaceType.Equals("GS") || Global.InterfaceType.Length == 0)
                        {
                            ...
                            if (Global.ThirdParty == "0")
                            {
                                comurl = Global.geturl(msg.str1, 1);
                            }
                            else
                            {
                                comurl = Global.geturl(msg.str1, 4);
                            }
                ...
                case MsgCode.MSG_DownCheckItems:
                    try
                    {
                        ...
                        if (a.resultCode == "10100")//下载成功
                        {
                            ...
                            {
                                string err = "";
                                _resultTable.DelDownItem(out err);//删除原来的数据
                                foreach (Json.ItemDetail detail in a.datalist)
                                {
                                    s = s + 1;
                                    //保存数据到本地数据库
                                    _resultTable.InDownItem(s, detail.itemname, detail.itemcode, out err);
                                }
                            }
                            catch (Exception ex)
                            {
