    ...
    public partial class frmServer : Form
    {
        ...
        private void BtnCommunicate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else if (Global.Platform == "ZJ")//浙江
                {
                    //通信测试重新读取身份认证信息，以防漏读导致无法上传
                    string comurl = "";

                    //浙江数据上传
                    if (Global.ThirdParty == "0")
                    {
                        comurl = Global.geturl(Global.ServerAdd , 1);
                    }
                    else
                    {
                        comurl = Global.geturl(Global.ServerAdd, 4);
                    }
                    string comAyy = "username=" + Global.ServerName + "&password=" + Global.ServerPassword;
                    string rtnok=  Global.CommunicationTest(comurl, comAyy);
                    if (rtnok == "OK")
                    {
                        IsTest = true;
                    ...
                    {
                        MessageBox.Show("通信失败！\r\n失败原因："+rtnok , "系统提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
        ...
        private void btnDownItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsTest == false)
                {
                    MessageBox.Show("请先进行通信测试再下载检测项目！","系统提示",MessageBoxButtons.OK ,MessageBoxIcon.Exclamation);
                    return;
                }
                btnDownItem.Enabled = false;
                int s = 0;
                string errMsg = "";
                string _item = Global.DownChkItem("GET", Global.geturl(Txt_Url.Text.Trim(), 2));
                ZJResultDatas a = JsonConvert.DeserializeObject<ZJResultDatas>(_item);

                if (a.resultCode == "10100")//下载成功
                {
                        string err = "";
                        sqlSet.DelDownItem(out err);//删除原来的数据
                        foreach (ZJResultDatas.ItemDetail detail in a.datalist)
                        {
                            //保存数据到本地数据库
                            sqlSet.InDownItem(s, detail.itemname, detail.itemcode, out err);
                            s = s + 1;
                        }
                }
                else
                ...
                if(errMsg=="")
                {
                    MessageBox.Show("检测项目下载成功！\r\n共成功下载" + s + "条数据！", "系统提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else if(errMsg.Length >0 && s>0)
                {
                    MessageBox.Show("部分数据下载失败！其中成功下载" + s + "条数据！", "系统提示", MessageBoxButtons.OK, MessageBoxIcon.Warning );
                }
                else if (errMsg.Length > 0 && s == 0)
                {
                    MessageBox.Show("数据下载失败！\r\n失败原因：" + errMsg, "系统提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
