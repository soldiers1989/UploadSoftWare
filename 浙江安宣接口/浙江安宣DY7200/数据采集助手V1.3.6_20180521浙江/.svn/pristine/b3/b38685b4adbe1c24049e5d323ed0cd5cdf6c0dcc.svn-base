            ...
            try
            {
                ...
                if (cdt != null && cdt.Rows.Count > 0)
                {
                    for (int n = 0; n < cdt.Rows.Count; n++)
                    {
                        ...
                        {
                            curDY5000.unitInfo[0, 0] = cdt.Rows[n]["TestUnitName"].ToString(); //检测单位
                            curDY5000.unitInfo[0, 1] = cdt.Rows[n]["DetectUnitName"].ToString();//被检单位
                            curDY5000.unitInfo[0, 2] = cdt.Rows[n]["SampleAddress"].ToString();//采样地址
                            curDY5000.unitInfo[0, 3] = cdt.Rows[n]["Tester"].ToString();//检测人
                        }
        ...
        protected override void btnDatsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < CheckDatas.Rows.Count; i++)
                {
                    ...
                    {
                        resultdata.Save = "是";
                        resultdata.HoleNumber = CheckDatas.Rows[i].Cells["孔位"].Value.ToString().Trim();
                        ...
                        resultdata.Getplace = CheckDatas.Rows[i].Cells["采样地址"].Value.ToString().Trim();
