...
namespace WorkstationDAL.Model
{
    public class Global
    {
       ...
       public static bool linkNet()
       {
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetXMLstr(DataTable dt)
        {
            Global.UpY =int.Parse( dt.Rows[0]["ID"].ToString());
            //查询数据库获取项目编号
            string err = "";
            StringBuilder sb = new StringBuilder();

            if (dt.Rows[0]["CheckData"].ToString() == "参考国标")
            {
                return "参考国标";
            }
            if (dt.Rows[0]["CheckData"].ToString() == "检测无效")
            {
                return "无效";
            }

            string chkvalue = dt.Rows[0]["CheckData"].ToString();
            if (Regex.IsMatch(chkvalue, @"[\u4e00-\u9fbb]+$"))//有汉字填负一
            {
                chkvalue = "-1";
            }
                           ...
                           + "<detectionData version='1.0'>"
                            + "<detectionRecords>"
                             + "<detectionRecord>"
                              ...
                              + "<booth>" + "A1" + "</booth>"                                                           //<!--摊位号 -->
                              + "<sampletime>" + (dt.Rows[0]["SampleTime"].ToString().Replace('/', '-')) + "</sampletime>"                   //<!--（抽样时间） -->
                              + "<detectiontime>" + (dt.Rows[0]["CheckTime"].ToString().Replace('/', '-')) + "</detectiontime>"        //<!--（检测时间） -->
                              + "<deviceno>" + (dt.Rows[0]["MachineNum"].ToString()) + "</deviceno>"               //<!--（检测设备内部编号） -->
                              + "<channelno>" + (dt.Rows[0]["HoleNum"].ToString()) + "</channelno>"                         //<!--（检测通道编号） -->      
                              + "<prodcode>" + (dt.Rows[0]["SampleCode"].ToString()) + "</prodcode>"              //<!--（被检产品编号） -->
                              + "<prodamount>" +(dt.Rows[0]["SampleNum"].ToString())  + "</prodamount >"                                               //<!--（被检产品数量） -->
                              + "<useunit>"+(dt.Rows[0]["NumberUnit"].ToString()) +"</useunit>"                                                             //<!--（数量单位） --> 
                              + "<detectItem>"
                              + "<itemcode>" + (dt.Rows[0]["Itemcode"].ToString()) + "</itemcode>"                                          //<!--（检测指标编号） -->
                              + " <result>" + (dt.Rows[0]["Result"].ToString() == "合格" ? "0" : "1") + "</result>"         //<!--（检测结果0=阴性合格，1=阳性不合格） -->
                              + "<extradata>" + dt.Rows[0]["LimitData"].ToString() + "</extradata>"              //<!--标准值 -->
        ...
        public static string CommunicationTest(string url, string json)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        cookie = "未配置市场,请先完善基础信息";
                    }
                    else if (resultCode == "10099")
                    {
                        cookie = "其它登录错误";
                    }
