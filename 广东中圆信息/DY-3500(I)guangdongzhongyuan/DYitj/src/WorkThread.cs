...
namespace AIO
{
    public class WorkThread : ChildThread, IDisposable
    {
        ...
        protected override void HandleMessage(Message msg)
        {
            ...
            switch (msg.what)
            {
                    ...
                    try
                    {
                        ...
                        if (Global.InterfaceType.Equals("DY") || Global.InterfaceType.Equals(""))
                        {
                            try
                            {
                                ...
                                conn.ConnectionString = string.Format("server={0};database={1};uid={2};pwd={3}", msg.str1, msg.str4, msg.str2, msg.str3);
                                conn.Open();
                ...
                case MsgCode.MSG_UPLOAD:
                    ...
                    {
                        msg.outError = UploadZYResult((CheckPointInfo)msg.obj1,msg.selectedRecords);
        ...
        /// <returns></returns>
        public string UploadZYResult(CheckPointInfo server, List<tlsTtResultSecond> selectedRecords)
        {
            ...
            {
                SqlConnection conn = new SqlConnection();
                conn.ConnectionString = string.Format("server={0};database={1};uid={2};pwd={3}", server.ServerAddr, server.DataBaseName, server.RegisterID, server.RegisterPassword);
                conn.Open();

                if (conn.State != ConnectionState.Open)
                {
                    return "连接服务器失败，请检查设置！";
                }
                if (conn.State == ConnectionState.Open)
                {
                    for (int i = 0; i < selectedRecords.Count; i++)
                    {
                        if (selectedRecords[i].IsUpload == "已上传")
                        {
                            continue;
                        }
                        if(selectedRecords[i].Result =="检测无效")
                        {
                            continue;
                        }

                        StringBuilder sb = new StringBuilder();
                        sb.AppendFormat("INSERT INTO {0} (checkno,checkmachine,checktime,checkmeth,itemtype,itemname,samplecode,samplename,checkhole,checkvalue,standvalue,result,makefactory,checkuser)VALUES(", server.TableName );
                        sb.AppendFormat("'{0}',", selectedRecords[i].CheckNo);
                        sb.AppendFormat("'{0}',", selectedRecords[i].CheckMachine);
                        sb.AppendFormat("'{0}',", selectedRecords[i].CheckStartDate );
                        sb.AppendFormat("'{0}',", selectedRecords[i].CheckMethod );
                        sb.AppendFormat("'{0}',", selectedRecords[i].ResultType);
                        sb.AppendFormat("'{0}',", selectedRecords[i].CheckTotalItem );
                        sb.AppendFormat("'{0}',", selectedRecords[i].SampleCode );
                        sb.AppendFormat("'{0}',", selectedRecords[i].FoodName );
                        sb.AppendFormat("'{0}',", selectedRecords[i].Hole);
                        sb.AppendFormat("'{0}',", selectedRecords[i].CheckValueInfo );
                        sb.AppendFormat("'{0}',", selectedRecords[i].StandValue );
                        sb.AppendFormat("'{0}',", selectedRecords[i].Result );
                        sb.AppendFormat("'{0}',", selectedRecords[i].CheckedCompany);
                        sb.AppendFormat("'{0}')", selectedRecords[i].CheckUnitInfo );

                        //插入数据
                        SqlCommand cmd = new SqlCommand(sb.ToString(), conn);
                        int InBase = cmd.ExecuteNonQuery();
                        //上传成功，更新上传状态
                        if (InBase == 1)
                        {
                            _resultTable.UpdateUploads(string.Format("('{0}')", selectedRecords[i].SysCode), out outStr);
                            Global.UploadSCount++;
                        }
                        else
                        {
