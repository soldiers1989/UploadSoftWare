...
namespace com.lvrenyang
{
    public class FileUtils
    {
        ...
        public static void KLog(string content,string SendReceive, int type)
        {
            ...
            {
                DateTime date = DateTime.Now;
                string fileName = "串口发送" + date.ToString("yyyy-MM-dd") + ".log";
                AddToFile(SaveLogDirectory + "\\send", fileName, SendReceive + date.ToString("HH:mm:ss\r\n") + content + "\r\n\r\n");
            }
            else if (type == 15)
            {
                DateTime date = DateTime.Now;
                string fileName = "串口接收" + date.ToString("yyyy-MM-dd") + ".log";
                AddToFile(SaveLogDirectory + "\\Receive", fileName, SendReceive + date.ToString("HH:mm:ss\r\n") + content + "\r\n\r\n");
            }
            else if (type == 16)
            {
                DateTime date = DateTime.Now;
                string fileName = "串口实时" + date.ToString("yyyy-MM-dd") + ".log";
                AddToFile(SaveLogDirectory + "\\Receive", fileName, SendReceive + date.ToString("HH:mm:ss\r\n") + content + "\r\n\r\n");
            }
        }
        /// <summary>
