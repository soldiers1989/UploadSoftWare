            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        shandong.ShanDongUpData model = new shandong.ShanDongUpData();
                        //样品类别
                        ...
                        if (obj != null && obj != DBNull.Value) model.limitdata = obj.ToString();
                        //检测编号
                        ...
                        if (Global.setInterface == true)
                        {
                            string dd = shandong.ShanDongUpData.PostWebService(model, out err);
                            shandong.ResultData results = shandong.ShanDongUpData.JsonToEntity<shandong.ResultData>(dd);
                        ...
                        else if (Global.DoubleWeb == true)//上传至双平台
                        {
                            ...
                            if (shandong.ShanDongUpData.InvokeAndCallWebService(model))
                            {
                                if (Global.UploadCount > 0)
                                {
                                    ...
                                    result.SendedDate = DateTime.Now;
                                    result.Sender = CurrentUser.GetInstance().UserInfo.UserCode;
                                    new clsResultOpr().SetUploadFlag(result, out err);//TODO 这里只更新选择的记录
                                }
                                else
