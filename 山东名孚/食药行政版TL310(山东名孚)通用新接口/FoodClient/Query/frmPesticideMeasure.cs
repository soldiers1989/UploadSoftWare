...
namespace FoodClient
{
    ...
    public class frmPesticideMeasure : TitleBarBase
    {
        ...
        private void InitializeComponent()
        {
            ...
            // lblUnsend
            // 
            this.lblUnsend.Location = new System.Drawing.Point(590, 5);
            this.lblUnsend.Name = "lblUnsend";
            this.lblUnsend.Size = new System.Drawing.Size(135, 21);
            ...
            // lblSended
            // 
            this.lblSended.Location = new System.Drawing.Point(446, 5);
            this.lblSended.Name = "lblSended";
            this.lblSended.Size = new System.Drawing.Size(142, 21);
            ...
            // lblUnPass
            // 
            this.lblUnPass.Location = new System.Drawing.Point(305, 5);
            this.lblUnPass.Name = "lblUnPass";
            this.lblUnPass.Size = new System.Drawing.Size(140, 21);
            ...
            // lblRecordSum
            // 
            this.lblRecordSum.Location = new System.Drawing.Point(6, 5);
            this.lblRecordSum.Name = "lblRecordSum";
            this.lblRecordSum.Size = new System.Drawing.Size(167, 21);
            ...
            // lblIsAllreadySended
            // 
            this.lblIsAllreadySended.Location = new System.Drawing.Point(831, 5);
            ...
            this.pnlSended.Location = new System.Drawing.Point(815, 8);
            ...
            this.pnlNoEligible.Location = new System.Drawing.Point(731, 9);
            ...
            // lblPassed
            // 
            this.lblPassed.Location = new System.Drawing.Point(171, 5);
            this.lblPassed.Name = "lblPassed";
            this.lblPassed.Size = new System.Drawing.Size(137, 21);
        ...
        private void setGridStyle()
        {
            ...
            c1FlexGrid1.Cols["StdCode"].Visible = true;
            ...
            c1FlexGrid1.Cols["Unit"].Visible = false;
            c1FlexGrid1.Cols["SampleNum"].Caption = "抽检数量";//"数据单位";
            c1FlexGrid1.Cols["SampleNum"].Visible = true;
            ...
            c1FlexGrid1.Cols["SampleUnit"].Visible = true;
            ...
            c1FlexGrid1.Cols["HolesNum"].Visible = true;
            ...
            c1FlexGrid1.Cols["ProducePlace"].Visible = false;
            c1FlexGrid1.Cols["CheckTotalItem"].Visible = false;
            c1FlexGrid1.Cols["CheckUnitName"].Visible = true;
        ...
        private void ShanDongUpData()
        {
            ...
            Global.UploadCount = 0;
            if (Global.setInterface == true)
            {
                if (Global.WebAddr.Length == 0)
                {
                    MessageBox.Show(this, "请先到基础数据【新接口】菜单中设置上传地址！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                if (Global.UserName.Length == 0)
                {
                    MessageBox.Show(this, "请先到基础数据【新接口】菜单中设置检测单位ID！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                if (Global.Password.Length == 0)
                {
                    MessageBox.Show(this, "请先到基础数据【新接口】菜单中设置密码！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
            ...
            else
            {
                ...
                {
                    MessageBox.Show(this, "请先到【基础数据同步】菜单中设置用户名！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                if (Global.AnHuiInterface.passWord.Length == 0)
                {
                    MessageBox.Show(this, "请先到【基础数据同步】菜单中设置密码！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
            ...
            PercentProcess process = new PercentProcess();
            process.BackgroundWork = this.UploadShanDongProcess;
            process.MessageInfo = "正在上传";
            //process.BackgroundWorkerCompleted += new EventHandler<BackgroundWorkerEventArgs>(backgroundWorkerCompleted);
            process.Start();
        ...
        private void UploadShanDongProcess(Action<int> percent)
        {
            percent(0);
            string sysCode = string.Empty, err = string.Empty, msg = string.Empty, outErr = string.Empty;
            int count = 0, uploadCount = 0;
            try
            {
                ...
                for (int i = 1; i < c1FlexGrid1.Rows.Count; i++)
                {
                    sysCode = c1FlexGrid1[i, "SampleCode"].ToString().Trim();
                    if (c1FlexGrid1.Rows[i].Selected == true && !Convert.ToBoolean(c1FlexGrid1.Rows[i]["IsSended"]))
                    {
                        for (int j = 0; j < dt.Rows.Count; j++)
                        {
                            if (dt.Rows[j]["SampleCode"].ToString().Trim().Equals(sysCode))
                            {
                                ShanDongUpData model = new ShanDongUpData();
                                ...
                                //数值单位
                                obj = dt.Rows[j]["ResultInfo"];
                                if (obj != null && obj != DBNull.Value) model.unit = obj.ToString();
                                //限定值
                                obj = dt.Rows[j]["StandValue"];
                                if (obj != null && obj != DBNull.Value) model.limitdata = obj.ToString();// 100f;
                                ...
                                //检测编号
                                obj = dt.Rows[j]["CheckNo"];
                                if (obj != null && obj != DBNull.Value) model.CHknum = obj.ToString();
                                //检测仪器
                                obj = dt.Rows[j]["CheckMachine"];
                                if (obj != null && obj != DBNull.Value) model.ChkMachine = obj.ToString();
                                //检测依据
                                obj = dt.Rows[j]["Standard"];
                                if (obj != null && obj != DBNull.Value) model.testbase = obj.ToString();
                                //shandong.ShanDongUpData.InvokeAndCallWebService("","");
                                //上传
                                if (Global.setInterface == true)//使用新接口上传数据
                                {

                                    string rtnn=  shandong.ShanDongUpData.PostWebService(model,out err);
                                    shandong.ResultData results = shandong.ShanDongUpData.JsonToEntity<shandong.ResultData>(rtnn);
                                    if (results.message == "成功" && results.status == "0")
                                    {
                                    ...
                                    {
                                        outErr += "样品名称：[" + model.SampleName + "] 上传失败！失败原因：" + results.status + results.message;
                                    }
                                }
                                else
                                {
                                    if (shandong.ShanDongUpData.InvokeAndCallWebService(model))
                                    {
                                        if (Global.UploadCount > 0)
                                        {
                                            err = string.Empty;
        ...
        {        
            ...
            //    return;
            //}
            if (Global.setInterface == true)//上传数据到新平台
            {
                if (Global.WebAddr.Length == 0)
                {
                    MessageBox.Show(this, "请先到基础数据【新接口】菜单中设置上传地址！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            else if(Global.DoubleWeb ==true)//上传数据到双平台
            {
                if (Global.WebAddr.Length == 0)
                {
                    MessageBox.Show(this, "请先到基础数据【新接口】菜单中设置上传地址！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void UploadHeFeiProcess(Action<int> percent)
        {
            ...
            try
            {
                //从数据库中拿到全部数据，然后和选择的记录对比。
                //DataTable dt = _curObjectOpr.GetUploadDataTable(" IsSended=false ", "A.SysCode", ShareOption.ApplicationTag);
                DataTable dt = _curObjectOpr.GetUploadDataTable("", "A.SysCode", ShareOption.ApplicationTag);//开放重传
                percent(3);
                uploadCount = c1FlexGrid1.Rows.Selected.Count;
                    ...
                    if (c1FlexGrid1.Rows[i].Selected == true)// && !Convert.ToBoolean(c1FlexGrid1.Rows[i]["IsSended"])
                    {
                        ...
                        {  
                            if (dt.Rows[j]["SampleCode"].ToString().Trim().Equals(sysCode))
                            {
                                ...
                                if (obj != null && obj != DBNull.Value) model.testbase = obj.ToString();
                                //条形码
                                obj = dt.Rows[j]["StdCode"];
                                if (obj != null && obj != DBNull.Value) model.barcode = obj.ToString();
                                //样品数量
                                obj = dt.Rows[j]["SampleNum"];
                                if (obj != null && obj != DBNull.Value) model.samplenum = obj.ToString();
                                //数量单位
                                obj = dt.Rows[j]["SampleUnit"];
                                if (obj != null && obj != DBNull.Value) model.numunit = obj.ToString();
                                ...
                                else if (Global.DoubleWeb == true)//上传至双平台
                                {
                                    string rtnn = shandong.ShanDongUpData.PostWebService(model, out err);
                                    shandong.ResultData results = shandong.ShanDongUpData.JsonToEntity<shandong.ResultData>(rtnn);
                                    if (results.message == "成功" && results.status == "0")
                                    ...
                                    {
                                        outErr += "新平台 样品名称：[" + model.SampleName + "] 上传失败！失败原因：" + results.status + results.message;
                                        if (count < 100)
                                        {
                                            percent(count);
                                            percentage2 += percentage1;
                                    ...
                                    if (shandong.ShanDongUpData.InvokeAndCallWebService(model))
                                    {
                                        if (Global.UploadCount > 0)
                                        {
                                            ...
                                            outErr += "样品名称：[" + model.SampleName + "] 上传失败！失败代码：" + Global.ErrData;
        ...
        /// 大连报表
        /// </summary>
        private void MultipleReport_DLBB()
        {
            try
            {
                if (c1FlexGrid1.RowSel < 1)
                {
                    MessageBox.Show("请选择需要打印的检测记录", "操作提示");
                    return ;
                }
                //保存数据
                SaveFile(GetHtmlDoc());
        ...
        private string GetHtmlDoc()
        {
            ...
            try
            {
                ...
                Dictionary<string, clsResultDL> dicReprot = new Dictionary<string, clsResultDL>();
                ...
                for (int i = 1; i < c1FlexGrid1.Rows.Count; i++)
                {
                    if (c1FlexGrid1.Rows[i].Selected == true)
                    {
                        ...
                        string strWhere = "A.SysCode ='" + sysCode+"'";
                        dt = _clsResultOpr.GetAsDataTable(strWhere, "A.SysCode");
                        break;
                    }
                }
                ...
                {
                    List<clsResultDL> report = (List<clsResultDL>)StringUtil.DataTableToIList<clsResultDL>(dt, 1);
                    for(int i=0;i<report.Count;i++)
                    {
                        string Result = string.Empty;
                        string CheckPlanCode = report[i].CheckPlanCode;
                        ...
                        Result = report[i].Result;
                        htmlDoc = System.IO.File.ReadAllText(path + "\\Others\\CheckedReportTitle.txt", System.Text.Encoding.Default);
                        //htmlDoc += string.Format("<div class=\"number\">编号：{0}</div>", report[i].CheckNo.Length > 0 ? report[i].CheckNo : "/");
                        ...
                        htmlDoc += string.Format("<td width=\"190\" height=\"76\" align=\"center\" class=\"as\">{0}</td>", report[i].FoodName.Length > 0 ? report[i].FoodName : "/");
                        htmlDoc += "<th width=\"190\" height=\"76\" class=\"as\">样品编号</th>";
                        htmlDoc += string.Format("<td width=\"190\" height=\"76\" align=\"center\" class=\"as box\">{0}</td></tr>", report[i].SampleCode.Length > 0 ? report[i].SampleCode : "/");

                        htmlDoc += "<tr><th width=\"190\" height=\"76\" class=\"bs\">采样（抽样）时间</th>";
                        htmlDoc += string.Format("<td width=\"190\" height=\"76\" align=\"center\" class=\"bs\">{0}</td>", report[i].CheckStartDate != null ? report[i].CheckStartDate.ToString("yyyy-MM-dd HH:mm:ss") : "/");
                        htmlDoc += "<th width=\"190\" height=\"76\" class=\"bs\">采样（抽样）地点</th>";
                        htmlDoc += string.Format("<td width=\"190\" height=\"76\" align=\"center\" class=\"bs box\">{0}</td></tr>", report[i].UpperCompanyName.Length > 0 ? report[i].UpperCompanyName.ToString() : "/");

                        htmlDoc += "<tr><th width=\"190\" height=\"76\" class=\"bs\">被检单位</th>";
                        htmlDoc += string.Format("<td width=\"190\" height=\"76\" align=\"center\" class=\"bs\">{0}</td>", report[i].CheckedCompanyName.Length > 0 ? report[i].CheckedCompanyName  : "/");
                        htmlDoc += "<th width=\"190\" height=\"76\" class=\"bs\">条形码</th>";
                        htmlDoc += string.Format("<td width=\"190\" height=\"76\" align=\"center\" class=\"bs box\">{0}</td></tr>", report[i].StdCode.Length >0?report[i].StdCode:"/");

                        htmlDoc += "<tr><th width=\"190\" height=\"76\" class=\"bs\">检测时间</th>";
                        htmlDoc += string.Format("<td width=\"190\" height=\"76\" align=\"center\" class=\"bs\">{0}</td>", report[i].CheckStartDate != null ? report[i].CheckStartDate.ToString("yyyy-MM-dd HH:mm:ss") : "/");
                        htmlDoc += "<th width=\"190\" height=\"76\" class=\"bs\">检测依据</th>";
                        htmlDoc += string.Format("<td width=\"190\" height=\"76\" align=\"center\" class=\"bs box\">{0}</td></tr>", report[i].StandardName.Length > 0 ? report[i].StandardName : "/");
                        htmlDoc += "</tbody></table>";

                        htmlDoc += "<table width=\"760\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\"><tbody><tr>";
                        htmlDoc += "<th height=\"54\" width=\"165\" class=\"ds\">检测项目</th>";
                        htmlDoc += "<th height=\"54\" width=\"167\" class=\"ds\">检测仪器</th>";
                        htmlDoc += "<th height=\"54\" width=\"165\" class=\"ds\">检测结果</th>";
                        htmlDoc += "<th height=\"54\" width=\"128\" class=\"ds\">标准值</th>";
                        htmlDoc += "<th height=\"54\" width=\"128\" class=\"ds\" id=\"unique\">检测结论</th></tr>";

                        htmlDoc += string.Format("<tr><td height=\"54\" align=\"center\" class=\"ds\">{0}</td>", report[i].CheckTotalItemName.Length > 0 ? report[i].CheckTotalItemName : "/");
                        htmlDoc += string.Format("<td height=\"54\" align=\"center\" class=\"ds\">{0}</td>", report[i].MachineName.Length > 0 ? report[i].MachineName : "/");
                        htmlDoc += string.Format("<td height=\"54\" align=\"center\" class=\"ds\">{0}</td>", report[i].CheckValueInfo.Length > 0 ? report[i].CheckValueInfo + report[i].ResultInfo : "/");
                        htmlDoc += string.Format("<td height=\"54\" align=\"center\" class=\"ds\">{0}</td>", report[i].StandValue.Length > 0 ? report[i].StandValue + report[i].ResultInfo : "/");  
                        ...
                        htmlDoc += string.Format("<td height=\"54\" width=\"380\" style=\"padding-left:40px;\">检测单位：{0}</td>", report[i].CheckUnitName.Length > 0 ? report[i].CheckUnitName : "/");
                        ...
                        htmlDoc += string.Format("<div class=\"personnel\">检测人员：{0}</div>", report[i].CheckerName.Length > 0 ? report[i].CheckerName : "/");
