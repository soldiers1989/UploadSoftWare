...
namespace WorkstationModel.Instrument
{
    public class clsDY6200 : CommBase
    {
        ...
        private void doDataResult(string input, string item)
        {
            ...
            for (int j = 0; j < len; j++)
            {
                id = clsStringUtil.ConvertString(input.Substring(2, 2) + input.Substring(0, 2), 16, 10);
        ...
        protected override void OnRxChar(byte rec)
        {
            ...
            if (ind >= 0)//表示数据采集完成
            {
                ...
                if (index != 0)
                {
                    ...
                    {
                        clsMessageNotify.Instance().SendMessage(clsMessageNotify.NotifyInfo.ReadDY6200Data, "不是有效数据");
                        return;
                    }
                }
                ...
                if (len < 1)
                {
                    ...
                    if (clsMessageNotify.Instance() != null)
                    {
                        clsMessageNotify.Instance().SendMessage(clsMessageNotify.NotifyInfo.ReadDY6200Data, "数据已清空或暂无数据");
                        return;
                    }
                }
                string id = string.Empty;//编号
                ...
                for (int i = 0; i < len; i++)
                {
                    ...
                    id = clsStringUtil.ConvertString(tt.Substring(2, 2) + tt.Substring(0, 2), 16, 10);
                    ...
                    strWhere.AppendFormat(" AND CheckTime=#{0}#", DateTime.Parse(sdate.Replace("-", "/")));
                ...
                if (checkDtbl.Rows.Count > 0)
                {
                    if (clsMessageNotify.Instance() != null)
                    {
                        clsMessageNotify.Instance().SendMessage(clsMessageNotify.NotifyInfo.ReadDY6200Data, "OK");
                        return;
                    }
                }
                else
                {
                    if (clsMessageNotify.Instance() != null)
                    {
                        clsMessageNotify.Instance().SendMessage(clsMessageNotify.NotifyInfo.ReadDY6200Data, "暂无有效数据");
                        return;
                    }
                    //throw new FormatException("暂无数据");
                }
