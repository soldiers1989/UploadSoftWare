    ...
    {   
        ...
        public int ResuInsert(clsSaveResult result, out string errMsg)
        {
            ...
            try
            {
                ...
                _strBd.Append("(ChkNum,Checkitem,SampleName,CheckData,Unit,CheckTime,CheckUnit,Result,Save,Machine,SampleTime,SampleAddress,DetectUnit,TestBase,LimitData,Tester,");
                _strBd.Append("StockIn,SampleNum,IsUpload,SampleCode,SampleCategory,MachineNum,ProductPlace,ProductDatetime,Barcode,ProcodeCompany,ProduceAddr,ProduceUnit,SendTestDate,NumberUnit)");
                ...
                _strBd.AppendFormat("'{0}',", result.CheckUnit == "" ? Global.CheckedUnit : result.CheckUnit);
                _strBd.AppendFormat("'{0}',", result.Result);
                _strBd.AppendFormat("'{0}',", "是");
                _strBd.AppendFormat("'{0}',", Global.ChkManchine);
                _strBd.AppendFormat("'{0}',", result.Gettime == "" ? "" : result.Gettime);
                ...
                _strBd.AppendFormat("'{0}',", "LZ4000");
                _strBd.AppendFormat("'{0}',", result.ProductPlace);
                _strBd.AppendFormat("'{0}',", result.ProductDatetime);
                _strBd.AppendFormat("'{0}',", result.Barcode);
                _strBd.AppendFormat("'{0}',", result.ProcodeCompany);
                _strBd.AppendFormat("'{0}',", result.ProduceAddr);
                _strBd.AppendFormat("'{0}',", result.ProduceUnit);
                _strBd.AppendFormat("'{0}',", result.SendTestDate);
        ...
        public DataTable GetResultTable(string whereSQL, string oderby, int type,int count, out string errMsg)
        {
            ...
            try
            {
                if (type == 1)
                {
                    _strBd.AppendFormat(" Select Top {0} ", count);
                    _strBd.Append(" * From CheckResult t order by ID desc ");
                }
                else if(type==2)
                {
        ...
        public int LoadInData(clsSaveResult result, out string errMsg)
        {
            ...
            {
                _strBd.Append("INSERT INTO CheckResult ");
                _strBd.Append("(Checkitem,SampleName,CheckData,Unit,CheckTime,CheckUnit,Result,Save,Machine,SampleTime,SampleAddress,TestBase,LimitData,Tester,");
                _strBd.Append("DetectUnit,SampleNum,SampleCategory,NumberUnit,Barcode,ProduceUnit,ProduceAddr,ProcodeCompany,ProductPlace,ProductDatetime,SendTestDate)");
                _strBd.Append("VALUES('");
        ...
        public DataTable GetIntrument(string sql, out string errMsg)
        {
            ...
            try
            {
                _strBd.Append("SELECT Name,Manufacturer,communication,Isselect,ChkStdCode,Numbering,Protocol,ID");
                _strBd.Append(" FROM ");
                _strBd.Append("Instrument ");
                _strBd.Append(sql);
                string[] cmd = new string[1] { _strBd.ToString() };
        ...
        public void RepairResult(clsUpdateData SR,int wID, out string errMsg)
        {
            ...
            try
            {
                ...
                _strBd.Append(",SampleNum='");
                _strBd.Append(SR.sampleNum);
                _strBd.Append("',SampleCategory='");
                _strBd.Append(SR.sampletype);
                _strBd.Append("'");
                _strBd.Append(" where ID=");
                _strBd.Append(wID);
