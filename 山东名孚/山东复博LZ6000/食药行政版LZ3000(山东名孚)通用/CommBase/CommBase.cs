...
namespace JH.CommBase
{
    ...
    public abstract class CommBase : IDisposable
    {
        ...
        public class CommBaseSettings
        {
            ...
            public void SetStandard(string Port, int Baud, Handshake Hs, Parity Ps)
            {
                dataBits = 8;
        ...
        public PortStatus IsPortAvailable(string s)
        {
            ...
            hand = Win32Com.CreateFile(s, Win32Com.GENERIC_READ | Win32Com.GENERIC_WRITE, 0, IntPtr.Zero,
                Win32Com.OPEN_EXISTING, Win32Com.FILE_FLAG_OVERLAPPED, IntPtr.Zero);
            if (hand == (IntPtr)Win32Com.INVALID_HANDLE_VALUE)
            {
                ...
                else
                {
                    ...
                    hand = Win32Com.CreateFile(AltName(s), Win32Com.GENERIC_READ | Win32Com.GENERIC_WRITE, 0, IntPtr.Zero,
                        Win32Com.OPEN_EXISTING, Win32Com.FILE_FLAG_OVERLAPPED, IntPtr.Zero);
                    if (hand == (IntPtr)Win32Com.INVALID_HANDLE_VALUE)
                    {
                        if (Marshal.GetLastWin32Error() == Win32Com.ERROR_ACCESS_DENIED)
        ...
        public bool Open()
        {
            ...
            //cp.dwCurrentRxQueue = 1024;
            //cp.dwCurrentTxQueue = 1024;
            //PortDCB.EofChar = 0x68;
            ...
            if (hPort == (IntPtr)Win32Com.INVALID_HANDLE_VALUE)
            {
                ...
                else
                {
                    ...
                    if (hPort == (IntPtr)Win32Com.INVALID_HANDLE_VALUE)
                    {
                        ...
                        {
                            throw new CommPortException("¶Ë¿Ú´ò¿ªÊ§°Ü");
                        }
                    }
                }
            }
            ...
            PortDCB.init(((cs.parity == Parity.odd) || (cs.parity == Parity.even)), cs.txFlowCTS, cs.txFlowDSR, (int)cs.useDTR, cs.rxGateDSR, !cs.txWhenRxXoff, cs.txFlowX, cs.rxFlowX, (int)cs.useRTS);
            ...
            {
                ThrowException("´íÎó¶Ë¿ÚÉèÖÃ");
                return false;
            }
            if (!Win32Com.SetCommTimeouts(hPort, ref commTimeouts))
            {
                ThrowException("´íÎó³¬Ê±ÉèÖÃ");
                return false;
            }
            ...
            if (checkSends)
            {
                overlapped.hEvent = writeEvent.SafeWaitHandle.DangerousGetHandle();//writeEvent.Handle;
            }
            else
            {
            ...
            ptrUWO = Marshal.AllocHGlobal(Marshal.SizeOf(overlapped));

            Marshal.StructureToPtr(overlapped, ptrUWO, true);
            writeCount = 0;
            empty[0] = true;
            dataQueued = false;
            rxException = null;
        ...
        protected virtual CommBaseSettings NewCommSettings() { return new CommBaseSettings(); }
        ...
        private void ReceiveThread()
        {
            ...
            //°Ñsg.HandleÐÞ¸ÄÎªsg.SafeWaitHandle.DangerousGetHandle();2011-08-26
            ...
            try
            {
                while (true)
                {
                    ...
                    if (!Win32Com.SetCommMask(hPort, Win32Com.EV_RXCHAR | Win32Com.EV_RXFLAG | Win32Com.EV_TXEMPTY | Win32Com.EV_CTS | Win32Com.EV_DSR
                        | Win32Com.EV_BREAK | Win32Com.EV_RLSD | Win32Com.EV_RING | Win32Com.EV_ERR | Win32Com.EV_RX80FULL))
                    {
                        throw new CommPortException("ÊäÈëÊä³ö´íÎó [001]");
                    }
                    ...
                    if (!Win32Com.WaitCommEvent(hPort, uMask, unmanagedOv))
                    {
                        ...
                        {
                            throw new CommPortException("ÊäÈëÊä³ö´íÎó [002]");
                        }
                    }
                    ...
                    if ((eventMask & Win32Com.EV_ERR) != 0)
                    {
                        ...
                        if (Win32Com.ClearCommError(hPort, out errs, IntPtr.Zero))
                        {
                            ...
                            int ec = 0;
                            StringBuilder sb = new StringBuilder("UART Error: ", 40);
                            if ((errs & Win32Com.CE_FRAME) != 0)
                            {
                                sb = sb.Append("Framing,");
                                ec++;
                            }
                            ...
                            {
                                sb = sb.Append("Overrun,");
                                ec++;
                            }
                            if ((errs & Win32Com.CE_RXOVER) != 0)
                            {
                                sb = sb.Append("Receive Cverflow,");
                                ec++;
                            }
                            ...
                            {
                                sb = sb.Append("Transmit Overflow,");
                                ec++;
                            }
                            if (ec > 0)
                            {
                                sb.Length = sb.Length - 1;
                                throw new CommPortException(sb.ToString());
                            }
                            else
                            {
                                ...
                                {
                                    throw new CommPortException("ÊäÈëÊä³ö´íÎó [003]");
                                }
                            }
                        }
                        else
                    ...
                    if ((eventMask & Win32Com.EV_RXCHAR) != 0)
                    {
                        do
                        {
                            ...
                            if (!Win32Com.ReadFile(hPort, buf, 1, out gotbytes, unmanagedOv))
                            {
                                ...
                                throw new CommPortException("ÊäÈëÊä³ö´íÎó [004]");
                            }
                            if (gotbytes == 1)
                            {
                                OnRxChar(buf[0]);
                    ...
                    if (i != 0)
                    {
                        ...
                        if (!Win32Com.GetCommModemStatus(hPort, out f))
                        {
                            throw new CommPortException("ÊäÈëÊä³ö´íÎó [005]");
                        }
        ...
        private bool CheckOnline()
        {
            if ((rxException != null) && (!rxExceptionReported))
            {
                ...
                ThrowException("½ÓÊÕ´íÎó");
            }
            if (online)
            {
                //JH 1.1: Avoid use of GetHandleInformation for W98 compatability.
                if (hPort != (System.IntPtr)Win32Com.INVALID_HANDLE_VALUE)
                {
                    return true;
                }
                ThrowException("¶ÏÏß");
    ...
    public class CommPortException : ApplicationException
    {
        ...
        public CommPortException(Exception e) : base("½ÓÊÕÊý¾Ý³öÏÖÒì³£´íÎó", e) { }
    }
    ...
    internal class Win32Com
    {
        ...
        [DllImport("kernel32.dll")]
        internal static extern Boolean PurgeComm(IntPtr hFile, Int32 dwFlags);
