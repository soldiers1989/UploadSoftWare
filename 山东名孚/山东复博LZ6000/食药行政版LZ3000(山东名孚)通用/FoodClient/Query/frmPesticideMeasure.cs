    ...
    public class frmPesticideMeasure : TitleBarBase
    {
        ...
        private void UploadShanDongProcess(Action<int> percent)
        {
            ...
            finally
            {
                ...
                {
                    MessageBox.Show("数据上传失败！ \r\n\r\n失败原因： " + outErr+ Global.rtdata, "系统提示");
                }
                else if (outErr.Length > 0 && Global.UploadCount > 0)
                {
                    MessageBox.Show("部分数据上传失败！ \r\n\r\n失败原因： " + outErr + Global.rtdata, "系统提示");
                    ...
                    if (c1FlexGrid1.Rows[i].Selected == true && !Convert.ToBoolean(c1FlexGrid1.Rows[i]["IsSended"]))
                    {
                        ...
                        {  
                            if (dt.Rows[j]["SampleCode"].ToString().Trim().Equals(sysCode))
                            {
                                ...
                                //处理结果
                                obj = dt.Rows[j]["Remark"];
                                if (obj != null && obj != DBNull.Value) model.remarkd = obj.ToString();
                                //检测编号
                                obj = dt.Rows[j]["SysCode"];
                                if (obj != null && obj != DBNull.Value) sysCode = obj.ToString();
