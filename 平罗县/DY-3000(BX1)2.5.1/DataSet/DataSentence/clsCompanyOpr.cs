...
namespace DYSeriesDataSet
{
    ...
    public class clsCompanyOpr
    {
        ...
        public int InsertPLSample(PLData model, out string errMsg)
        {
            errMsg = string.Empty;
            int rtn = 0;
            try
            {
                sql.Length = 0;
                sql.Append("Insert Into PLSample (idPL,category_id,samplename) VALUES(");
                sql.AppendFormat("'{0}',",model.id );
                sql.AppendFormat("'{0}',", model.category_id );
        ...
        public int DeletePLSample(string where ,string order,out string errMsg)
        {
            errMsg = string.Empty;
            int rtn = 0;
            try
            {
                sql.Length = 0;
                sql.Append("delete from PLSample ");
                if(where !="")
                {
                ...
                if(order !="")
                {
                    sql.AppendFormat(" order by {0}", where);
                }
