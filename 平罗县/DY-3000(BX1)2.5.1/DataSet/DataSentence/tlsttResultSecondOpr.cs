...
namespace DYSeriesDataSet
{
    public class tlsttResultSecondOpr
    {
        ...
        public DataTable GetPLSample(string WhereSql,string orderSql,int type,out string errMsg)
        {
            errMsg = string.Empty;
            DataTable dt = null;
            sql.Length = 0;
            try
            {
                if (type == 1)
                {
                    sql.Append("select * from PLSample ");
                }
                ...
                string[] names = new string[1] { "PLSample" };
                dt = DataBase.GetDataSet(cmd, names, out errMsg).Tables["PLSample"];
                sql.Length = 0;
            }
            catch (Exception e)
            {
        ...
        public DataTable ExportData(string whereSql, string InterfaceType = "")
        {
            ...
            try
            {
                ...
                sql.Append(" DateManufacture AS 生产日期,CheckMethod AS 检测方法,APRACategory AS 单位类别,Hole AS 检测孔,SamplingPlace AS 抽样地点,CheckType AS 检测类型,ContrastValue AS 对照值,CKCKNAMEUSID,SysCode AS 系统编号,ProduceCompany as 生产单位 ");
                ...
                else if (InterfaceType.Equals("PL"))
                {
                    sql.Append(",SampleId AS 样品ID");
                }
                sql.Append(" FROM ttResultSecond ");
                if (!whereSql.Equals(""))
                    sql.AppendFormat(" WHERE {0}", whereSql);
        ...
        public int Insert(tlsTtResultSecond model, out string errMsg)
        {
            ...
            try
            {
                ...
                sql.Append(" CheckMachineModel,MachineCompany,CheckTotalItem,CheckValueInfo,StandValue,Result,ResultInfo,CheckUnitName,CheckUnitInfo, ");
                sql.Append(" Organizer,UpLoader,ReportDeliTime,IsReconsider,ReconsiderTime,ProceResults,CheckedCompanyCode,CheckedCompany,CheckedComDis, ");
                sql.Append(" CheckPlanCode,Hole,CheckMethod,APRACategory,CheckType,DateManufacture,ContrastValue,DeviceId,SampleId,ProduceCompany,fTpye,SysCode) ");
                sql.Append(" VALUES(");
                sql.AppendFormat("'{0}',", model.ResultType);
                sql.AppendFormat("'{0}',", model.CheckNo);
                sql.AppendFormat("'{0}',", model.SampleCode);
