...
namespace AIO.src
{
    ...
    public static class InterfaceHelper
    {
        ...
        public static string PLHttpPost(string url)
        {
            string result = string.Empty;
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "POST";   
        ...
        public static string GetPLUrl(string url, int type)
        {
            int index = url.LastIndexOf('/');
            if (type == 1)
            {
                url += (index == url.Length - 1) ? "Index/get_goods" : "/Index/get_goods";
            }
            else if (type == 2)
            {
                url += (index == url.Length - 1) ? "Index/save_goods" : "/Index/save_goods";
            }
