...
namespace AIO
{
    ...
    public class PrintHelper
    {
        ...
        public static bool PrintProductCompany = true;//生产单位
        ...
        public static bool Xprinter = false;//是否启用热干胶打印机  默认不启用
        public static string XprinterName = "";//打印机名称
        ...
        public class Report
        {
            ...
            public string ProductCompany = "";//生产单位
            ...
            public byte[] GeneratePrintBytes_3500I()
            {
            ...
            public byte[] GeneratePrintBytesEvery()
            {
                List<byte> data = new List<byte>();
                data.AddRange(CMD_RESET);
                data.AddRange(CMD_SETGBK);
                ...
                if (PrintQR) data.AddRange(CreateQRCode());
            ...
            private List<byte> CreateQRCode()
            {
                ...
                if (PrintInstrumentName) QRbyte.AddRange(GBKEncoding.GetBytes("\r\n检测设备:" + Global.InstrumentName));
                if (PrintItemCategory) QRbyte.AddRange(GBKEncoding.GetBytes("\r\n检测类别:" + ItemCategory));
                if (PrintDate) QRbyte.AddRange(GBKEncoding.GetBytes("\r\n检测日期:" + Date));
                if (PrintUnit) QRbyte.AddRange(GBKEncoding.GetBytes("\r\n单位:" + Unit));
                ...
                if (PrintUser) QRbyte.AddRange(GBKEncoding.GetBytes("\r\n检测人员:" + User));
            ...
            public byte[] GeneratePrintBytes()
            {
                ...
                if(PrintProductCompany  )
                {
                    vals = GetVals("生产单位:" + ProductCompany );
                    for (int j = vals.Count - 1; j >= 0; j--)
                    {
                        data.AddRange(GBKEncoding.GetBytes("\r\n" + vals[j]));
                    }
