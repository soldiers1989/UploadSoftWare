            ...
            try
            {
                ...
                for (int i = 0; i < Result.GetLength(0); i++)
                {
                    if (!Result[i, 4].ToString().Contains("Abs") && !Result[i, 4].ToString().Contains("NA") && !Result[i, 4].ToString().Contains("*") && !string.IsNullOrEmpty(Result[i, 4]))
                    {
                        ...
                        //根据样品名称查询样品ID ，种类ID
                        DataTable dtsam = _bll.GetPLSample("samplename='" + _model.FoodName + "'", "", 1, out ourEgg);
                        if (dtsam != null && dtsam.Rows.Count > 0)
                        {
                            _model.SampleId = dtsam.Rows[0]["idPL"].ToString();
                            if (dtsam.Rows[0]["category_id"].ToString() == "2")
                            {
                                _model.FoodType = "蔬菜";
                            }
                            else if (dtsam.Rows[0]["category_id"].ToString() == "3")
                            {
                                _model.FoodType = "水果";
                            }
                            else
                            {
                                _model.FoodType = "食用农产品";
                            }
                            _model.fTpye = dtsam.Rows[0]["category_id"].ToString();
                        }
