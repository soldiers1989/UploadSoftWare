        ...
        private void UpdateLable()
        {
            try
            {
                if (_item.Method == 0 && _item.ir.RefDeltaA != Double.MinValue)
                {
                    ...
                    {
                        if (_item.ir.SaveTime != null && _item.ir.SaveTime.Length > 0)
                        {
                            DateTime t1 = DateTime.Now;
                            DateTime t2 = DateTime.Parse(_item.ir.SaveTime);
                            ...
                            if (getHours > 24)
                            {
                                //MessageBox.Show("对照值失效！", "操作提示", MessageBoxButton.OK, MessageBoxImage.Information);
                                label1.Content = "对照值：" + String.Format("{0:F3}", _item.ir.RefDeltaA) + "(超过24小时)！";
                            }
                            else
                            {
                                label1.Content = "对照值：" + String.Format("{0:F3}", _item.ir.RefDeltaA);
                            }
                        }
                        else
                        {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (Global.EachDistrict.Equals("GS"))
                {
                    if (0 == _item.Method)
                    {
                        if (_item.ir.RefDeltaA < 0.15)
                        {
                            MessageBox.Show("对照值过低，检测无效！","提示",MessageBoxButton.OK ,MessageBoxImage.Warning );
                            UpdateItem();
                            UpdateLable();
                    ...
                    if (dtList.Count > 0)
                    {
                        ...
                        foreach (var item in dic)
                        {
                            ...
                            PrintHelper.Report model = new PrintHelper.Report
                            {
                                ...
                                ProductCompany = models[0].ProduceCompany,
                            };
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        if (dtList.Count > 0)
                        {
                           ...
                           for(int i=0;i<dtList.Count;i++)
                           {
                                ...
                                sb.AppendFormat("   {0} {1}  {2}  {3}\r\n", dtList[i].SampleCode, dtList[i].FoodName, dtList[i].CheckValueInfo, dtList[i].Result);
                        ...
                        for (int i = 0; i < Global.deviceHole.HoleCount; ++i)
                        {
                            if (_use[i])
                            {
                                ...
                                else
                                {
                                    ...
                                    //甘肃对照值大于0.15保存，小于0.15判为无效，同时保存对照值时间
                                    if (Global.EachDistrict.Equals("GS"))
                                    {
                                        //保存时间
                                        //Global.ContrastTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                                        //CFGUtils.SaveConfig("ContrastTime", Global.ContrastTime);
                                        _item.ir.SaveTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
