    ...
    public partial class FgdMeasureWindow : Window
    {
                    ...
                    if (Global.EachDistrict.Equals("GS"))
                    {
                        if (_item.ir.SaveTime != null && _item.ir.SaveTime .Length > 0)
                        {
                            ...
                            if (getHours > 24)
                            {
                                ...
                                LabelInfo.Content = "对照值：" + String.Format("{0:F3}", _item.ir.RefDeltaA)+ "(超过24小时)！" ;
                            }
                            else
                            {
                                LabelInfo.Content = "对照值：" + String.Format("{0:F3}", _item.ir.RefDeltaA);
                            }
                        }
                        else
                        {
        ...
        private void ButtonSampleTest_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (Global.EachDistrict.Equals("GS"))
                {
                    ...
                    {
                        MessageBox.Show("当前检测项目对照值过低，请重新做对照测试！", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                        return;
                    }
                }
        ...
        private void Update(byte[] data)
        {
            try
            {
                ...
                // 更新firstATs和lastATs
                //if ((_passms > _preHeatms) && (null == _firstATs))//动力学测试全为0
                //    _firstATs = _curATs;
        ...
        private void JumpToResult(int resultType)
        {
            try
            {
                ...
                //CFGUtils.SaveConfig("FgIsStart", "0");
                //Global.sendMessages("BatteryManage", "关闭分光度");//发windows消息给电池
                //System.Console.WriteLine(CFGUtils.GetConfig("FgIsStart", "0"));
