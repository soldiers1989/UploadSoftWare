    ...
    public partial class LoginWindow : Window
    {
        ...
        public LoginWindow()
        {
            ...
            try
            {
                ...
                if (!machine.Contains("DY-3500(I)食品综合分析仪") && Global.Is3500I ==false )
                {
            ...
            try
            {
                if (CheckedUser())
                {
                    foreach (UserAccount account in Global.userAccounts)
                    {
                        ...
                        {
                            if (account.UserPassword.Equals(TextBoxUserPassword.Password.Trim ()))
                            {
                                ...
                                {
                                    if (Global.InterfaceType.Equals("GS") ||Global.EachDistrict.Equals("GS"))//甘肃记住密码
                                    {
                                        if(checkBoxUser.IsChecked==true  )
                                        {
                                            for (int i = 0; i < Global.userAccounts.Count; i++)
                                            {
                                                if (TextBoxUserName.Text.Trim() == Global.userAccounts[i].UserName && TextBoxUserPassword.Password.Trim() == Global.userAccounts[i].UserPassword)
                                                {
                                                    Global.userAccounts[i].Remember = true;
                                                }
                                                else
                                                {
                                                    Global.userAccounts[i].Remember = false ;
                                                }
                                            }
                                        }
                                        else
                                        ...
                                        Global.SerializeToFile(Global.userAccounts, Global.userAccountsFile);//保存
                                    }
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (Global.InterfaceType.Equals("GS") || Global.EachDistrict.Equals("GS"))
                {
                    ...
                    for (int i = 0; i < Global.userAccounts.Count; i++)
                    {
                        if (Global.userAccounts[i].Remember)
                        {
                            TextBoxUserName.Text = Global.userAccounts[i].UserName;
                            TextBoxUserPassword.Password = Global.userAccounts[i].UserPassword;
                ...
                if (fgitems != null && fgitems.Count > 0)
                {
                    for (int i = 0; i < fgitems.Count; i++)
                    {
                        ...
                        {
                            if (fgitems[i].Method != 0)//甘肃的分光抑制率不用更新对照值
            ...
            {
                if (CFGUtils.GetConfig("AddColum", "0").Equals("1"))
                {
                    ReslfTest();
                }
                //是否需要更新数据库
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ReslfTest()
        {
            PercentProcess process = new PercentProcess()
            {
                BackgroundWork = AddRepairProjects,
                MessageInfo = "程序正在自检，请稍后"
            };
            process.Start();
        }
        private void AddRepairProjects(Action<int> percent)
        {
            percent(0);
            try
            {
        ...
        {
            if (!Global.InstrumentName.Contains("DY-3500(I)") && Global.Is3500I ==false )
            {
                //判定程序路径是否存在
