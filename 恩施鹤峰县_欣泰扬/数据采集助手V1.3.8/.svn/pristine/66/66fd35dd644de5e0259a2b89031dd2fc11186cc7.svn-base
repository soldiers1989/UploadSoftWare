    ...
    public partial class ucDY6200 : BasicContent
    {
        ...
        private void showOnControl(DataTable dtbl)
        {
            ...
            for (int i = 0; i < CheckDatas.Rows.Count; i++)
            {
                ...
                CheckDatas.Rows[i].Cells["被检单位"].Value = unitInfo[0, 1];//被检单位
                CheckDatas.Rows[i].Cells["采样地址"].Value = unitInfo[0, 2];//采样地址
                CheckDatas.Rows[i].Cells["检测人"].Value = unitInfo[0, 3];//检测人
        ...
        protected override void btnDatsave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < CheckDatas.Rows.Count; i++)
            {
                if (CheckDatas.Rows[i].Cells["已保存"].Value.ToString() == "False" || CheckDatas.Rows[i].Cells["已保存"].Value.ToString() == "否")
                {
                    ...
                    resultdata.CheckUnit = CheckDatas.Rows[i].Cells["检测单位"].Value.ToString().Trim();
