    ...
    public class tlsttResultSecondOpr
    {
        ...
        public DataTable ExportData(string whereSql, string InterfaceType = "")
        {
            ...
            try
            {
                ...
                sql.Append(" DateManufacture AS 生产日期,CheckMethod AS 检测方法,APRACategory AS 单位类别,Hole AS 检测孔,SamplingPlace AS 抽样地点,CheckType AS 检测类型,ContrastValue AS 对照值,CKCKNAMEUSID,SysCode AS 系统编号,ItemName as 匹配项目名称,ItemCode as 匹配项目编号,Opertorname as 经营户,operatorCode as 经营户编号,stallNumber as 摊位号,marketCode as 市场编号 ");
        ...
        public int Insert(tlsTtResultSecond model, out string errMsg)
        {
            ...
            try
            {
                ...
                sql.Append("Opertorname,ItemCode,stallNumber,operatorCode,ItemName,marketCode) VALUES(");
                sql.AppendFormat("'{0}',", model.ResultType);
                sql.AppendFormat("'{0}',", model.CheckNo);
                sql.AppendFormat("'{0}',", model.SampleCode);
                sql.AppendFormat("'{0}',", model.CheckPlaceCode);
                ...
                sql.AppendFormat("'{0}',", model.SysCode);
                sql.AppendFormat("'{0}',", model.Opertorname);
                sql.AppendFormat("'{0}',", model.ItemCode);
                sql.AppendFormat("'{0}',", model.stallNumber);
