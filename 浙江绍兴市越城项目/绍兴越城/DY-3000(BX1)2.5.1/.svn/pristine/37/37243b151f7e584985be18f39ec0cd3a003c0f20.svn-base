...
namespace AIO.src
{
	...
	/// * */
        ...
        public static int[][] getWaveInfo(double[] origin, double dValue, int point)
        {
            ...
            for (int i = 0; i < boguPoint.Count; i++)
            {
                ...
                s = boguPoint[i] - 7 < 0 ? 0 : boguPoint[i] - 7;
                ...
                if (Sindex[i][0] == 0)
                {
                    ...
                    Sindex[i][0] = boguPoint[i] - 7 < 0 ? 0 : boguPoint[i] - 7;
                }
            }
            //找出波谷结束点
            for (int i = 0; i < boguPoint.Count; i++)
            {
                ...
                s = boguPoint[i] + 7 > len ? len : boguPoint[i] + 7;
                ...
                if (Sindex[i][2] == 0)
                {
                    ...
                    Sindex[i][2] = boguPoint[i] +7 > len ? len - 1 : boguPoint[i] + 7;
                }
            }
            indexs = Sindex;
            return Sindex;
        ...
        private static int getArrSum(int[] origin, int[] compare)
        {
            ...
            for (int i = 0; i < origin.Length; i++)
            {
                ...
                else if (origin[i] == 0)
                {
                    ...
                    //sum=0.91+sum; //改
                    //ph0118小费修改
                    //sum += (double)(origin.length-2)/origin.length;
                }
                else
                {
                    sum = sum - 1;
            ...
            //return (int)Math.floor(sum);//向下取整
            return (int)sum;
        }
        ...
        /// <param name="point">点数</param>
        /// <returns>该段波的信息二维数组</returns>
        public static int[][] getWaveInfo_old(double[] origin, double dValue, int point)
        {
            //定义比较器, (i+1) -i>0 为正
            //起始点
            int[] startPoint = new int[point + 1];
        ...
        public static double[] getPointValue(double[] first, double[] duoxiangshi)
        {
            ...
            int[][] waveInfo =getWaveInfo_old(result, 0.001, 6);//0.0015
        ...
        public static double[] dyMath(double[] arr, bool isTest = false)
        {
            ...
            int[][] bogu =getWaveInfo_old(newArray, isTest ? 0.001 : 30, 6);
   ...
   *            移动平均点数
   */
        public static double[] getMovingAverage(double[] orign, int point)
        {

            double[] temp = new double[orign.Length];

            if (orign.Length > point)
            {
                //和值
                double sum = 0;
