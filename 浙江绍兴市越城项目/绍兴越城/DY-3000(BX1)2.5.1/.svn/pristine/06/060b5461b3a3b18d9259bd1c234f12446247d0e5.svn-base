...
namespace com.lvrenyang
{
    public class ComPort : IDisposable
    {
        ...
        {
            byte[] Rdata;
            Rdata = rxBuffer.ReadData(dlength);
            Console.WriteLine("缓存长度：" + dlength);
            return Rdata;
        }
                ...
                do
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < len; ++i)
                        {
                            ...
                            //darasr += darasr == "" ? buf[i].ToString("X2") : "," + buf[i].ToString("X2"); 
                        }
                    }
                }
                while (mSerial.BytesToRead > 0);
                //if (darasr != "" && darasr.Length > 4 && darasr.Substring(0, 5) == "7E,16")
                //{
                //    Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") +":"+darasr);
                //}
                //FileUtils.OprLog(2, "串口数据Hex", darasr);
            }
            catch (Exception ex)
            {
                FileUtils.Log(ex.ToString());
