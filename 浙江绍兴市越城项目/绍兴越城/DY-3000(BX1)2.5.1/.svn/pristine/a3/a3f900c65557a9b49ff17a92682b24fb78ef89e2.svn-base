...
namespace DYSeriesDataSet
{
    ...
    public class clsCompanyOpr
    {
        ...
        public DataTable GetAsDataTable(string whereSql, string orderBySql, int type)
        {
            ...
            try
            {
                ...
                else if (type == 2)
                {
                    ...
                    sql.Append("SELECT * FROM ycMarket ");
                else if (type == 14)
                    sql.Append("SELECT * FROM ycMarketDetail ");
        ...
        public int InsertMarket(marketInfo model, out string errMsg)
        {
            ...
            try
            {
                sql.Append("insert into ycMarket(marketName,marketCode) values(");
                sql.AppendFormat("'{0}',", model.marketName);
                sql.AppendFormat("'{0}')", model.marketCode);
        ...
        public int deletemarket(string where, string order, out string errMsg)
        {
            ...
            try
            {
                sql.Append("delete from ycMarket ");
        ...
        public int InsertMarketDetail(marketInfoDetail model, out string errMsg)
        {
            errMsg = "";
            int rtn = 0;
            sql.Length = 0;
            try
            {
                sql.Append("insert into ycMarketDetail(marketName,marketCode,contactNum,address,chargePerson,shopPeoperty,marketType) values(");
                sql.AppendFormat("'{0}',", model.name);
                sql.AppendFormat("'{0}',", model.marketCode );
                sql.AppendFormat("'{0}',", model.contactNum );
                sql.AppendFormat("'{0}',", model.address );
                sql.AppendFormat("'{0}',", model.chargePerson );
                sql.AppendFormat("'{0}',", model.shopPeoperty );
        ...
        public int deletemarketDetail(string where, string order, out string errMsg)
        {
            errMsg = "";
            int rtn = 0;
            sql.Length = 0;
            try
            {
                sql.Append("delete from ycMarketDetail ");
        ...
        public int deleteItem(string where, string order, out string errMsg)
        {
            ...
            try
            {
                sql.Append("delete from ycCheckItem ");
        ...
        public int InsertItem(Items model, out string errMsg)
        {
            ...
            try
            {
                sql.Append("insert into ycCheckItem(itemName,itemCode,itemType) values(");
                sql.AppendFormat("'{0}',", model.itemName);
                sql.AppendFormat("'{0}',", model.itemCode);
                sql.AppendFormat("'{0}')", model.itemType);
        ...
        public int deleteSample(string where, string order, out string errMsg)
        {
            ...
            try
            {
                sql.Append("delete from ycSample ");
        ...
        public int InsertSample(samples model, out string errMsg)
        {
            ...
            try
            {
                sql.Append("insert into ycSample(goodsCode,goodsName,goodsType) values(");
                sql.AppendFormat("'{0}',", model.goodsCode);
                sql.AppendFormat("'{0}',", model.goodsName);
                sql.AppendFormat("'{0}')", model.goodsType);
        ...
        public int InsertOpertor(Operates model,string marketname,string marketcode, out string errMsg)
        {
            errMsg = "";
            int rtn = 0;
            sql.Length = 0;
            try
            {
                sql.Append("insert into ycOperate(stallNo,operatorCode,operatorName,socialCreditCode,marketName,marketCode) values(");
                sql.AppendFormat("'{0}',", model.stallNo);
                sql.AppendFormat("'{0}',", model.operatorCode);
                sql.AppendFormat("'{0}',", model.operatorName);
                sql.AppendFormat("'{0}',", model.socialCreditCode);
                sql.AppendFormat("'{0}',", marketname);
        ...
        public int deleteOpertor(string where, string order, out string errMsg)
        {
            ...
            try
            {
                sql.Append("delete from ycOperate ");
        ...
        public int InsertOpertorDetail(OperatesDetail model, string marketname, string marketcode, out string errMsg)
        {
            errMsg = "";
            int rtn = 0;
            sql.Length = 0;
            try
            {
                sql.Append("insert into ycOperatorDetailed(legalerp,opscope,operatorName,contact,contactAddress,identiId,stallNo,socialCreditCode,marketName,marketCode) values(");
                sql.AppendFormat("'{0}',", model.legalerp);
                sql.AppendFormat("'{0}',", model.opscope);
                sql.AppendFormat("'{0}',", model.operatorName);
                sql.AppendFormat("'{0}',", model.contact);
                sql.AppendFormat("'{0}',", model.contactAddress );
                sql.AppendFormat("'{0}',", model.identiId );
                sql.AppendFormat("'{0}',", model.stallNo );
                sql.AppendFormat("'{0}',", model.socialCreditCode );
                sql.AppendFormat("'{0}',", marketname);
                sql.AppendFormat("'{0}')", marketcode);
        ...
        public int deleteOpertorDetail(string where, string order, out string errMsg)
        {
            errMsg = "";
            int rtn = 0;
            sql.Length = 0;
            try
            {
                sql.Append("delete from ycOperatorDetailed ");
        ...
        public DataTable GetYCItemTable(string name)
        {
            ...
            try
            {
                sql.Append("SELECT * FROM ycCheckItem");
        ...
        /// <param name="name"></param>
        /// <returns></returns>
        public DataTable GetYCOpertorTable(string name,int type)
        {
            string errMsg = string.Empty;
            DataTable dt = null;
            sql.Length = 0;
            try
            {
                if(type ==1)
                {
                    sql.Append("SELECT * FROM ycOperate");
                }
                else if (type == 2)
                {
                    sql.Append("SELECT * FROM ycOperatorDetailed");
                }
                ...
                string[] cmd = new string[1] { sql.ToString() };
                string[] names = new string[1] { type == 1 ? "ycOperate" : "ycOperatorDetailed" };
                dt = DataBase.GetDataSet(cmd, names, out errMsg).Tables[type == 1 ? "ycOperate" : "ycOperatorDetailed"];
                sql.Length = 0;
            }
            catch (Exception e)
            {
        ...
        public DataTable GetYCSampleTable(string name)
        {
            string errMsg = string.Empty;
            DataTable dt = null;
            sql.Length = 0;
            try
            {
                sql.Append("SELECT * FROM ycSample ");
                if (!name.Equals(string.Empty))
                {
                    sql.Append(" WHERE ");
                    sql.Append(name);
