    ...
    public partial class GszWindow : Window
    {
                ...
                for (int i = 0; i < borderList.Count; ++i)
                {
                    if (type.Equals("border"))
                    {
                        if (sender == borderList[i])
                        {
                            ...
                            //{
                            //    ItemList[i].Text = Global.gszItems[_SelIndex].Name;
        ...
        public UIElement GenerateItemBriefLayout(DYGSZItemPara item, string name, bool FalseOrTrue)
        {
            ...
            TextBox textBoxCompany = new TextBox()
            {
                ...
                Margin = new Thickness(0, 2, 0, 0),
                FontSize = 15,
                Text =Global.marketCheck ==true ?Global.marketName : string.Empty,
                VerticalContentAlignment = System.Windows.VerticalAlignment.Center,
                Name = "Company",
                ToolTip = "双击可查询所有被检单位",
        ...
        private void btnMatchItem_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                GhxMathItemWindow window = new GhxMathItemWindow();
                window.ShowDialog();
                ShowAllItems(Global.gszItems);
            }
            catch (Exception ex)
