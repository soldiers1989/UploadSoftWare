    ...
    public class WorkThread : ChildThread, IDisposable
    {
            ...
            switch (msg.what)
            {
                ...
                case MsgCode.MSG_CHECK_CONNECTION:
                    try
                    {
                        ...
                        {
                            msg.result = ZJYCUpdateDevice(msg.str1, msg.str2, msg.str3, msg.str4, msg.str5, msg.str6, out msg.errMsg);
                        }
                        else
                        {
                            msg.result = KjLogin(msg.str1, msg.str2, out msg.errMsg);
                ...
                case MsgCode.MSG_UPLOAD:
                    ...
                    {
                        msg.outError = uploadZJYC((CheckPointInfo)msg.obj1, msg.table);//浙江越城
                    }
                ...
                case MsgCode.MSG_DownCompany:
                    try
                    {
                        ...
                        //msg.DownLoadCompany = _DownCompany;
                        string urlm = "";
                        if(Global.marketCheck)//下载市场详细信息
                        {
                            urlm = getUrl.GetUrl(msg.str1, 7);
                        }
                        else
                        {
                            urlm = getUrl.GetUrl(msg.str1, 1);
                        }
        ...
        public string uploadZJYC(CheckPointInfo server, DataTable results)
        {
            ...
            try
            {
                ...
                {
                    url = getUrl.GetUrl(server.ServerAddr, 9);
                }
                else
                {
                    url = getUrl.GetUrl(server.ServerAddr, 6);
                }
                ...
                if (dtList != null && dtList.Count > 0)
                {
                    for (int i = 0; i < dtList.Count; i++)
                    {
                        ...
                        u.thirdCompanyName = server.RegisterID;
                        u.thirdCompanyCode = server.RegisterPassword;
                        ...
                        if(Global.marketCheck )
                        {
                            u.marketCode = dtList[i].marketCode;
                            //clsCompanyOpr _company = new clsCompanyOpr();
                            //DataTable dtc= _company.GetAsDataTable("marketName = '" + dtList[i].CheckedCompany + "'", string.Empty, 14);
                            //if(dtc!=null && dtc.Rows.Count>0)
                            //{
                            //    ck.marketCode = dtc.Rows[0]["marketCode"].ToString();
                            //}
                        }
        ...
        private bool ZJYCUpdateDevice(string url, string username, string password, string uniqecode, string model, string factory, out string errMsg)
        {
            bool isok = false;
            errMsg = string.Empty;
            try
            {
                string urll = getUrl.GetUrl(url, 5);
                updateDeviceInfo u = new updateDeviceInfo();
                u.Count = "1";//数量
                u.thirdCompanyName = username;
                u.thirdCompanyCode =password;
                List<devices> dd = new List<devices>();
                devices d = new devices();
                d.type = model ;//设备型号
                d.deviceId = uniqecode ;//设备唯一码
                d.factory = factory;
                dd.Add(d);
                u.device = dd;
                string data = JsonHelper.EntityToJson(u);

                string rtn = Global.ycHttpMath("POST", urll, data);
