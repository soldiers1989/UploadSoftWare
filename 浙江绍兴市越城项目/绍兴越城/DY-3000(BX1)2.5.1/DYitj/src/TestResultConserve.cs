            ...
            try
            {
                ...
                for (int i = 0; i < Result.GetLength(0); i++)
                {
                    if (!Result[i, 4].ToString().Contains("Abs") && !Result[i, 4].ToString().Contains("NA") && !Result[i, 4].ToString().Contains("*") && !string.IsNullOrEmpty(Result[i, 4]))
                    {
                        ...
                        _model.Opertorname = Result[i, 20];//经营户
                        if(Global.marketCheck )
                        {
                            //查询市场编号
                            clsCompanyOpr _company = new clsCompanyOpr();
                            DataTable dtc = _company.GetAsDataTable("marketName = '" + _model.CheckedCompany + "'", string.Empty, 14);
                            if (dtc != null && dtc.Rows.Count > 0)
                            {
                                _model.marketCode = dtc.Rows[0]["marketCode"].ToString();
                            }

                            DataTable dtop = _bll.GetYCOpertorTable("operatorName='" + _model.Opertorname + "'", 2);
                            if (dtop != null && dtop.Rows.Count > 0)
                            {
                                //_model.operatorCode = dtop.Rows[0]["operatorCode"].ToString();
                                _model.stallNumber = dtop.Rows[0]["stallNo"].ToString();
                            }
                        }
                        else
                        {
                            DataTable dtop = _bll.GetYCOpertorTable("operatorName='" + _model.Opertorname + "'",1);
                            if (dtop != null && dtop.Rows.Count > 0)
                            {
                                _model.operatorCode = dtop.Rows[0]["operatorCode"].ToString();
                                _model.stallNumber = dtop.Rows[0]["stallNo"].ToString();
                        ...
                        _model.ItemName = Result[i, 22];
                        _model.ItemCode = Result[i, 23];
        ...
        public static int ResultConserve(string[,] Result, bool contrast, string ContrastValue)
        {
            ...
            for (int i = 0; i < forLenght; i++)
            {
                if (!Result[i, 4].ToString().Contains("NA") && !Result[i, 4].ToString().Contains("*") && !string.IsNullOrEmpty(Result[i, 4]))
                {
                    ...
                    _model.Opertorname = Result[i, 19];//经营户
                    ...
                    if(Global.marketCheck )
                    {
                        ...
                        //查询经营户摊位号
                        DataTable dtop = _bll.GetYCOpertorTable("operatorName='" + _model.Opertorname + "'",2);//查询经营户信息
                        if (dtop != null && dtop.Rows.Count > 0)
                        {
                            //_model.operatorCode = dtop.Rows[0]["operatorCode"].ToString();
                            _model.stallNumber = dtop.Rows[0]["stallNo"].ToString();
                    ...
                    {
                        DataTable dtop = _bll.GetYCOpertorTable("operatorName='" + _model.Opertorname + "'",1);//查询经营户信息
                        if (dtop != null && dtop.Rows.Count > 0)
                        {
                            _model.operatorCode = dtop.Rows[0]["operatorCode"].ToString();
                            _model.stallNumber = dtop.Rows[0]["stallNo"].ToString();
                    ...
                    _model.ItemCode = Result[i, 22];
                    _model.ItemName = Result[i, 21];
