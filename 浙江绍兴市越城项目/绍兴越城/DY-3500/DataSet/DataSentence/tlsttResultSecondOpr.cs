    ...
    public class tlsttResultSecondOpr
    {
        ...
        public DataTable ExportData(string whereSql, string InterfaceType, string EACHDISTRICT)
        {
            ...
            try
            {
                ...
                sql.Append(" DateManufacture AS 生产日期,CheckMethod AS 检测方法,APRACategory AS 单位类别,Hole AS 检测孔,SamplingPlace AS 抽样地点,CheckType AS 检测类型,ContrastValue AS 对照值,ItemName as 匹配项目名称,ItemCode as 匹配项目编号,Opertorname as 经营户,operatorCode as 经营户编号,");
                sql.Append("stallNumber as 摊位号");
        ...
        public int Insert(tlsTtResultSecond model, out string errMsg)
        {
            ...
            try
            {
                ...
                sql.Append("Opertorname,ItemCode,stallNumber,operatorCode,ItemName) VALUES(");
