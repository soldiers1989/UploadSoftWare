    ...
    public class TestResultConserve : Window
    {
        ...
        public static int ResultConserve(string[,] result)
        {
            ...
            try
            {
                ...
                {
                    if (!result[i, 4].ToString().Contains("NA") && !result[i, 4].ToString().Contains("*") &&
                        !string.IsNullOrEmpty(result[i, 4]) && !result[i, 4].ToString().Contains("无效"))
                    {
                        ...
                        _model.ItemName = result[i, 13];

                        _model.Opertorname = result[i, 15];//经营户

                        DataTable dtop = _bll.GetYCOpertorTable("operatorName='" + _model.Opertorname + "'");//查询经营户信息
                        ...
                        _model.ItemName = result[i, 18];
                        _model.ItemCode = result[i, 19];
            ...
            try
            {
                ...
                for (int i = 0; i < forLenght; i++)
                {
                    if (!result[i, 4].ToString().Contains("NA") && !result[i, 4].ToString().Contains("*") && !string.IsNullOrEmpty(result[i, 4]))
                    {
                        ...
                        _model.ItemName = result[i, 17];
                        _model.ItemCode = result[i, 18];
                        _model.ItemType = result[i, 19];
