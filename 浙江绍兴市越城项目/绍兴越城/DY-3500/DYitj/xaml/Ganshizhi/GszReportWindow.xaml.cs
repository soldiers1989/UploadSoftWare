        ...
        private void ShowResult()
        {
            ...
            for (int i = 0; i < Global.deviceHole.SxtCount; ++i)
            {
                if (_item.Hole[i].Use)
                {
                    ...
                    if (_rgbValues[i] != null)
                    {
                        r = (byte)_rgbValues[i][0];
                        g = (byte)_rgbValues[i][1];
                        b = (byte)_rgbValues[i][2];

                        for (int j = 0; j < _rgbValues[i].Length; j++)
                        {
                            canvasVal += canvasVal.Length == 0 ? ((byte)_rgbValues[i][j]).ToString() : "|" + ((byte)_rgbValues[i][j]).ToString();
                        }
                    ...
                    if (!_listJudmentValue[i].Text.Trim().Equals("合格"))
                    {
                        ...
                        listJudgmentRes[i].Foreground = new SolidColorBrush(Colors.Red);
                    ...
                    //匹配项目
                    _CheckValue[(num > 0 ? (i - num) : i), 18] = _item.Mitem;
                    _CheckValue[(num > 0 ? (i - num) : i), 19] = _item.MCode;
                    _CheckValue[(num > 0 ? (i - num) : i), 20] = _item.MType;
                }
                else
                {
                    num += 1;
