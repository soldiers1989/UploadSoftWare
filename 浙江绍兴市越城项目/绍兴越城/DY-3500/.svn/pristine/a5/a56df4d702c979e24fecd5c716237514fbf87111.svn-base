...
namespace AIO
{
    ...
    public partial class ItemResultEdit : Window
    {
        ...
        {
            sp_gps.Visibility = Global.EnableGPS ? Visibility.Visible : Visibility.Collapsed;
        ...
        internal void GetValue(tlsTtResultSecond model)
        {
            ...
            _ID = model.ID;
            DateTime cddt = DateTime.Now;
            if (DateTime.TryParse(model.CheckStartDate, out cddt))
            {
                if (cddt < DateTime.Parse("2018-01-06"))
                {
                    tb_gpsInfo.Text = "(23.156730,113.502384)";
                    tb_gpsAddress.Text = "广东省广州市黄埔区开发区科技企业加速器 附近";
                }
            }
        }
