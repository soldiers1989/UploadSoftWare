...
namespace AIO
{
    ...
    public class FgdCaculate
    {
        ...
        public static DeltaA[] CaculateDeltaA(AT[] first, AT[] last, bool contrast)
        {
            ...
            {
                //数据无效 结束值 = 初始值 = 0 时 结果为NA
                if ((!IsValid(first[i].a) && !IsValid(last[i].a)) || (last[i].a == 0 && first[i].a == 0))
                {
                ...
                else
                {
                    ...
                    //{
                    //    // 结束值 > 初始值
                    //    // 结束值 > 初始值 有效，若相等(初始值和结束值都在很靠近0)的话则代表无任何变化，可判定为未放比色皿
                    //    result[i].deltaA = last[i].a > first[i].a ? last[i].a - first[i].a : _VALUE_INVALID;
                    //    contrast = false;
                    //}
                    ////样品
                    //else
