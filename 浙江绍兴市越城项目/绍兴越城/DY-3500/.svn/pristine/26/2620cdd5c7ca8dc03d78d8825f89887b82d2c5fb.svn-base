...
namespace AIO.xaml.Dialog
{
    ...
    public partial class ManagementCompany : Window
    {
        ...
        private void SearchKjcCompany()
        {
            ...
            {
                List<kjcCompany> ItemNames = (List<kjcCompany>)IListDataSet.DataTableToIList<kjcCompany>(dtbl, 1);
                DataGridRecord_kjc.DataContext = ItemNames;
            }
        }
        ...
        private void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            if (Global.InterfaceType.Equals("KJC"))
            {
                ...
                {
                    string where = string.Empty;
                    kjcCompany kjcmodel = null;
                    for (int i = 0; i < DataGridRecord_kjc.SelectedItems.Count; i++)
                    {
                        kjcmodel = (kjcCompany)DataGridRecord_kjc.SelectedItems[0];
                        where += where.Length == 0 ? string.Format("'{0}'", kjcmodel.regId) : string.Format(",'{0}'", kjcmodel.regId);
                    }
                    where = string.Format("regId In({0})", where);
                    kjcCompanyBLL.Deleted(where, out errMsg);
                    if (errMsg.Length == 0)
                    {
        ...
        private void btnUpdate_Click(object sender, RoutedEventArgs e)
        {
            if (Global.InterfaceType.Equals("KJC"))
            {
                AddOrUpdateKjcCompany window = new AddOrUpdateKjcCompany();
                try
                {
                    if (DataGridRecord_kjc.SelectedItems.Count > 0)
                    {
                        kjcCompany kjcmodel = (kjcCompany)DataGridRecord_kjc.SelectedItems[0];
                        window.model = kjcmodel;
                        window.ShowDialog();
                        SearchKjcCompany();
                    }
                    else
