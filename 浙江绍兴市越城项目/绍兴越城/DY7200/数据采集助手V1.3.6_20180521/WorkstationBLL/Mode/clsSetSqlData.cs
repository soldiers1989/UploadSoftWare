...
namespace WorkstationBLL.Mode
{
    ...
    {   
        ...
        public DataTable ExportExcel(string whereSql, string orderBySql, out string errMsg)
        {
            ...
            try
            {
                sb.Append("SELECT ChkNum as 检测编号,Checkitem as 检测项目,SampleName as 样品名称,SampleCode as 样品编号,CheckData as 检测结果,Unit as 单位,CheckTime as 检测时间,");
                sb.Append("Result as 结论,TestBase as 检测依据,LimitData as 标准值,Machine as 检测仪器,DetectUnit as 检测单位,Tester as 检测员,CheckUnit as 被检单位,");
                sb.Append("SampleNum as 检测数量,IsUpload as 已上传,SampleCategory as 样品种类,MachineNum as 仪器编号,");
                sb.Append("NumberUnit as 数量单位,ItemName as 匹配项目,ItemCode as 匹配项目编号,Opertorname as 经营户,operatorCode as 经营户编号,stallNumber as 摊位号");
