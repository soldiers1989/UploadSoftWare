    ...
    public class clsResultOpr
    {
       private   string errMsg = string.Empty;
        ...
        public bool IsExist(string strWhere)
        {
            ...
            {
                _strBd.AppendFormat(" WHERE {0}", strWhere);
            }
        ...
        public DataTable GetUploadDataTable(string strWhere, string orderBy, string appTag)
        {
            ...
            {
                _strBd.Append("SELECT ");//新两个字段
                _strBd.Append("A.SysCode,A.ResultType,A.CheckNo,A.SampleCode,A.StdCode,A.CKCKNAMEUSID,");
                //_strBd.Append("A.CheckedCompany AS CheckedCompanyCode,");//企业编号
                //2017年1月11日 被检单位编号上传ID不传UUID
                _strBd.Append("(SELECT L.CompanyID FROM tCompany AS L WHERE A.CheckedCompany=L.SysCode)AS CheckedCompanyCode,");
                ...
                _strBd.Append("(select z.Cdcode from tProprietors as z where z.Cdname = A.CheckedCompanyName and z.Ciname = A.UpperCompanyName and z.DisplayName = A.CheckedComDis)as CheckedDealerCode,");//被检经营户编号
                _strBd.Append("(SELECT B.Name FROM tFoodClass AS B WHERE A.FoodCode=B.SysCode)AS FoodName,");//查询食品名称 A.FoodCode,
                _strBd.Append("(SELECT FC.Name FROM tFoodClass AS FC WHERE FC.StdCode=IIf(Len(A.FoodCode)=5,A.FoodCode,(MID(A.FoodCode,1,(Len(A.FoodCode)-5))))) AS FoodType,");//食品种类
                ...
                _strBd.Append("'1' AS UpLoader,A.Notes,'' AS UpLoadDate");
        ...
        public int Insert(clsResult model, out string errMsg)
        {
            ...
            try
            {
                ...
                _strBd.Append("Standard,CheckMachine,CheckTotalItem,CheckValueInfo,StandValue,Result,ResultInfo,UpperCompany,UpperCompanyName,");
                _strBd.Append("OrCheckNo,CheckType,CheckUnitCode,Checker,Assessor,Organizer,Remark,CheckPlanCode,SaleNum,Price,CheckederVal,");
                _strBd.Append("IsSentCheck,CheckederRemark,Notes, HolesNum,MachineSampleNum,MachineItemName)");
                _strBd.Append("VALUES('");
                _strBd.Append(model.SysCode);
                _strBd.Append("','");
                _strBd.Append(model.ResultType);
        ...
        //    try
        //    {
        //        //旧版本是获取syscode，新版本的syscode存的是UUID，所以改为CheckNo
        ...
        //        string sql = string.Format("SELECT CheckNo FROM tResult WHERE CheckNo LIKE '{0}' ORDER BY CheckNo DESC", prevCode + StringUtil.RepeatChar('_', lev));
        ...
        public void UpdateResultByWhere(string where, out string errMsg)
        {
            errMsg = string.Empty;
            try
            {
                _strBd.Length = 0;
                _strBd.AppendFormat("UPDATE tResult SET {0}", where);
                DataBase.ExecuteCommand(_strBd.ToString(), out errMsg);
                _strBd.Length = 0;
            }
            catch (Exception e)
        ...
        /// 修改
        /// </summary>
        /// <param name="model">结果集</param>
        /// <param name="type">类型</param>
        /// <param name="errMsg"></param>
        /// <returns></returns>
        public int UpdateResult(clsResult model, int type, out string errMsg)
        {
            errMsg = string.Empty;
            int rtn = 0;
            try
            {
                ...
                {
                    _strBd.Append("UPDATE tResult SET ");
                    _strBd.AppendFormat("SysCode='{0}' ", model.SysCode);
                    _strBd.AppendFormat("WHERE CheckNo='{0}'", model.CheckNo);
                }
                DataBase.ExecuteCommand(_strBd.ToString(), out errMsg);
                _strBd.Length = 0;
                rtn = 1;
