         ...
         public DY7200()
        {
         ...
         private void AddTable(string samplename, string item, string checkValue, string unit, string conclusion, string checkunit, string sentunit, string standard, string time)
         {
             ...
             if (time != "")
             {
                 ...
                 strWhere.AppendFormat(" CheckData='{0}' AND CheckTime=#{1}#", checkValue.Replace('>', ' ').Replace('<', ' ').Trim(), DateTime.Parse(time));
             }

             dr["已保存"] = sqlSet.IsExist(strWhere.ToString()) == true ? "是" : "否";
             dr["样品名称"] = samplename;
             dr["检测项目"] = item;
             dr["检测结果"] = checkValue.Replace('>', ' ').Replace('<', ' ').Trim();
             dr["单位"] = unit;//检测值
             dr["检测依据"] = "";
             dr["标准值"] = standard;
             ...
             dr["经营户"] = unitInfo[0, 1];
             DataTable dtitem = _sql.GetYCItemTable("itemName='" + item+"'");
             if (dtitem != null && dtitem.Rows.Count > 0)
             {
                 dr["匹配项目"] = dtitem.Rows[0]["itemName"].ToString();
             }
             else
             {
                 dr["匹配项目"] = "";
