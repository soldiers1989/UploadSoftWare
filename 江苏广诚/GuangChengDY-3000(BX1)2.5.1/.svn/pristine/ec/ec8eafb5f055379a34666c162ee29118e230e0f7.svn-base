    ...
    public class tlsttResultSecondOpr
    {
        ...
        public DataTable ExportData(string whereSql, string InterfaceType = "")
        {
            ...
            try
            {
                ...
                sql.Append(" DateManufacture AS 生产日期,CheckMethod AS 检测方法,APRACategory AS 单位类别,Hole AS 检测孔,SamplingPlace AS 抽样地点,CheckType AS 检测类型,ContrastValue AS 对照值,CKCKNAMEUSID,SysCode AS 系统编号,projectID as 项目部ID,project as 项目部名称,stallID as 摊位号ID,stall as 摊位号 ");
        ...
        public int Insert(tlsTtResultSecond model, out string errMsg)
        {
            ...
            try
            {
                ...
                sql.Append(" CheckMachineModel,MachineCompany,CheckTotalItem,CheckValueInfo,StandValue,Result,ResultInfo,CheckUnitName,CheckUnitInfo, ");
                sql.Append(" Organizer,UpLoader,ReportDeliTime,IsReconsider,ReconsiderTime,ProceResults,CheckedCompanyCode,CheckedCompany,CheckedComDis, ");
                sql.Append(" CheckPlanCode,Hole,CheckMethod,APRACategory,CheckType,DateManufacture,ContrastValue,DeviceId,SampleId,ProduceCompany,SysCode,projectID,project,stallID,stall) ");
                sql.Append(" VALUES(");
                sql.AppendFormat("'{0}',", model.ResultType);
                sql.AppendFormat("'{0}',", model.CheckNo);
                sql.AppendFormat("'{0}',", model.SampleCode);
                ...
                sql.AppendFormat("'{0}',", model.SysCode);
                sql.AppendFormat("'{0}',", model.projectID );
                sql.AppendFormat("'{0}',", model.project );
                sql.AppendFormat("'{0}',", model.stallID );
                sql.AppendFormat("'{0}')", model.stall );
