...
namespace AIO
{
    public class WorkThread : ChildThread, IDisposable
    {
        ...
        protected override void HandleMessage(Message msg)
        {
            ...
            switch (msg.what)
            {
                ...
                case MsgCode.MSG_CHECK_CONNECTION:
                    try
                    {
                        if (Global.InterfaceType.Equals("GC"))//广诚通信测试
                        {
                            msg.result = false;
                            AIO.WebReference.DatRockService webDRS = new WebReference.DatRockService();
                            string _users= webDRS.getUser(msg.str2, msg.str3);
                            if (_users.Contains("code"))
                            {
                                GCResult _ruser = Json.JsonToEntity<GCResult>(_users);
                                if (_ruser != null && _ruser.code == "1000")
                                {
                                    msg.result = true;
                                    Global.Token = _ruser.token;
                                    Global.userId = _ruser.id;
                                }
                                else
                                {
                                    msg.errMsg = _ruser.code + errDetail(_ruser.code);
                                }
                            }
                            else
                            {
                                msg.errMsg = _users +":"+ errDetail(_users);
                            }

                        }
                        else  if (Global.InterfaceType.Equals("DY") || Global.InterfaceType.Equals(""))
                        {
                ...
                case MsgCode.MSG_UPLOAD:
                    ...
                    else if (Global.InterfaceType.Equals("GC"))
                    {
                        msg.outError = UploadResult(msg.selectedRecords);
                    }
                ...
                case MsgCode.MSG_DownCompany:
                    try
                    {
                        ...
                        msg.DownLoadCompany ="";
                        AIO.WebReference.DatRockService webDRS = new WebReference.DatRockService();
                        string _company = webDRS.getCompany(msg.str1, msg.str2);
                        if (_company.Contains("code"))
                        {
                            GCResult _rcompany =Json.JsonToEntity<GCResult>(_company);
                            if(_rcompany!=null && _rcompany.code=="1000")
                            {
                                msg.DownLoadCompany = _rcompany.company.ToString ();
                                msg.result = true;
                            }
                            else
                            {
                                if (_rcompany.msg.Trim().Length == 0)
                                {
                                    msg.errMsg = _rcompany.code +":"+ errDetail(_rcompany.code);
                                }
                                else
                                {
                                    msg.errMsg =_rcompany.code+ _rcompany.msg.Trim();
                                }
                            }
                        }
                        else
                        {
                            msg.errMsg = _company + ":" + errDetail(_company);
                        }
                ...
                case MsgCode.MSG_PROJECTS :
                     try
                     {
                        ...
                        msg.responseInfo = "";
                        AIO.WebReference.DatRockService webDRS = new WebReference.DatRockService();
                        string _Department = webDRS.getDepartment(msg.str1);
                        if (_Department.Contains("code"))
                        {
                            GCResult _rDepartment = Json.JsonToEntity<GCResult>(_Department);
                            if (_rDepartment != null && _rDepartment.code == "1000")
                            {
                                msg.responseInfo = _rDepartment.jigougl.ToString ();
                                msg.result = true;
                            }
                            else 
                            {
                                if(_rDepartment.msg.Trim ().Length ==0)
                                {
                                    msg.errMsg = _rDepartment.code +":"+ errDetail(_rDepartment.code);
                                }
                                else
                                {
                                    msg.errMsg = _rDepartment.code + _rDepartment.msg.Trim();
                                }
                            }
                        }
                        else
                        {
                            msg.responseInfo = _Department + ":" + errDetail(_Department);
                        }
                    }
                    catch (Exception ex)
                    {
                        FileUtils.OprLog(0, "downLoadDepartment=-error", ex.ToString());
                        msg.errMsg = ex.Message;
                        msg.result = false;
                    }
        ...
        private string errDetail(string code)
        {
            string rtn = "";
            if(code=="1001")
            {
        ...
        private byte[] jbkIN(ComPort comPort, bool isTest)
        {
            ...
            //{
            //    str += str == "" ? sendBuffer[i].ToString("X2") : "," + sendBuffer[i].ToString("X2");
            //}
            //FileUtils.OprLog(2,"发送进卡指令",str);
                    ...
                    if (receiveData[0] == 0x7e && receiveData[1] == 0x02 && receiveData[8] == 0x7e)
                    {
                            ...
                            */CMD.Validation(receiveData))//新摄像头校验
                        {
                            ...
                            else if (holenum == "通道2")
                            {
                                ...
                                Global.JtjVersion= Global.JtjVersionTwo = receiveData[5] == 0x30 ? 3 : 2;
                            ...
                            else if (holenum == "通道3")
                            {
                                ...
                                Global.JtjVersion = Global.JtjVersionThree = receiveData[5] == 0x30 ? 3 : 2;
                            ...
                            else if (holenum == "通道4")
                            {
                                ...
                                Global.JtjVersion = Global.JtjVersionFour = receiveData[5] == 0x30 ? 3 : 2;
        ...
        private String UploadResult(List<tlsTtResultSecond> selectedRecords)
        {
            ...
            else if (Global.InterfaceType.Equals("GC"))
            {
                ...
                {
                    if (selectedRecords[i].IsUpload == "已上传")//禁止重传
                        continue;
                    if (selectedRecords[i].Result == "检测无效")//禁止上传无效
                    {
                        continue;
                    }
                    List<GCUpData> mm = new List<GCUpData>();
                    GCUpData upload = new GCUpData();
                    upload.CheckID = selectedRecords[i].CheckNo;
                    upload.department = selectedRecords[i].projectID;
                    upload.detComp = selectedRecords[i].CheckedCompanyCode;
                    upload.boothNumber = selectedRecords[i].stallID;
                    upload.sampNumber = selectedRecords[i].SampleCode;
                    upload.category = selectedRecords[i].FoodType;
                    upload.sampleName = selectedRecords[i].FoodName;
                    upload.testItem = selectedRecords[i].CheckTotalItem;
                    upload.standardVal = selectedRecords[i].StandValue;
                    upload.detection = selectedRecords[i].CheckValueInfo;
                    upload.detectionCom = selectedRecords[i].ResultInfo;
                    upload.detResult = selectedRecords[i].Result;
                    upload.detGist = selectedRecords[i].Standard ;
                    upload.detDate = selectedRecords[i].CheckStartDate;
                    upload.jcshi = selectedRecords[i].CheckUnitName;
                    upload.inspector = Global.userId;// selectedRecords[i].CheckUnitInfo;
                    upload.kjshebei = selectedRecords[i].CheckMachine;

                    mm.Add(upload);
                    string upJson = Json.Tojson(mm);
                    AIO.WebReference.DatRockService webDRS = new WebReference.DatRockService();
                    string rtn= webDRS.uploadDataDock(upJson, Global.Token);
                    if(rtn.Contains("code"))
                    {
                        //ResultUpload _rresult = Json.AnalyseObject<ResultUpload>(rtn);
                        List<ResultUpload> _rresult = JsonConvert.DeserializeObject<List<ResultUpload>>(rtn);
                        if (_rresult != null && _rresult[0].code == "1000")
                        {
                            _resultTable.UpdateUploads(string.Format("('{0}')", selectedRecords[i].SysCode), out errMsg);
                            Global.UploadSCount++;
                        }
                        else
                        {
                            if (_rresult[0].msg.Trim().Length == 0)
                            {
                                errMsg = _rresult[0].code + errDetail(_rresult[0].code);
                            }
                            else
                            {
                                errMsg = _rresult[0].code + _rresult[0].msg.Trim();
                            }
                            //errMsg = _rresult[0].code + " " + _rresult[0].msg; 
                        }
                    }
                    else
                    {
                        errMsg = rtn + ":" + errDetail(rtn);
                    }
