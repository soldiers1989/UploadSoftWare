                    ...
                    if (_contrast)
                    {
                        ...
                        for (int i = 0; i < Global.deviceHole.HoleCount; ++i)
                        {
                            if (_use[i] && i != contrastIdx)
                            {
                                ...
                                _CheckValue[(num > 0 ? (i - num) : i), 15] = _item.Hole[i].SampleTypeName;//样品种类编号
                                _CheckValue[(num > 0 ? (i - num) : i), 16] = _item.testPro;//检测项目编号
                                _CheckValue[(num > 0 ? (i - num) : i), 17] = "1";//检测结果类型 1，定量，2定性 分光光度都为1
                                _CheckValue[(num > 0 ? (i - num) : i), 18] = _item.Hole[i].CompanyCode; //检测结果编号 dataNum
                                ...
                                _CheckValue[(num > 0 ? (i - num) : i), 21] = _item.Hole[i].projectCode;
                                _CheckValue[(num > 0 ? (i - num) : i), 22] = _item.Hole[i].projectName ;
                                _CheckValue[(num > 0 ? (i - num) : i), 23] = _item.Hole[i].StallCode ;
                                _CheckValue[(num > 0 ? (i - num) : i), 24] = _item.Hole[i].stall;
