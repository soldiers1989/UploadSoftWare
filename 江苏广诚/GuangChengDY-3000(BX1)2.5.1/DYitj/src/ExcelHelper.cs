...
namespace AIO.src
{
    public static class ExcelHelper
    {
        ...
        /// <param name="sheetIndex">sheet索引</param>
        /// <returns>成功返回datatable，失败返回null</returns>
        public static DataTable ImportExcelss(string excelPath, int sheetIndex)
        {
            IWorkbook workbook = null;//全局workbook
            ISheet sheet;//sheet
            DataTable table = null;
            try
            {
                ...
                if (workbook != null)
                {
                    ...
                    IRow VerRow = sheet.GetRow(0);//第一行版本

                    int VcellCount = VerRow.LastCellNum;//得到版本列数

                    if (VerRow.GetCell(1) != null)
                    {
                        VerItem = VerRow.Cells[1].ToString();
                        Console.WriteLine(VerItem);
                    }

                    table = new DataTable();//初始化一个table

                    IRow headerRow = sheet.GetRow(1);//获取第一行，一般为表头
                    int cellCount = headerRow.LastCellNum;//得到列数
        ...
        /// 生成Excel
        /// </summary>
        public static bool CreateExcel(DataTable dt, string path)
        {
            try
            {
