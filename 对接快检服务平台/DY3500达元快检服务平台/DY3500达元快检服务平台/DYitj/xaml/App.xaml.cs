...
namespace AIO
{
    ...
    public partial class App : Application
    {
        private void Application_Startup(object sender, StartupEventArgs e)
        {
            ...
            //Global.Token = CFGUtils.GetConfig("QToken", "");

            CFGUtils.addConfig("showCT", "0");
            CFGUtils.addConfig("FactoryNumber", "DY3500I_1234");
            CFGUtils.addConfig("heattime", "15");
            CFGUtils.addConfig("GPSAddress", "http://dyxx.chinafst.cn:8898/UpdateTool/");
            ...
            Global.MachineNum = CFGUtils.GetConfig("IntrumentSeriersNum", "");//平台下载的系列号
            Global.MachineModel = CFGUtils.GetConfig("InstrumentNameModel", "");
            Global.SDdongying = CFGUtils.GetConfig("dongying", "");//东营投标用

            Global.IsAutoTest = CFGUtils.GetConfig("isAutoTest", "");//是否自动测试
            Global.showCT = CFGUtils.GetConfig("showCT", "0").Equals("1") ? true : false;

            Global.FactoryNumber = CFGUtils.GetConfig("FactoryNumber", "DY3500I_1234");//加载出厂编号

            Global.heatTime = double.Parse(CFGUtils.GetConfig("heattime", "15"));//心跳包更新时间

            Global.GPSAddress = CFGUtils.GetConfig("GPSAddress", "http://dyxx.chinafst.cn:8898/UpdateTool/");
            ...
            if (Global.samplenameadapter.Count > 0)
            {
                ...
                Global.IsSweepCode = CPoint.IsAutoSweed;
            }
        }
