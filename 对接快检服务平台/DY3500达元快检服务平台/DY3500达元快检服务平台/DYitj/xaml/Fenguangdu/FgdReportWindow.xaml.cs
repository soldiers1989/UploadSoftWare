    ...
    public partial class FgdReportWindow : Window
    {
        ...
        private void UpdateLable()
        {
            try
            {
                ...
                {
                    if (_item.detect_method == "抑制率法" && _item.ir.RefDeltaA != Double.MinValue)
                        this.label1.Content = "对照值：" + String.Format("{0:F3}", _item.ir.RefDeltaA);
                    else if (_item.detect_method == "标准曲线法" && _item.sc.RefA != Double.MinValue)
            ...
            try
            {
                ...
                for (int i = 0; i < HoleCount; ++i)
                {
                    if (Global.ManuTest == true)
                    {
                        ...
                        if (_item.detect_method == "抑制率法" || _item.detect_method == "动力学法")
                        {
                            if (_item.Hole[i].Use && _item.Hole[i].IsTest)
                            {
                                _use[i] = true;
                                _CheckValue = new string[_HoleNumber, 20];
                                _HoleNumber++;
                                num++;
                            }
                            else
                ...
                for (int i = 0; i < HoleCount; ++i)
                {
                    if (_use[i])
                    {
                        ...
                        if (Global.ManuTest == true)
                        {
                            element = GenerateReportLine(String.Format("{0:D2}", (i + 1)), String.Format("{0:D5}", sampleNum++), _item.Hole[i].SampleName, string.Empty + _realHeatms / 1000, _item.Unit, string.Empty, string.Empty, string.Empty);
                        }
                        else
                        {
                            element = GenerateReportLine(String.Format("{0:D2}", (i + 1)), String.Format("{0:D5}", sampleNum++), _item.Hole[i].SampleName, string.Empty + _realHeatms / 1000, _item.Unit, string.Empty, string.Empty, string.Empty);
        ...
        private void Upload()
        {
            ...
            try
            {
                ...
                if (Global.ManuTest == false)
                {
                    ...
                    if (Global.iscanzhao == true)
                    {
                        Global.deviceHole.SxtCount = Global.deviceHole.SxtCount - 1;
                    }
        ...
        private string[] GetResult(string[] Qualifide, string strValue)
        {
            ...
            else
            {
                if ("抑制率法" == _item.detect_method)
                {
                    ...
                    Qualifide[1] = Qualifide[1].Length == 0 ? "GB/T 5009.199-2003" : Qualifide[1];
                    Qualifide[2] = Qualifide[2].Length == 0 ? _item.yin_max : Qualifide[2];// _item.ir.MinusH.ToString("F2") : Qualifide[2];
                    Qualifide[3] = "≤";
                    Qualifide[4] = "%";
                }
        ...
        private void ShowResult()
        {
            try
            {
                if (_item.stdA0!=null && _item.stdA0 != "")
                {
                    _item.sc.A0 = double.Parse(_item.stdA0);
                    _item.co.A0 = double.Parse(_item.stdA0);
                }
                if (_item.stdA1 != null && _item.stdA1 != "")
                {
                    _item.sc.A1 = double.Parse(_item.stdA1);
                    _item.co.A1 = double.Parse(_item.stdA1);
                }
                if (_item.stdA2 != null && _item.stdA2 != "")
                {
                    _item.sc.A2 = double.Parse(_item.stdA2);
                    _item.co.A2 = double.Parse(_item.stdA2);
                }
                if (_item.stdA3 != null && _item.stdA3 != "")
                {
                    _item.sc.A3 = double.Parse(_item.stdA3);
                    _item.co.A3 = double.Parse(_item.stdA3);
                }
                if (_item.stdB0 != null && _item.stdB0 != "")
                {
                    _item.sc.B0 = double.Parse(_item.stdB0);
                }
                if (_item.stdB1 != null && _item.stdB1 != "")
                {
                    _item.sc.B1 = double.Parse(_item.stdB1);
                }
                if (_item.stdB2 != null && _item.stdB2 != "")
                {
                    _item.sc.B2 = double.Parse(_item.stdB2);
                }
                if (_item.stdB3 != null && _item.stdB3 != "")
                {
                    _item.sc.B3 = double.Parse(_item.stdB3);
                }
                        ...
                        for (int i = 0; i < HoleCount; ++i)
                        {
                            if (_use[i])
                            {
                                ...
                                else
                                {
                                    ...
                                    //保存对照时间
                                    _item.SaveTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                                    //Global.SerializeToFile(Global.fgdItems, Global.fgdItemsFile);
                                    string data = " SaveTime='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',RefDeltaA='" + _item.ir.RefDeltaA+"'";

                                    _resultTable.UpdateMItem(_item.mid, data);
                                }
                        ...
                        for (int i = 0; i < HoleCount; ++i)
                        {
                            if (_use[i] && i != contrastIdx)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (izl >= double.Parse(_item.yin_min) && izl <= double.Parse(_item.yin_max))
                                        //str = String.Format("{0:P1}", izhilv[i]);
                                        str = String.Format("{0:N3}", izhilv[i] * 100) + "%";
                                    else if (izl >= double.Parse(_item.yang_min) && izl <= double.Parse(_item.yang_max))
                                        //str = String.Format("{0:P1}", izhilv[i]);
                                        str = String.Format("{0:N3}", izhilv[i] * 100) + "%";
                                    else
                                        str = "NA";
                                ...
                                _listUnit[i].Content =_item.detect_unit;

                                _CheckValue[(num > 0 ? (i - num) : i), 0] = _item.Hole[i].HoleNumber; //String.Format("{0:D2}", (i + 1)); 
                                ...
                                _CheckValue[(num > 0 ? (i - num) : i), 5] = UnqualifiedValue[4].Equals(string.Empty) ? _item.detect_unit : UnqualifiedValue[4];// _item.Unit
                                ...
                                _CheckValue[(num > 0 ? (i - num) : i), 17] = _item.Hole[i].tID;//检测任务ID
                            }
                            else
                            {
                                num += 1;
                        ...
                        for (int i = 0; i < HoleCount; ++i)
                        {
                            ...
                            if (_use[i])
                            {
                                ...
                                else
                                {
                                    ...
                                    int izl = (int)(izhilv[i] * 100);
                                    if (izl >= double.Parse(_item.yin_min) && izl <= double.Parse(_item.yin_max))
                                        //str = String.Format("{0:P}", izhilv[i]);//转成百分数后保留两位小数
                                        str = String.Format("{0:N3}", izhilv[i]*100)+"%";
                                    else if (izl >= double.Parse(_item.yang_min) && izl <= double.Parse(_item.yang_max))
                                        //str = String.Format("{0:P}", izhilv[i]);//转成百分数后保留两位小数
                                        str = String.Format("{0:N3}", izhilv[i] * 100)+"%";
                ...
                {
                    if (_item.reserved1 != null && _item.reserved1.Length >0)
                    {
                        string[] s = _item.reserved1.Split('-');
                        if (s.GetLength(0) > 1)
                        {
                            _item.sc.ATo = double.Parse(s[1]);
                        }
                    }
                    // 标准曲线， 计算浓度
                    if (_contrast)
                        ...
                        for (int i = 0; i < HoleCount; ++i)
                        {
                            if (_use[i])
                            {
                                ...
                                if (FgdCaculate.IsValid(_item.sc.RefA))
                                {
                                    ...
                                    _item.SaveTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                                    //Global.SerializeToFile(Global.fgdItems, Global.fgdItemsFile);
                                    string data = " SaveTime='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',RefDeltaA='" + _item.sc.RefA + "'";
                ...
                else if ("动力学法" == _item.detect_method)
                {
                    string[] s = _item.reserved1.Split('-');
                    if (s.GetLength(0) > 1)
                    {
                    ...
                    if (_sample)
                    {
                        ...
                        for (int i = 0; i < HoleCount; ++i)
                        {
                            ...
                            if (_use[i])
                            {
                                ...
                                _CheckValue[(num > 0 ? (i - num) : i), 1] = "分光光度";
                                _CheckValue[(num > 0 ? (i - num) : i), 2] = _item.item;
                                _CheckValue[(num > 0 ? (i - num) : i), 3] = _item.item;
                                _CheckValue[(num > 0 ? (i - num) : i), 4] = str;
                                //单位直接从样品表中拿，不在项目配置文件中拿
                                //_CheckValue[(num > 0 ? (i - num) : i), 5] = _item.Unit;
                                _CheckValue[(num > 0 ? (i - num) : i), 5] = UnqualifiedValue[4].Equals(string.Empty) ? _item.Unit : UnqualifiedValue[4];
                ...
                else if ("系数法" == _item.detect_method)
                {
                    //系数法
        ...
        private void Btn_ShowDatas_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                window._CheckItemName = _item.item;
            }
            window.Show();
        }
        //原来的结果显示
                    ...
                    if (_contrast)
                    {
                        ...
                        for (int i = 0; i < HoleCount; ++i)
                        {
                            if (_use[i] && i != contrastIdx)
                            {
                                ...
                                {
                                    int izl = (int)(izhilv[i] * 100);
                                    //if (izl >= _item.ir.MinusL && izl <= _item.ir.MinusH)//以前的
                                    //    str = String.Format("{0:P1}", izhilv[i]);
                                    //else if (izl >= _item.ir.PlusL && izl <= _item.ir.PlusH)
                                    //    str = String.Format("{0:P1}", izhilv[i]);
                                    //else
                                ...
                                _StandValue[i].Content = Convert.ToString(UnqualifiedValue[3]) + Convert.ToString(UnqualifiedValue[2] + UnqualifiedValue[4]);
                                _StandValue[i].ToolTip = _StandValue[i].Content;
                                _listUnit[i].Content=  UnqualifiedValue[4].Equals(string.Empty) ? _item.detect_unit : UnqualifiedValue[4];
                                ...
                                _CheckValue[(num > 0 ? (i - num) : i), 3] = _item.detect_method;
                                _CheckValue[(num > 0 ? (i - num) : i), 4] = strCheckValue;
                                _CheckValue[(num > 0 ? (i - num) : i), 5] = UnqualifiedValue[4].Equals(string.Empty) ? _item.detect_unit : UnqualifiedValue[4];
                                _CheckValue[(num > 0 ? (i - num) : i), 6] = NowDateTime;
                                _CheckValue[(num > 0 ? (i - num) : i), 7] = LoginWindow._userAccount.UserName;
                                _CheckValue[(num > 0 ? (i - num) : i), 8] = string.IsNullOrEmpty(_item.Hole[i].SampleName) ? string.Empty : _item.Hole[i].SampleName;
                                _CheckValue[(num > 0 ? (i - num) : i), 9] = Convert.ToString(UnqualifiedValue[0]);
                                ...
                                _CheckValue[(num > 0 ? (i - num) : i), 15] = string.IsNullOrEmpty(_item.Hole[i].SampleId) ? string.Empty : _item.Hole[i].SampleId;
                                _CheckValue[(num > 0 ? (i - num) : i), 16] = _item.Hole[i].ProduceCompany;
                                _CheckValue[(num > 0 ? (i - num) : i), 17] = "是";
