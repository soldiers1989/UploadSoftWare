    ...
    public partial class TaskDisplay : Window
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                timer.Interval = TimeSpan.FromMilliseconds(550);//500ms
                timer.Tick += timer_Tick;
                if (Global.IsSweepCode)
                {
                    GridBarTast.Visibility=Visibility.Visible  ;
                    DataGridRecord.Visibility = Visibility.Collapsed;
                }
                else
                {
                    DataGridRecord.Visibility = Visibility.Visible;
                    GridBarTast.Visibility = Visibility.Collapsed;
                }
        ...
        private void timer_Tick(object sender, EventArgs e)
        {
            timer.Stop();
           //编写程序
            DownBarTask(TxtBarInput.Text.Trim());
            //TxtBarInput.SelectAll();
            TxtBarInput.Text = "";
            TxtBarInput.Focus();
        }
        /// <summary>
        ...
        private  void _GetTaskTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                if (Global.IsSweepCode)//扫码输入不自动接受任务
                {
                    return;
                }

                gotimes = gotimes + 1;
                if(gotimes ==120)//20分钟重新登录一次
                {
                    ServerLogon();
                    gotimes = 0;
                }
                heats = heats + 1;
                //更新心跳包
                if ((Global.heatTime == heats / 6) || (Global.heatTime < heats / 6))//转成分钟计算 10S定时一次6次一分钟
                {
                    //heats = 0;
                    //UpHeartBeat();
                }
        ...
        private void SearchTask()
        {
            if (Global.IsSweepCode)
            {
                DataGridBar.ItemsSource = null;
                string stime = DateTime.Now.ToString("yyyy-MM-dd");
                sb.Length = 0;
                sb.AppendFormat(" IsTest is null or IsTest='' or IsTest='已检测' and getSampleTime like '%{0}%' ", stime);
                DataTable dt = _Tskbll.GetBarTask(sb.ToString(), "IsTest,getSampleTime", 1, out err);
                if (dt != null && dt.Rows.Count > 0)
                {
                    List<clsBarTask> Items = (List<clsBarTask>)IListDataSet.DataTableToIList<clsBarTask>(dt, 1);
                    if (Items.Count > 0)
                        this.DataGridBar.ItemsSource = Items;//(Items != null && Items.Count > 0) ? Items : null;这写法有问题
                }
            }
            else
            {
        ...
        private void selecttask()
        {
            ...
            try
            {
                ...
                {
                    clsBarTask BarTask;
                    if (DataGridBar.SelectedItems.Count > 0)
                    {
                        //判断是否重检
                        ArrayList retest = new ArrayList();
                        retest.Clear();
                        for (int i = 0; i < DataGridBar.SelectedItems.Count; i++)
                        {
                            BarTask = (clsBarTask)DataGridBar.SelectedItems[i];
                            if (retest.Count == 0)
                            {
                                retest.Add(BarTask.IsTest );
                            }
                            //if (BarTask.IsTest == "已检测")
                            //{
                        ...
                        Mokuailist.Clear();
                        for (int j = 0; j < DataGridBar.SelectedItems.Count; j++)
                        {
                            BarTask = (clsBarTask)DataGridBar.SelectedItems[j];
                            if (Mokuailist.Count == 0)
                            {
                                Mokuailist.Add(BarTask.mokuai);
                            }
                            else
                            {
                                if (!Mokuailist.Contains(BarTask.mokuai))
                                {
                        ...
                        for (int j = 0; j < DataGridBar.SelectedItems.Count; j++)
                        {
                            ...
                            if (Itemlist.Count == 0)
                            {
                                Itemlist.Add(BarTask.itemName );
                            }
                            else
                            {
                                if (!Itemlist.Contains(BarTask.itemName ))
                                {
                        ...
                        Global.TestCompany = new string[DataGridBar.SelectedItems.Count];//被检单位
                        //加载样品
                        Global.Testsample = new string[DataGridBar.SelectedItems.Count, 4];
                        for (int j = 0; j < DataGridBar.SelectedItems.Count; j++)
                        {
                            BarTask = (clsBarTask)DataGridBar.SelectedItems[j];
                            Global.Testsample[j, 0] = BarTask.foodName ;
                            Global.Testsample[j, 1] = BarTask.bid;
                            Global.Testsample[j, 2] = BarTask.InBarCode ;
                            Global.Testsample[j, 3] = BarTask.bagCode;
                        ...
                        {
                            BarTask = (clsBarTask)DataGridBar.SelectedItems[j];
                            if (!list.Contains(BarTask.mokuai ))
                            {
                                list.Add(BarTask.mokuai);
                            }
                            else
                            {
                            ...
                            if (BarTask.mokuai == "分光光度")
                            {
                                ...
                                window._itemname = BarTask.itemName ;
                                window._sample = BarTask.foodName ;
                                //window._Company = tlsTrTask.s_reg_name;
                                //window._title = tlsTrTask.t_task_title;
                                Global.Kitem = BarTask.itemName;
                                Global.Ksample = BarTask.foodName;
                ...
                else 
                {
                    ...
                    if (DataGridRecord.SelectedItems.Count > 0)
                    {
                        ...
                        //加载样品
                        Global.Testsample = new string[DataGridRecord.SelectedItems.Count, 4];
                        for (int j = 0; j < DataGridRecord.SelectedItems.Count; j++)
                        {
                            tlsTrTask = (tlsTrTask)DataGridRecord.SelectedItems[j];
        ...
        private void btnoject_Click(object sender, RoutedEventArgs e)
        {
            if(Global.IsSweepCode )
            {
                try
                {
                    if (DataGridBar.SelectedItems.Count < 1)
                    {
                        MessageBox.Show("请选择需要拒收的任务");
                        return;
                    }
                    string code = "";
                    clsBarTask BarTask = (clsBarTask)DataGridBar.SelectedItems[0];
                    if (BarTask != null)
                    {
                        code = BarTask.bid;
                    }

                    btnoject.IsEnabled = false;
                

                    string rtn = InterfaceHelper.QuickTestServerLogin(Global.samplenameadapter[0].url, Global.samplenameadapter[0].user, Global.samplenameadapter[0].pwd, code, 6);
                    FileUtils.KLog(rtn, "接收", 24);
                    if (rtn.Contains("msg") || rtn.Contains("success") || rtn.Contains("resultCode") || rtn.Contains("obj"))
                    {
                        ResultData Jresult = JsonHelper.JsonToEntity<ResultData>(rtn);

                        if (Jresult.resultCode == "0X00000")
                        {
                            
                            LabelInfo.Content = "任务拒收成功！";
                            _Tskbll.DeleteBarTask("ID=" + BarTask.ID , "", 1, out err);
                            SearchTask();
                        }
                        else
                        {
                            MessageBox.Show("任务拒收失败，失败原因：" + Jresult.msg);
                            LabelInfo.Content = "任务拒收失败！";
                        }
                    }
                    else
                    {
                        MessageBox.Show("任务拒收失败，失败原因：" + rtn);
                        LabelInfo.Content = "任务拒收失败！";
                    }
                    
                    LabelInfo.Visibility = Visibility.Visible ;
                    btnoject.IsEnabled = true;
        ...
        private void Repair_Click(object sender, RoutedEventArgs e)
        {
            if (Global.IsSweepCode)
            {
                ...
                try
                {
                    ...
                    clsBarTask BarTask = (clsBarTask)DataGridBar.SelectedItems[0];
                    BarRepairTaskWindow window = new BarRepairTaskWindow();
                    window.model = BarTask.mokuai;
                    window.item = BarTask.itemName;
                    window.samplename = BarTask.foodName;
                    window.SampleTime = BarTask.getSampleTime;
                    window.SampleID = BarTask.bid;

                    window.ShowDialog();

                    if (Global.modelkuai != "")
                    {
                        _Tskbll.UpdateBarTask(BarTask.bid , Global.modelkuai, 1, out err);
                        SearchTask();
                    }
        ...
        private void DownBarTask(string code )
        {
            try
            {
                //string data = "[{\"id\":497824,\"foodName\":\"白菜\",\"itemName\":\"农药残留\",\"bagCode\":\"A00000017\"}]";

                //List<BarTask> infoList = JsonConvert.DeserializeObject<List<BarTask>>(data);
                //if (infoList != null && infoList.Count > 0)
                //{
                //    BarTask model = new BarTask();
                //    tlsttResultSecondOpr _bll = new tlsttResultSecondOpr();
                //    for (int i = 0; i < infoList.Count; i++)
                //    {
                //        model.id = infoList[i].id;
                //        model.bagCode = infoList[i].bagCode;
                //        model.foodName = infoList[i].foodName;
                //        model.itemName = infoList[i].itemName;
                //        //model.IsTest = "否";
                //        model.getSampleTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                //        model.InBarCode = code;

                //        DataTable mdt = _bll.GetModel(infoList[i].itemName, "", out err);//查询获取模块
                //        if (mdt != null && mdt.Rows.Count > 0)
                //        {
                //            model.mokuai = mdt.Rows[0]["project_type"].ToString();
                //        }
                //        int rtnd = _Tskbll.InsertBarTask(model, out err);
                ...
                int icount = 0;
                string rtn = InterfaceHelper.QuickTestServerLogin(Global.samplenameadapter[0].url, Global.samplenameadapter[0].user, Global.samplenameadapter[0].pwd, code, 5);
                FileUtils.KLog(rtn, "接收", 24);
                if (rtn.Contains("msg") || rtn.Contains("success") || rtn.Contains("resultCode") || rtn.Contains("obj"))
                {
                    ResultData Jresult = JsonHelper.JsonToEntity<ResultData>(rtn);
                    if (Jresult.resultCode == "0X00000")
                    {
                        List<BarTask> infoList = JsonConvert.DeserializeObject<List<BarTask>>(Jresult.obj.ToString());
                        if (infoList != null && infoList.Count > 0)
                        {
                            BarTask model = new BarTask();
                            tlsttResultSecondOpr _bll = new tlsttResultSecondOpr();
                    ...
                    {
                        MessageBox.Show("扫码下载任务失败，失败原因：" + Jresult.msg);
                    }
                }
                else
                {
                    MessageBox.Show("扫码下载任务失败，失败原因：" + rtn);
                }
                LabelInfo.Content = "共下载" + icount + "条数据！";
                LabelInfo.Visibility = Visibility.Visible;
                SearchTask();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnDownTask_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                btnDownTask.IsEnabled = false;
                DownBarTask(TxtKeyInput.Text.Trim());
                btnDownTask.IsEnabled = true;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message );
        ...
        private void TxtBarInput_TextChanged(object sender, TextChangedEventArgs e)
        {
            
            if (TxtBarInput.Text.Trim().Length > 1)//有一个字符输入时开始计时,等待输入完成再识别
            {
                timer.Start();
            }
        }

        private void DataGridBar_LoadingRow(object sender, DataGridRowEventArgs e)
        {
            e.Row.Header = e.Row.GetIndex() + 1;
            DataGridRow dataGridRow = e.Row;
            clsBarTask dataRow = e.Row.Item as clsBarTask;
            if (dataRow != null)
            {
                if (dataRow.IsTest  == "已检测")
                {
                    dataGridRow.Background = Brushes.YellowGreen;
                }
                else
        ...
        private void DataGridBar_MouseDoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {

        }

        private void closeform_ImageFailed(object sender, ExceptionRoutedEventArgs e)
        {
