    ...
    public static class InterfaceHelper
    {
        ...
        public static string GetServiceURL(string url, int type)
        {
            lock (urlFlag)
            {
                ...
                else if(type ==16)//上传心跳包
                {
                    url += (index == url.Length - 1) ? "iLog/uploadStatus.do" : "/iLog/uploadStatus.do";
                }
                else if (type == 17)//扫码下载检测任务
                {
                    url += (index == url.Length - 1) ? "iTbSampling/newMode/getTaskByCode" : "/iTbSampling/newMode/getTaskByCode";
                }
                else if (type == 18)//扫码数据上传
                {
                    url += (index == url.Length - 1) ? "iDataChecking/newMode/uploadCheckData" : "/iDataChecking/newMode/uploadCheckData";
                }
                else if (type == 19)//扫码数据上传
                {
                    url += (index == url.Length - 1) ? "iTbSampling/newMode/rejectTask" : "/iTbSampling/newMode/rejectTask";
                }
        ...
        /// <returns></returns>
        public static string QuickTestServerLogin(string url,string user,string pwd,string code, int type)
        {
            lock (dtFlag)
            {
                string err = "";
                ...
                else if (type == 5)//扫码下载任务
                {
                    try
                    {
                        ...
                        responseInfo = string.Empty;
                        lasttime = Global.TasknumTime;
                        string uria = GetServiceURL(url, 17);
                        StringBuilder builder = new StringBuilder();
                        builder.Append(uria);
                        builder.AppendFormat("?userToken={0}",  Global.Token);
                        builder.AppendFormat("&code={0}", code);
                        builder.AppendFormat("&serialNumber={0}", Global.MachineNum);

                        FileUtils.KLog(builder.ToString(), "发送", 24);
                ...
                else if (type ==6)//拒收扫码任务
                {
                    try
                    {
                        ...
                        responseInfo = string.Empty;
                        lasttime = Global.TasknumTime;
                        string uria = GetServiceURL(url, 19);
                        StringBuilder builder = new StringBuilder();
                        builder.Append(uria);
                        builder.AppendFormat("?userToken={0}", Global.Token);
                        builder.AppendFormat("&id={0}", code);
                        builder.AppendFormat("&serialNumber={0}", Global.MachineNum);
