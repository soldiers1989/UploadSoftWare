...
namespace com.lvrenyang
{
    public class FileUtils
    {
        ...
        private static object logob = new object();
        /// <summary>
        /// 快检服务平台收发日记
        /// </summary>
        ...
        public static void KLog(string content,string SendReceive, int type)
        {
            lock (logob)
            {
                ...
                else if (type == 23)//更新心跳包
                {
                    DateTime date = DateTime.Now;
                    string fileName = "心跳包" + date.ToString("yyyy-MM-dd") + ".log";
                    AddToFile(SaveLogDirectory + "\\xintiaobao", fileName, SendReceive + date.ToString("HH:mm:ss\r\n") + content + "\r\n\r\n");
                }
                else if (type == 24)
                {
                    DateTime date = DateTime.Now;
                    string fileName = "扫码下任务" + date.ToString("yyyy-MM-dd") + ".log";
                    AddToFile(SaveLogDirectory + "\\saomaxiarenwu", fileName, SendReceive + date.ToString("HH:mm:ss\r\n") + content + "\r\n\r\n");
                }
