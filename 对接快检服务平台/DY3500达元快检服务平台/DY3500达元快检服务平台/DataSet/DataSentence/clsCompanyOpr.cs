    ...
    public class clsCompanyOpr
    {
        ...
        public DataTable GetRegulator(string name,int type)
        {
            ...
            try
            {
                if (type == 1)
                {
                    sb.Append("select reg_name,link_user,reg_address,link_phone,update_date ");
                    sb.Append(" from regulardata  ");//where b.reg_id=r.rid
                }
                else if (type == 2)
                {
                    sb.Append("SELECT * FROM tCompany");
        ...
        /// <param name="errMsg"></param>
        /// <returns></returns>
        public  int DeleteTasks(string deviceId, out string errMsg)
        {
            errMsg = string.Empty;
            int rtn = 0;
            try
            {
                sb.Length = 0;
                sb.AppendFormat("delete from KTask where recevie_device<>'{0}' " ,deviceId);
