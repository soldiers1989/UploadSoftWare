    ...
    public partial class FgdMeasureWindow : Window
    {
        ...
        private void Initial()
        {
            if (Global.ManuTest == true)
            {
                try
                {
                    this.LabelTitle.Content = "正在检测  " + _item.item;
                    if ("子项目法" == _item.detect_method)
                    {
                    ...
                    this.ButtonSampleTest.IsEnabled = (_item.detect_method == "抑制率法") ? ((_item.ir.RefDeltaA == Double.MinValue) ? false : true) : ((_item.detect_method == "标准曲线法" && _item.sc.RefA == Double.MinValue) ? false : true);
        ...
        {
            string path = string.Format(@"{0}\KnowledgeBbase\检测项目操作说明\{1}.rtf", Environment.CurrentDirectory, _item.item);
            try
            {
                if (File.Exists(path))
                {
        ...
        private void ButtonSampleTest_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (Global.ManuTest == true)
                {
                    if (!_sample)
                    {
                        ...
                        else
                        {
                            ...
                            if ("抑制率法" == _item.detect_method)
                            {
                                ...
                                _preHeatms = int.Parse(_item.pre_time) * 1000; //_item.ir.PreHeatTime * 1000;
                                _realHeatms = int.Parse(_item.dec_time) * 1000; //_item.ir.DetTime * 1000;
                            }
                    ...
                    else if (_item.detect_method == "系数法")
                    {
                        _sample = true;
                        _startTime = DateTime.Now;
                        _firstATs = null;
                ...
                else
                {
                    if (!_sample)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                _preHeatms = int.Parse(_item.dec_time) * 1000; //_item.sc.PreHeatTime * 1000;
                                _realHeatms = int.Parse(_item.dec_time) * 1000; //_item.sc.DetTime * 1000;
                            }
                            else if ("动力学法" == _item.detect_method)
                            {
                                _preHeatms = int.Parse(_item.dec_time) * 1000; //_item.dn.PreHeatTime * 1000;
                                _realHeatms = int.Parse(_item.dec_time) * 1000;//_item.dn.DetTime * 1000;
                            }
                            else if ("系数法" == _item.detect_method)
                            {
                                _preHeatms = 0;
            ...
            try
            {
                ...
                if (!_contrast)
                {
                    ...
                    if (Global.ManuTest == true)
                    {
                        if ("抑制率法" == _item.detect_method)
                        {
                            _preHeatms = int.Parse(_item.pre_time) * 1000;
                            _realHeatms = int.Parse(_item.dec_time) * 1000;
                        }
                        else if ("标准曲线法" == _item.detect_method)
                        {
                            _preHeatms = int.Parse(_item.pre_time) * 1000;
                    ...
                    else 
                    {
                        ...
                        {
                            _preHeatms = int.Parse(_item.pre_time) * 1000;//_item.sc.PreHeatTime * 1000;
                            _realHeatms = int.Parse(_item.dec_time) * 1000; //_item.sc.DetTime * 1000;
                        }
                        else if ("动力学法" == _item.detect_method)
                        {
                            _preHeatms = int.Parse(_item.pre_time) * 1000;//_item.dn.PreHeatTime * 1000;
                            _realHeatms = int.Parse(_item.dec_time) * 1000; //_item.dn.DetTime * 1000;
                        }
                    }
        ...
        private UIElement GenerateSampleMeasureForm(string channel, string sampleNum, string sampleName)
        {
            ...
            if (Global.ManuTest == true)
            {
                separateTest.Visibility = (_item.detect_method == "抑制率法" || _item.detect_method == "动力学法") ? Visibility.Visible : Visibility.Collapsed;
            }
            else
            {
                separateTest.Visibility = (_item.detect_method == "抑制率法" || _item.detect_method == "动力学法") ? Visibility.Visible : Visibility.Collapsed;
        ...
        private void Updates()
        {
            try
            {
                if (Global.ManuTest == true)
                {
                    ...
                    if ("系数法" == _item.detect_method)//DYFGDItemPara.METHOD_CO
                    {
                        if (_firstATs == null)
        ...
        private void Update(byte[] data)
        {
            try
            {
                ...
                if (0 == _adClear)
                {
                    ...
                    if (_item.detect_method == "抑制率法" || _item.detect_method == "标准曲线法")
                        this.ButtonSampleTest.IsEnabled = (_item.detect_method == "抑制率法") ? ((_item.ir.RefDeltaA == Double.MinValue) ? false : true) : ((_item.detect_method == "标准曲线法" && _item.sc.RefA == Double.MinValue) ? false : true);
                ...
                for (int i = 0; i < HoleCount; ++i)
                {
                    if (_item.Hole[i].Use)
                    {
                        ...
                        {
                            if (isContrast && _item.detect_method == "抑制率法" && _item.ir.RefDeltaA == Double.MinValue)
                            {
                                _listBtnSeparate[i].IsEnabled = _isSeparateContrast ? false : true;
                                _listBtnSeparate[i].Content = "对    照 " + String.Format("{0:D2}", (i + 1));
                                isContrast = false;
                            }
                            else if (_isSeparateContrast && _item.detect_method == "抑制率法")
                                _listBtnSeparate[i].IsEnabled = !_listIsStart[i];
                            else
                            {
                                _listBtnSeparate[i].Content = "样    品 " + String.Format("{0:D2}", (i + 1));
                                _listBtnSeparate[i].IsEnabled = (_item.detect_method == "抑制率法") ? ((_item.ir.RefDeltaA == Double.MinValue) ? false : !_listIsStart[i]) : !_listIsStart[i];
                            }
                        }
                        if (_listpassms != null)
                        {
                ...
                {
                    // 抑制率法 从第一通道进入对照方法
                    if (_isSeparateContrast && _item.detect_method == "抑制率法" && _item.ir.RefDeltaA == Double.MinValue)
                    {
                        for (int i = 0; i < HoleCount; ++i)
                        {
                ...
                switch (msg.what)
                {
                    case MsgCode.MSG_READ_AD_CYCLE:
                        ...
                        else
                        {
                            ...
                            if (Global.ManuTest == true)
                            {
                                if (wnd._item.detect_method != "系数法")
                                {
        ...
        private void btnPlayer_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (Global.IsPlayer)
                {
                    MainWindow._aPlayer._ItemName = _item.item;
        ...
        private void oUpdate(byte[] data)
        {
            try
            {
                TimeSpan ts = DateTime.Now - _startTime;
