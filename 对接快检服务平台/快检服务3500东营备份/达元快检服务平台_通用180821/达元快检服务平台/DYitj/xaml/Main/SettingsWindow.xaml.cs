...
namespace AIO
{
    ...
    public partial class SettingsWindow : Window
    {
        ...
        private void Btn_autoUpdater_Click(object sender, RoutedEventArgs e)
        {
            ...
            //    ////服务器通讯验证
            //    str1 = (index == str1.Length - 1) ? str1 : str1 + "/";
            //    sb.Append(str1 + "upload/checkVersion.do");
            //    //sb.AppendFormat("?userToken={0}", Global.Token);
            //    sb.AppendFormat("?appName={0}", Global.MachineModel);
            //    //sb.AppendFormat("&appVersion={0}", "1.0.0");
            //    sb.AppendFormat("&param1={0}", "");
            //    sb.AppendFormat("&param2={0}", "");
            //    sb.AppendFormat("&param3={0}", "");

            //    FileUtils.KLog(sb.ToString(), "发送", 13);
            //    string tasklist = InterfaceHelper.HttpsPost(sb.ToString());
            //    FileUtils.KLog(tasklist, "接收", 13);
            //    checkVersion Mresult = JsonHelper.JsonToEntity<checkVersion>(tasklist);
            //    if (Mresult.msg == "操作成功" && Mresult.success == true)
            //    {
            //        getversion dr = JsonHelper.JsonToEntity<getversion>(Mresult.obj.ToString());
            //        durl = dr.url_path;
            //        int v = int.Parse(CFGUtils.GetConfig("UpdateVersion", ""));
            //        if (int.Parse(dr.version) > v)
            //        {
            //            if (MessageBox.Show("检测有版本更新，版本号为： " + dr.param1 + ", 是否下载更新", "系统提示", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
            //            {
            //                CFGUtils.SaveConfig("UpdateVersion", dr.version);

            //                IniWriteValue("Server", "ServerPath", durl);//保存下载更新软件地址到ini文件
            //                System.Diagnostics.Process.Start(Environment.CurrentDirectory + "\\update.exe");//启动更新软件

            //                //FileInfo[] files = dirInfo.GetFiles();   // 获取该目录下的所有文件
            //                //foreach (FileInfo file in files) { file.Delete(); }
            //                //Directory.Delete(Directory.GetCurrentDirectory() + "\\update", true);//删除文件
            //                //判断文件是否存在，不存在就创建
            //                //    if (!Directory.Exists("D:\\update\\update"))
            //                //    {
            //                //        Directory.CreateDirectory("D:\\update\\update");
            //                //    }
            //                //    FileUtils.KLog(durl, "发送", 13);
            //                //    string down = InterfaceHelper.VersionUpdate(durl);
            //                //    FileUtils.KLog(down, "接收", 13);
            ...
            //                //        FileProperties[0] = "D:\\update\\Debug.rar";//"D:\\zip\\Debug.rar";//待解压的文件
            //                //        FileProperties[1] = path + "jieya";//解压后放置的目标目录
            //                //        //判断是否存在文件夹
            //                //        if (!Directory.Exists(path + "\\jieya"))
            //                //        {
            //                //            Directory.CreateDirectory(path + "\\jieya");
            //                //        }
            ...
            //        else
            //        {
            //            MessageBox.Show("当前版本已是最新版", "系统提示");
            //        }
        ...
        private void Btn_DataBaseRepair_Click(object sender, RoutedEventArgs e)
        {
            ...
            //string str1 = Global.samplenameadapter[0].url;
            ////string str2 = Global.samplenameadapter[0].user;
            ////string str3 = Global.samplenameadapter[0].pwd;
            //int index = str1.LastIndexOf('/');
            //////服务器通讯验证
            //str1 = (index == str1.Length - 1) ? str1 : str1 + "/";
            //sb.Append(str1 + "upload/download.do");
            //sb.AppendFormat("?userToken={0}", Global.Token);
            //sb.AppendFormat("&appName={0}", "DY3500");
            //sb.AppendFormat("&appVersion={0}", "1.0.0");
            //sb.AppendFormat("&param1={0}", "");
            //sb.AppendFormat("&param2={0}", "");
            //sb.AppendFormat("&param3={0}", "");
            ...
            //if (Mresult.msg == "操作成功" && Mresult.success == "true")
            //{
            //    MessageBox.Show("检测有版本更新，版本号为： " + Mresult.obj);
            //}
