...
namespace AIO
{
    ...
    public partial class GszReportWindow : Window
    {
        ...
        public static KJFWGHXItem _item = null;
        public static DYGSZItemPara _itemo = null;
        ...
        {
            if (_item.detect_method == "定性")
            {
                _item.Method = 0;

            }
            else if (_item.detect_method == "定量")
            {
                _item.Method = 1;
            }
            // 计算RGB值
            ...
            for (int i = 0; i < Global.deviceHole.SxtCount; ++i)
            {
                if (_item.Hole[i].Use)
                {
                    ...
                    GSZResult gszResult = oCalGSZFResult(_rgbValues[i], _item);
        ...
        private GSZResult oCalGSZFResult(double[] rgb, KJFWGHXItem mItem)
        {
            ...
            {
                mItem.dx.CompareMinus = 0;//判定符号序号
                mItem.dx.ComparePlus = 1;//判定符号选择序号
                mItem.dx.RGBSelPlus = 1;//RGB值比较的上限
                mItem.dx.RGBSelMinus = 1;//RGB值比较的下限
                mItem.dx.PlusT = double.Parse(mItem.division);
                mItem.dx.MinusT = double.Parse(mItem.division);
                // 定性 阳性
        ...
        private GSZResult CalGSZFResult(double[] rgb, KJFWGHXItem mItem)
        {
            ...
            {   // 定性 阳性
                //if (_item.reserved1 == "A")
                //{
                //    mItem.dx.RGBSelPlus = 0;
                //}
                //else if (_item.reserved1 == "B")
                //{
                //    mItem.dx.RGBSelPlus = 1;
                //}
                //else if (_item.reserved1 == "C")
                //{
                //    mItem.dx.RGBSelPlus = 2;
                //}
