    ...
    { 
        ...
        protected override void HandleMessage(Message msg)
        {
            ...
            switch (msg.what)
            {
                ...
                case MsgCode.MSG_UPLOAD://数据上传
                    ...
                    {
                        if (msg.selectedRecords[i].shoudong == "是" && msg.selectedRecords[i].Result != "检测无效" && msg.selectedRecords[i].Result != "无效" && msg.selectedRecords[i].Result != "NA")//手动测试
                        {
        ...
        public string MutestUpload(tlsTtResultSecond result, string url, string user, out string errMsg)//List<tlsTtResultSecond> 
        {
            ...
            try
            {
                ...
                qsr.taskId = result.taskID;//dt.Rows[0]["t_id"].ToString();//任务ID为空
                qsr.taskName = result.taskNames; //dt.Rows[0]["t_task_title"].ToString();//任务主题
                ...
                DataTable dtf = _bll.GetTestTask("t_id='" + result.taskID+"'", 1, out errMsg);
                if (dtf != null && dtf.Rows.Count > 0)
                {
                    qsr.param1 = dtf.Rows[0]["d_id"].ToString();
                }
                else
                {
                    qsr.param1 = result.taskID; //dt.Rows[0]["param2"].ToString();//预留参数
                }
        ...
        public string QuickUpLoad(tlsTtResultSecond result, string url, string user, string pwd)//List<tlsTtResultSecond> 
        {
            ...
            try
            {
                ...
                DataTable dtf = _bll.GetTestTask("t_id='" + result.taskID + "'", 1, out err);
                if (dtf != null && dtf.Rows.Count > 0)
                {
                    qsr.param1 = dtf.Rows[0]["d_id"].ToString();
                }
                ...
                //qsr.param1 = dt.Rows[0]["d_id"].ToString();//dt.Rows[0]["param2"].ToString();//预留参数
