    ...
    public partial class FgdSelChannelWindow : Window
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    testhole = Global.Testsample.GetLength(0);
                    isduizhao = false;
                    if (null == _item)
                    {
                    ...
                    for (int i = 0; i < testhole; i++)
                    {
                        if (isduizhao == true)
                        {
                            if (i == 0)
                            {
                                listSampleName[i].Text = Global.Testsample[0, 0];
                                _item.Hole[i].tID = Global.Testsample[0, 1];//记录检测任务ID判断数据是否保存过
                            }
                            else
                            {
                                listSampleName[i].Text = Global.Testsample[i-1, 0];
                        ...
                        {
                            listSampleName[i].Text = Global.Testsample[i , 0];
        ...
        private UIElement GenerateChlBriefLayout(int channel, string name)
        {
            ...
            TextBox tbSampleid = new TextBox()
            {
                ...
                Margin = new Thickness(0, 2, 0, 0),
                FontSize = 15,
                Text = _item.Hole[channel].OperatorID ,
                VerticalContentAlignment = VerticalAlignment.Center,
                Name = "Sampleid",
