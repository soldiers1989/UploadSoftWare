    ...
    public partial class SearchSample : Window
    {
        ...
        private void SearchSampleByName()
        {
            try
            {
                ...
                DataTable dataTable = _clsTaskOpr.GetSampleByNameOrCode(textBoxName.Text.Trim(), _projectName, true, _IsFirst, 3);
                if (dataTable != null && dataTable.Rows.Count > 0)
                {
                    //_Items = (List<samplestand>)IListDataSet.DataTableToIList<samplestand>(dataTable, 1);
        ...
        private void DataGridRecord_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            try
            {
                if (DataGridRecord.SelectedItems.Count > 0)
                {
                    ...
                    Global.sampleName = (DataGridRecord.SelectedItem as DataRowView).Row["food_name"].ToString();
                    this.Close();
                }
            }
            catch (Exception ex)
