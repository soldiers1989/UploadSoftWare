       ...
       private StringBuilder sb = new StringBuilder();
        ...
        public DataTable GetAsDataTable(string whereSql, string orderBySql, int type)
        {
            ...
            try
            {
                ...
                else if (type == 2)//根据foodCode获取名称
                {
                    sb.Append("select Name,SysCode from tFoodClass");
                }
        ...
        /// 根据编码查询样品类型
        /// </summary>
        /// <param name="Code">样品编码，需要对样品编码做处理</param>
        /// <returns></returns>
        public static string SearchTypeNameByCode(string Code)
        {
            if (string.IsNullOrEmpty(Code))
            {
                return string.Empty;
