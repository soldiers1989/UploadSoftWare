...
namespace DY.FoodClientLib
{
	...
	public class clsDY5000 : CommBase
	{
		...
		private void AddNewHistoricData(string holes, string item, string abs, string checkValue, string unit, string time)
		{
			...
			_WhereBuilder.AppendFormat(" checkMachine='010' AND HolesNum='{0}'", myDr["孔位"].ToString());
			_WhereBuilder.AppendFormat(" AND MachineItemName='{0}'", myDr["项目"].ToString());
			_WhereBuilder.AppendFormat(" AND CheckStartDate=#{0}#", myDr["检测时间"].ToString());
			myDr["已保存"] = m_ResultOperation.IsExist(_WhereBuilder.ToString());
