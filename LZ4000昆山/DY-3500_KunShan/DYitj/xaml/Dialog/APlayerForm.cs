        ...
        public void LoadPlayer() 
        {
            try
            {
                ...
                for (int i = 0; i < files.Length; i++)
                {
                    ...
                    {
                        _ItemName = _ItemName.Replace("*", "");//去掉名称中的星号
                ...
                //如果没有找到相关视频，则弹出提示是否查看全部视频教程
                if (listView.Items.Count <= 0)
                {
                    if (System.Windows.MessageBox.Show(string.Format("未找到 [{0}] 相关教程视频，是否查看所有？", _ItemName), "操作提示", System.Windows.MessageBoxButton.YesNo, System.Windows.MessageBoxImage.Question) == System.Windows.MessageBoxResult.Yes)
                    {
                        for (int i = 0; i < files.Length; i++)
                        {
                            li = new ListViewItem();
