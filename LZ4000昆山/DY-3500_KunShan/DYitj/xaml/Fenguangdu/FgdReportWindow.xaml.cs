...
namespace AIO
{
    ...
    public partial class FgdReportWindow : Window
    {
        ...
        private void ButtonUpdate_Click(object sender, RoutedEventArgs e)
        {
            if (IsUpLoad)
            {
                MessageBox.Show("正在上传数据或数据已上传!", "系统提示");
                return;
            }
            Upload();
        }
                    ...
                    if (_contrast)
                    {
                        ...
                        for (int i = 0; i < HoleCount; ++i)
                        {
                            if (_use[i] && i != contrastIdx)
                            {
                                ...
                                UnqualifiedValue = UnqualifiedOrQualified(izl);
                                //MessageBox.Show(string.Format("str:{0};val:{1}", str, izl));
                            ...
                            if (_item.Hole[i].Use)
                            {
                                if (_use[i] && i != contrastIdx)
                                {
                                    ...
                                    if (FgdCaculate.IsValid(x[i]))
                                        ...
                                        str = "NA";
                                    UnqualifiedValue = UnqualifiedOrQualified(xfix[i]);
                                    //MessageBox.Show(string.Format("str:{0};val:{1}", str, xfix[i]));
                                    //UnqualifiedValue = TestResultConserve.UnqualifiedOrQualified(str, _item.Hole[i].SampleName, _item.Name);
                                    //UnqualifiedValue = getResult(UnqualifiedValue, str);
                                    _listDetectResult[i].Content = str;
                                    _listDetectResult[i].ToolTip = str;
                    ...
                    if (_sample)
                    {
                        ...
                        for (int i = 0; i < HoleCount; ++i)
                        {
                            ...
                            if (_use[i])
                            {
                                ...
                                UnqualifiedValue = UnqualifiedOrQualified(aPercentFix[i]);
                                //MessageBox.Show(string.Format("str:{0};val:{1}", str, aPercentFix[i]));
                                //UnqualifiedValue = TestResultConserve.UnqualifiedOrQualified(str, _item.Hole[i].SampleName, _item.Name);
                                _listDetectResult[i].Content = str;
                                _listDetectResult[i].ToolTip = _listDetectResult[i];
                                _listJudmentValue[i].Content = Convert.ToString(UnqualifiedValue[0]);
                ...
                else if (4 == _item.Method)
                {
                    ...
                    for (int i = 0; i < HoleCount; ++i)
                    {
                        ...
                        if (_use[i])
                        {
                            ...
                            _listDetectResult[i].Content = str;
                            _listDetectResult[i].ToolTip = str;
                            UnqualifiedValue = UnqualifiedOrQualified(koh[i]);
                            //MessageBox.Show(string.Format("str:{0};val:{1}", str, koh[i]));
                            //UnqualifiedValue = TestResultConserve.UnqualifiedOrQualified(str, _item.Hole[i].SampleName, _item.Name);
                            _listJudmentValue[i].Content = Convert.ToString(UnqualifiedValue[0]);
                            _listJudmentValue[i].ToolTip = _listJudmentValue[i].Content;
                            if (!_listJudmentValue[i].Content.ToString().Equals("合格"))
        ...
        private string[] UnqualifiedOrQualified(double val)
        {
            string[] Qualifide = new string[5];
            //Qualifide[0] = "合格";
            //Qualifide[1] = "GB/T 5009.199-2003";
            //Qualifide[2] = "50";
            //Qualifide[3] = "<";
            //Qualifide[4] = "%";
            switch (_item.Method)
            {
                case 0:
                    //抑制率
                    if (_item.ir.MinusL <= val && _item.ir.MinusH >= val)
                    {
                        Qualifide[0] = "合格";
                    }
                    else
                    ...
                    Qualifide[2] = string.Format("{0} ~ {1}", _item.ir.MinusL, _item.ir.MinusH);
                    break;

                case 1:
                    //标准曲线
                    if (_item.sc.LLevel <= val && _item.sc.HLevel >= val)
                    {
                        Qualifide[0] = "合格";
                    }
                    else
                    ...
                    Qualifide[2] = string.Format("{0} ~ {1}", _item.sc.LLevel, _item.sc.HLevel);
                    break;
                ...
                case 3:
                    //动力学法
                    if (_item.dn.LLevel <= val && _item.dn.HLevel >= val)
                    {
                        Qualifide[0] = "合格";
                    }
                    else
                    ...
                    Qualifide[2] = string.Format("{0} ~ {1}", _item.dn.LLevel, _item.dn.HLevel);
                    break;

                case 4:
                    //系数法
                    //下限 _item.co.LLevel 上限_item.co.HLevel
                    if (_item.co.LLevel <= val && _item.co.HLevel >= val)
                    {
                        Qualifide[0] = "合格";
                    }
                    else
                    ...
                    Qualifide[2] = string.Format("{0} ~ {1}", _item.co.LLevel, _item.co.HLevel);
                    break;
            }
            DataTable dtbl = opr.GetAsDataTable("ttStandDecide", string.Format(" Name = '{0}'", _item.Name), out errMsg);
            if (dtbl != null && dtbl.Rows.Count > 0)
            {
                Qualifide[1] = dtbl.Rows[0]["ItemDes"].ToString();
            }
            else
            {
                Qualifide[1] = "参考国标";
