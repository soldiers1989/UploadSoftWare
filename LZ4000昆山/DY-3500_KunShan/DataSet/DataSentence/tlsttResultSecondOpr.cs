...
namespace DYSeriesDataSet
{
    public class tlsttResultSecondOpr
    {
        ...
        /// <param name="errMsg"></param>
        /// <returns></returns>
        public DataTable GetDtblByCodes(string codes, out string errMsg)
        {
            errMsg = string.Empty;
            DataTable dt = null;
            sql.Length = 0;
            try
            {
                sql.AppendFormat(" Select * from ttResultSecond Where SysCode In {0}", codes);
                string[] cmd = new string[1] { sql.ToString() };
                string[] names = new string[1] { "Result" };
                dt = DataBase.GetDataSet(cmd, names, out errMsg).Tables["Result"];
            }
        ...
        public DataTable ExportData(string whereSql, string InterfaceType, string EACHDISTRICT)
        {
            ...
            try
            {
                ...
                sql.Append(" ReportDeliTime AS 检测报告送达时间,IsReconsider AS 是否提出复议,ReconsiderTime AS 提出复议时间,ProceResults AS 处理结果,CheckedCompanyCode AS 被检单位编号,CheckedCompany AS 被检单位名称,CheckedComDis AS 档口、车牌号、门牌号,CheckPlanCode AS 任务编号, ");
                ...
                //}
                //else if (InterfaceType.Equals("KS"))
                //{
                sql.Append(",DABH AS 经营户身份证号码,PositionNo AS 经营户摊位编号,DABHName AS 经营户姓名,SubItemCode AS 品种编码,SubItemName AS 品种名称,QuickCheckItemCode AS 项目大类编号,QuickCheckSubItemCode AS 项目小类编号");
                //}
                sql.Append(" FROM ttResultSecond ");
                if (!whereSql.Equals(string.Empty))
