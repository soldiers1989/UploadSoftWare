...
namespace WorkstationUI.machine
{
    public partial class ucDY5000 : BasicContent 
    {
        ...
        private void cmbSample_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (DateTime.Now - dtCmbLastClick < new TimeSpan(0, 0, 0, 0, GetDoubleClickTime()))
            {
                ...
                try
                {
                    ...
                    CheckDatas.Rows[CheckDatas.CurrentCell.RowIndex].Cells["检测单位"].Value = Global.BCheckUnitName;
        ...
        protected override void CheckDatas_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            CheckDatas.Rows[CheckDatas.CurrentCell.RowIndex].Cells["样品ID"].Value = Global.BsampleID;
            CheckDatas.Rows[CheckDatas.CurrentCell.RowIndex].Cells["被检单位"].Value = Global.BcheckCommpany;
            CheckDatas.Rows[CheckDatas.CurrentCell.RowIndex].Cells["采样时间"].Value = Global.Bgettime;
            CheckDatas.Rows[CheckDatas.CurrentCell.RowIndex].Cells["采样地点"].Value = Global.Bgetplace ;
            CheckDatas.Rows[CheckDatas.CurrentCell.RowIndex].Cells["SID"].Value = Global.BID;
            CheckDatas.Rows[CheckDatas.CurrentCell.RowIndex].Cells["检测单位"].Value = Global.BCheckUnitName;
            cmbSample.Text = Global.Bsample;
            cmbSample.Visible = false;
        ...
        protected override void btnadd_Click(object sender, EventArgs e)
        {
            ...
            {
                if (CheckDatas.Rows[j].Cells[0].Value.ToString() == "是")
                {
                    nok = nok + 1;
                }
            }
            if(nok==0)
            {
                MessageBox.Show("请保存数据再上传");
                return;
            }
            DialogResult tishi = MessageBox.Show("共有" + nok.ToString() + "条数据是否上传", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        bupdata.testOrganization = dt.Rows[i]["DetectUnit"].ToString();//被检单位
                        ...
                        DateTime tt = DateTime.Parse(dt.Rows[i]["CheckTime"].ToString());
            ...
            try
            {
                ...
                for (int i = 0; i < CheckDatas.Rows.Count; i++)
                {
                    ...
                    if (CheckDatas.Rows[i].Cells["已保存"].Value.ToString() == "False" || CheckDatas.Rows[i].Cells["已保存"].Value.ToString() == "否")
                    {
                        ...
                        resultdata.detectunit = CheckDatas.Rows[i].Cells["检测单位"].Value.ToString().Trim();
                ...
                {
                    MessageBox.Show("共保存" + iok + "条数据，共有"+nok.ToString()+"数据不完整！", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    while (str != null)
                    {
                        ...
                        if (i != 0)//过滤表头  
                        {
                            ...
                            {
                                addTable("", holename, samplename, ItemName, CheckData, unit, testbase, standvalue, "DY5800高通量农药残留测定仪", conclusion, "", CheckTime,"","" ,tester);
                            }
        ...
        private void addTable(string isSave,string Hole,string sample,string item,string result,string unit,string testbase, string standvalue
            ,string machine,string conclusion,string Company,string CheckTime,string gettime,string getplace, string Tester)
        {
