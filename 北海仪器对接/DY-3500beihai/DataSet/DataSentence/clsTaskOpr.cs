    ...
    public class clsTaskOpr
    {
        ...
        public DataTable GetTestData(string where,string orderby,int type,out string errMsg)
        {
            ...
            try
            {
                ...
                {
                    sb.Length = 0;
                    sb.Append("SELECT * FROM BeiHaiSample");
                }
                else if (type == 2)
                {
        ...
        public int InsertTestSample(TestSamples model, out string errMsg)
        {
            ...
            try
            {
                sb.Append("Insert Into BeiHaiSample(productId,goodsName,operateId,operateName,marketId");
                sb.Append(",marketName,samplingPerson,samplingTime,positionAddress,goodsId,IsTest)VALUES(");
                sb.AppendFormat("'{0}',",model.productId);
                sb.AppendFormat("'{0}',", model.goodsName);
                sb.AppendFormat("'{0}',", model.operateId);
                sb.AppendFormat("'{0}',", model.operateName);
                sb.AppendFormat("'{0}',", model.marketId);
                sb.AppendFormat("'{0}',", model.marketName);
                sb.AppendFormat("'{0}',", model.samplingPerson);
                sb.AppendFormat("'{0}',", model.samplingTime);
                sb.AppendFormat("'{0}',", model.positionAddress);
                sb.AppendFormat("'{0}',", model.goodsId);
                sb.AppendFormat("'{0}')", "否");
        ...
        public int DeleteBeiHaiData(string where, string orderby, out string errMsg)
        {
            ...
            try
            {
                sb.Append("Delete from BeiHaiSample");
