            ...
            switch (msg.what)
            {
                ...
                case MsgCode.MSG_CHECK_CONNECTION://通信测试
                    ...
                    try
                    {
                        Global.UnitName = "";
                        //北海接口 通信测试
                        msg.responseInfo = InterfaceHelper.BeihaiCommunicateTest(msg.str1, msg.str2, msg.str3,1, out err);
                        BeiHaiCommuncate rtn = JsonHelper.JsonToEntity<BeiHaiCommuncate>(msg.responseInfo);
                        if (rtn.status == "1")//登录成功过
                        {
                            msg.result = true;
                            Global.UnitID = rtn.unit;
                            Global.UnitName = rtn.unitname;
                        }
                ...
                case MsgCode.MSG_UPLOAD://数据上传
                    ...
                    try
                    {
                        string httpurl = InterfaceHelper.BeiHaiURL(msg.str1, 3);

                        for (int i = 0; i < msg.selectedRecords.Count; i++)
                        {
                            bupdata.productId = msg.selectedRecords[i].SampleId;
                            bupdata.goodsName = msg.selectedRecords[i].FoodName;
                            bupdata.testItem  = msg.selectedRecords[i].CheckTotalItem;
                            bupdata.testOrganization = msg.selectedRecords[i].CheckUnitName;
                            bupdata.checkPerson = msg.selectedRecords[i].CheckUnitInfo;
                            bupdata.checkValue = msg.selectedRecords[i].CheckValueInfo;
                            bupdata.standardValue = msg.selectedRecords[i].StandValue;
                            bupdata.result = (msg.selectedRecords[i].Result=="合格")?"1":"2";
                            bupdata.checkTime = msg.selectedRecords[i].CheckStartDate;
                            js = InterfaceHelper.ToJson(bupdata);

                            rdata = InterfaceHelper.BeihaiCommunicateTest(httpurl + "?paramValue=["+js+"]", msg.str2, msg.str3, 3, out err);
                            if (rdata.Length > 0)
                            {
                                BeiHaiCommuncate rda = JsonHelper.JsonToEntity<BeiHaiCommuncate>(rdata);
                                if (rda.status == "1")//上传成功
                                {
                                    Global.UploadSCount = Global.UploadSCount + 1;
                                    msg.result = true;
                                    _bll.UpdateResult("Y", msg.selectedRecords[i].ID, out err);//更新上传标志
                                }
                                else 
                                {
                ...
                case MsgCode.MSG_DownTask:
                     ...
                     string statu = InterfaceHelper.BeihaiCommunicateTest(msg.str1, msg.str2, msg.str3,2, out err);
                     int d = 0;
                     if (statu.Length > 5)
                     {
                         ...
                         {
                             BeiHaiCommuncate stu = JsonHelper.JsonToEntity<BeiHaiCommuncate>(st);
                             if (stu.status == "2")
                             {
                                 msg.result = true;
                                 msg.DownLoadTask = sample; 
                             }
                         }
                     }
                    if (null != this.target)
