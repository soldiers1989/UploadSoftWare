    ...
    public partial class frmServer : Form
    {
        ...
        private void frmServer_Load(object sender, EventArgs e)
        {
            ...
            if (Global.Platform == "AH")//安徽定制软件
            {
                ...
                txtInstrumentType.Text = ConfigurationManager.AppSettings["IntrumentType"];//仪器类型
                txtIntrumentNum.Text = ConfigurationManager.AppSettings["IntrumentNums"];//仪器编号
                Global.MacAddr = Global.GetMACComputer();
                txtMacAddr.Text =Global.MacAddr= Global.MacAddr.Replace(':','-');// ConfigurationManager.AppSettings["MacAddr"];//MAC地址


                txtInterfaceVer.Text = ConfigurationManager.AppSettings["InterfaceVer"];//接口版本

                labelchkstation.Visible = false;
                labelchkstationNum.Visible = false;
                labelChkunittype.Visible = false;
                labelchkusername.Visible = false;
                labeljigoubianhao.Visible = false;
                labeljigoumingcheng.Visible = false;
                labelMachineCode.Visible = false;
                labelMachineSerialModel.Visible = false;
        ...
        {
            //if (Global.Platform == "AH")//安徽定制软件退出保存软件
            //{
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            Global.ServerName = Txt_User.Text.Trim();
            Global.ServerPassword = Txt_PassWord.Text.Trim();
            Global.InstrumentType = txtInstrumentType.Text.Trim();//仪器类型
            Global.IntrumentNums = txtIntrumentNum.Text.Trim();//仪器编号
            Global.MacAddr = txtMacAddr.Text.Trim();//MAC地址
            Global.InterfaceVer = txtInterfaceVer.Text.Trim();//接口版本
            ...
            //仪器类型
            string IntruType = ConfigurationManager.AppSettings["IntrumentType"];
            if (IntruType != null)
            {
                cf.AppSettings.Settings["IntrumentType"].Value = Global.InstrumentType;
            }
            else
            {
                cf.AppSettings.Settings.Add("IntrumentType", Global.InstrumentType);
            }
            //仪器编号
            string IntruNum = ConfigurationManager.AppSettings["IntrumentNums"];
            if (IntruNum != null)
            {
                cf.AppSettings.Settings["IntrumentNums"].Value = Global.IntrumentNums;
            }
            else
            {
                cf.AppSettings.Settings.Add("IntrumentNums", Global.IntrumentNums);
            }
            //MAC地址
            string MacAddr = ConfigurationManager.AppSettings["MacAddr"];
            if (MacAddr != null)
            {
                cf.AppSettings.Settings["MacAddr"].Value = Global.MacAddr;
            }
            else
            {
                cf.AppSettings.Settings.Add("MacAddr", Global.MacAddr);
            }
            //接口版本
            string InterfaceVer = ConfigurationManager.AppSettings["InterfaceVer"];
            if (InterfaceVer != null)
            {
                cf.AppSettings.Settings["InterfaceVer"].Value = Global.InterfaceVer;
            }
            else
            {
                cf.AppSettings.Settings.Add("InterfaceVer", Global.InterfaceVer);
            }
            cf.Save();
            ConfigurationManager.RefreshSection("appSettings");
            ...
            //System.Xml.XmlDocument doc = new System.Xml.XmlDocument();
            //doc.Load(configFileName);
            //string configString = @"configuration/applicationSettings/WorkstationUI.Properties.Settings/setting[@name='数据采集系统_WebReference1_instrumentDockingServiceProviderService']/value";
            //System.Xml.XmlNode configNode = doc.SelectSingleNode(configString);
            //if (configNode != null)
            //{
            //    configNode.InnerText = Global.ServerAdd;
