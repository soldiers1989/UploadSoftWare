...
namespace WorkstationUI.function
{
    public partial class ucAddUnit : UserControl
    {
        ...
        private void ucAddUnit_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        dt = sql.GetAHCompany("", "", out err);//安徽
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            //}
                            CheckDatas.DataSource = dt;
                            CheckDatas.Columns["isShows"].HeaderText = "默认选择";
                            CheckDatas.Columns["FullName"].HeaderText = "被检单位";
                            CheckDatas.Columns["Incorporator"].HeaderText = "法人代表";
                            CheckDatas.Columns["Address"].HeaderText = "地址";
                            CheckDatas.Columns["LinkMan"].HeaderText = "联系人";
                            CheckDatas.Columns["LinkInfo"].HeaderText = "联系信息";
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnDownCompany_Click(object sender, EventArgs e)
        {
            //被检单位下载 - 安徽
            int icount = 0;
            ...
            if (icount > 0 && error.Length == 0)
            {
                MessageBox.Show("数据已全部下载完成！本次共下载了 " + icount + " 条数据！", "操作提示");
            }
            else if (icount > 0 && error.Length > 0)
            {
                MessageBox.Show("数据下载时出了点小问题！本次共下载了 " + icount + " 条数据！\r\n\r\n异常信息：" + error, "操作提示");
            }
            else if (icount == 0 && error.Length == 0)
            {
        ...
        private void btnAllData_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                icount = data_dictionary + checked_unit + standard_limit;
            }
            catch (Exception ex)
            {
                MessageBox.Show("下载数据时出现异常！\r\n异常信息：" + ex.Message, "error");
            ...
            if (icount > 0 && error.Length == 0)
            {
                MessageBox.Show("数据已全部下载完成！本次共下载了 " + icount + " 条数据！\r\n\r\n" +
        ...
        private void CheckDatas_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.ColumnIndex == 0 && e.RowIndex > -1)
                {
                    ...
                    {
                        if (CheckDatas.Rows[e.RowIndex].Cells["isShows"].Value.ToString() == "False")
                        {
                            CheckDatas.Rows[e.RowIndex].Cells["isShows"].Value = true;
                            if (e.RowIndex > -1)
                            {
                                Global.tCompany = CheckDatas.Rows[e.RowIndex].Cells["FullName"].Value.ToString();
                                //Global.shengchandanwei = CheckDatas.Rows[e.RowIndex].Cells["检测单位"].Value.ToString();
                                //Global.chandidizhi = CheckDatas.Rows[e.RowIndex].Cells["检测单位地址"].Value.ToString();
                                //Global.shengchanqiye = CheckDatas.Rows[e.RowIndex].Cells["检测人"].Value.ToString();
                            ...
                            for (int j = 0; j < CheckDatas.Rows.Count; j++)
                            {
                                if (j != e.RowIndex)
                                {
                                    CheckDatas.Rows[j].Cells["isShows"].Value = false;
                                    sql.UpdatetCompany(CheckDatas.Rows[j].Cells["ID"].Value.ToString(),"False");
                                ...
                                {
                                    sql.UpdatetCompany(CheckDatas.Rows[j].Cells["ID"].Value.ToString(), "True");
                                    //sql.updateBasicIn(CheckDatas.Rows[j].Cells["默认选择"].Value.ToString(), CheckDatas.Rows[j].Cells["检测单位"].Value.ToString(),
                                    //            CheckDatas.Rows[j].Cells["检测单位地址"].Value.ToString(), CheckDatas.Rows[j].Cells["检测人"].Value.ToString(),
                                    //            CheckDatas.Rows[j].Cells["被检单位"].Value.ToString(), CheckDatas.Rows[j].Cells["被检单位性质"].Value.ToString(),
                                    //            CheckDatas.Rows[j].Cells["产地"].Value.ToString(), CheckDatas.Rows[j].Cells["生产单位"].Value.ToString(), out err);
                        ...
                        {
                            CheckDatas.Rows[e.RowIndex].Cells["isShows"].Value = false;
                            sql.UpdatetCompany(CheckDatas.Rows[e.RowIndex].Cells["ID"].Value.ToString(), "False");
