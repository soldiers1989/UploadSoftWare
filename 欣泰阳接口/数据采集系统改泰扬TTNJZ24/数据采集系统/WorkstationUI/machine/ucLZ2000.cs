...
namespace WorkstationUI.machine
{
    ...
    {    
        ...
        private void ucLZ2000_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                DataTable dt = sqlSet.GetSample("IsLock=false And IsReadOnly=true and CheckItemCodes like '%{" + "000029000001" + ":%'", "SysCode", out err);
                if (dt != null)
                {
                    if (dt.Rows.Count > 0)
                    {
                                ...
                                case "≤":
                                    {
                                        ...
                                        {
                                            CheckDatas.Rows[CheckDatas.CurrentCell.RowIndex].Cells[8].Value = "阴性";
                                        }
                                        else
                                        {
                                            CheckDatas.Rows[CheckDatas.CurrentCell.RowIndex].Cells[8].Value = "阳性";
                                        }
                                    }
                                    break;
                                case "<":
                                    ...
                                    case "≤":
                                        {
                                            ...
                                            {
                                                CheckDatas.Rows[j].Cells[8].Value = "阴性";
                                            }
                                            else
                                            {
                                                CheckDatas.Rows[j].Cells[8].Value = "阳性";
                                            }
                                        }
                                        break;
                                    case "<":
        ...
        private byte[] ReadAndWriteToDevice()
        {
            ...
            for (int i = 0; i < CheckDatas.Rows.Count; i++)
            {
                if (CheckDatas.Rows[i].Cells[8].Value.ToString() == "阳性" || CheckDatas.Rows[i].Cells[8].Value.ToString() == "不合格")
                {
                    CheckDatas.Rows[i].DefaultCellStyle.ForeColor = Color.Red;
                }
                CheckDatas.Rows[i].Cells[7].Value = Global.ChkManchine;
        ...
        protected override void CheckDatas_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (CheckDatas.CurrentCell.ColumnIndex > -1 && CheckDatas.CurrentCell.RowIndex > -1)
                {
                    ...
                    else if (CheckDatas.CurrentCell.ColumnIndex > 8 && CheckDatas.CurrentCell.ColumnIndex < 14)
                    {
                        cmbSample.Visible = false;
                        cmbChkItem.Visible = false;
