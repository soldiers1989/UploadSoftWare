                ...
                if (count > 0)
                {
                    ...
                    if (_atpLsit.Count > 0)
                    {
                        ...
                        foreach (clsATP item in _atpLsit)
                        {
                            ...
                            dr["检测单位"] = cdt.Rows[0][0].ToString();//检测单位
                            dr["采样时间"] = "";
                            dr["采样地点"] = cdt.Rows[0][3].ToString();
                            dr["被检单位"] = cdt.Rows[0][2].ToString();
                            dr["检测员"] = cdt.Rows[0][8].ToString();
                        ...
                        {
                            if (CheckDatas.Rows[i].Cells[8].Value.ToString() == "警告")
                            {
                                CheckDatas.Rows[i].DefaultCellStyle.ForeColor = Color.Orange;
                            }
                            else if (CheckDatas.Rows[i].Cells[8].Value.ToString() == "超标")
                            {
        ...
        private void btnadd_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                for (int j = 0; j < CheckDatas.Rows.Count; j++)
                {
                    ...
                    sqldata.UpdateTempResult("是", CheckDatas.Rows[j].Cells[1].Value.ToString(), out err);
                }
            }
        }
        /// <summary>
