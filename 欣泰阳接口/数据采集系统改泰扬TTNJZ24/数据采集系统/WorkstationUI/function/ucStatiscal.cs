...
namespace WorkstationUI.function
{
    public partial class ucStatiscal : UserControl
    {
        ...
        private void generateChart()
        {
            ...
            for (int j = 1; j < cmbSample.Items.Count; j++)
            {
                ...
                for (int n = 0; n < DataReadTable.Rows.Count; n++)
                {
                    ...
                    string s = DataReadTable.Rows[n][2].ToString();
                    if (cmbSample.Items[j].ToString() == DataReadTable.Rows[n][2].ToString())// CheckDatas.Rows[n].Cells[2].Value.ToString()
                    {
                        sam = sam + 1;
                    }
                }
            ...
            for (int j = 1; j < cmbTestItem.Items.Count; j++)
            {
                ...
                for (int n = 0; n < DataReadTable.Rows.Count; n++)
                {                  
                    if (cmbTestItem.Items[j].ToString() == DataReadTable.Rows[n][1].ToString())
                    {
                        Itm = Itm + 1;
                    }
                }             
        ...
        private void CmbgetItemName()
        {
            ...
            for (int i = 0; i < DataReadTable.Rows.Count; i++)
            {
                string name = DataReadTable.Rows[i][1].ToString();//this.CheckDatas.Rows[i].Cells[1].Value.ToString()
        ...
        private void CmbgetSampleName()
        {
            ...
            for (int i = 0; i < DataReadTable.Rows.Count; i++)
            {
                string name = DataReadTable.Rows[i][2].ToString();
                if (lsName.Contains(name))
                {
                    continue;
                }
        ...
        private void CheckDatas_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
