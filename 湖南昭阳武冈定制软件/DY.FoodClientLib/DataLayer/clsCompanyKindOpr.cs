	...
	public class clsCompanyKindOpr
	{
      ...
      public DataTable GetAsDataTable(string whereSql, string orderBySql, int type)
      {
          ...
          try
          {
              ...
              if (type == 0)
              {
                  sb.Append("SELECT SYSCODE,STDCODE,NAME,COMPANYPROPERTY,ISLOCK,ISREADONLY,REMARK FROM TCOMPANYKIND");
              }
              else if (type == 1)
              {
      ...
      public int Insert(clsCompanyKind model, out string errMsg)
      {
          ...
          {
              sb.Length = 0;
              sb.Append("INSERT INTO tCompanyKind(SysCode,StdCode,Name,CompanyProperty,IsReadOnly,IsLock,Remark)");
              sb.Append(" VALUES(");
