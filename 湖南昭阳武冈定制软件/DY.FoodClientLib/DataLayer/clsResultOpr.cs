	...
	public class clsResultOpr
	{
        ...
        public DataTable GetUploadDataTable(string strWhere, string orderBy,string appTag)
        {
            ...
            try
            {
                ...
                {
                    sb.Append("A.CheckedCompanyName AS CheckedCompany, ");
                }
                else
                {
		...
		public DataTable GetDataTable_ReportEx(string whereSql, string orderBySql)
		{
			...
			{
				sb.Append("SELECT");
				sb.Append(" (select ShortName from tUserUnit where sysCode='" + ShareOption.DefaultUserUnitCode + "') As ShortName,");
		...
		public DataTable GetDataTable_ReportGZ(string whereSql, string orderBySql)
		{
			...
			{
				sb.Append("SELECT");
				sb.AppendFormat(" (SELECT ShortName from tUserUnit where sysCode='{0}') As ShortName,A.CheckNo,",ShareOption.DefaultUserUnitCode);
                ...
                sb.Append("(select LinkMan from tCompany where A.ProduceCompany=tCompany.SysCode And tCompany.Property='生产单位') As ProduceLinkMan,(select LinkInfo from tCompany where A.ProduceCompany=tCompany.SysCode And tCompany.Property='生产单位') As ProduceLinkInfo,B.CompanyID,B.Incorporator,B.LinkInfo,B.PostCode,B.FullName As ComName,B.Address,");
				sb.Append("A.SampleState,A.Provider,A.SampleNum,A.SampleBaseNum,A.ImportNum,A.SentCompany,A.SaveNum,A.Remark,A.TakeDate,");
                ...
                sb.Append("B.ComProperty,(select FullName from tUserUnit where sysCode='0001') As CheckUnitFullName,(select LinkMan from tUserUnit where sysCode='0001') As CheckUnitLinkMan,(select LinkInfo from tUserUnit where sysCode='0001') As CheckUnitLinkInfo,");
