	...
	public class clsDY723PC : CommBase
	{
		...
		public clsDY723PC()
		{
			if (!IsCreatDT)
			{
				...
				dataCol = new DataColumn();
				dataCol.DataType = typeof(string);
				dataCol.ColumnName = "检测值";//定量结果
				checkDtbl.Columns.Add(dataCol);

				//dataCol = new DataColumn();
				//dataCol.DataType = typeof(string);
				//dataCol.ColumnName = "定性结果";//就是检测结论，合格/不合格
				// checkDtbl.Columns.Add(dataCol);
		...
		protected override void OnRxChar(byte bty)
		{
			...
			if (len >= 6)//FINISH,取最后结束符 
			{
				...
				if (recData.IndexOf("FINISH") >= 0)//数据接收完成
				{
					...
					{
						MessageNotify.Instance().SendMessage(MessageNotify.NotifyInfo.ReadDY723PCData, "暂无符合条件数据");
					}
					...
					{
						MessageNotify.Instance().SendMessage(MessageNotify.NotifyInfo.ReadDY723PCData, "OK");
					}
		...
		private void GetXmlFile()
		{
			...
			if (MessageNotify.Instance() != null)
			{
				MessageNotify.Instance().SendMessage(MessageNotify.NotifyInfo.ReadDY723PCData, "OK");//bty.ToString("X2")
			}
		}
